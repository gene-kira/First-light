...
# ============================================================
# Rogue Process Monitor v13.2 (SAFE, NON-DESTRUCTIVE)
# Ultra-Optimized Edition
# - Full sandbox isolation
# - Incremental / diff-based GUI updates
# - Virtualized-ish live view (limited rows)
# - Throttled alerts
# - Smoothed threat meter
# - Still NO killing, blocking, or destructive actions
# ============================================================

LOG_FILE = "rogue_monitor_v13_2_log.txt"
...
sandbox_threat_level_raw = 0  # raw threat level
sandbox_threat_level_smoothed = 0  # smoothed
...
def sandbox_set_threat_level(level):
    ...
    with sandbox_lock:
        ...
        # simple smoothing: new = 0.7*old + 0.3*raw
...
class SandboxScanner:
    ...
    def scan_loop(self):
        log("SandboxScanner v13.2 scan loop started")
        while self.running:
            try:
    ...
    def tree_loop(self):
        log("SandboxScanner v13.2 tree loop started")
        while self.running:
            try:
                self.build_tree()
            except Exception as e:
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v13.2 (SAFE, Ultra-Optimized Sandbox)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v13.2 (SAFE, Sandbox)", font=("Segoe UI", 14, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))

        desc = ttk.Label(
            frame,
            text=(
                "All heavy scanning runs in a separate sandbox.\n"
                "GUI uses incremental updates and throttling to stay responsive.\n"
...
def main():
    log("Rogue Process Monitor v13.2 (SAFE, Ultra-Optimized Sandbox) starting")
    root = tk.Tk()
