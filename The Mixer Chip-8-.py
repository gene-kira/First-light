"""
Golden Star MPU – AI Security Engine v11

Upgrades in this revision:
- DPIEngine:
  - Tuned Suricata rule parsing:
    * Parse header: proto, src/dst ports, direction
    * Extract content and pcre patterns
    * Map to DPIGroup and tags
  - YARA integration kept, with clearer mapping into DPIRule-style matches
- PCAPIngestor:
  - Real libpcap file parsing (global header + per-packet records)
  - Extract raw frame bytes for PacketParser

Still safe:
- Listener is local UDP only
- No privileged live capture; PCAP is file-based
"""
...
# ============================================================
# DPI ENGINE – TUNED SURICATA/YARA
# ============================================================
...
class DPIEngine:
    ...
    def _parse_suricata_header(self, header: str) -> Tuple[Optional[L4Proto], Optional[List[int]], FlowDirection]:
        # Example: alert tcp any any -> any 80
        proto = None
        ports = None
        ...
        # src_port = parts[3], dst_port = parts[6]
        try:
        ...
        # direction: parts[4] should be '->' or '<>'
        if parts[4] == "->":
        ...
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    # Extract msg, content, pcre, classtype, priority
                    msg = None
                    raw_pattern = None
                    pcre_pattern = None
                    severity = 5
                    ...
                    m_msg = re.search(r'msg:"([^"]+)"', body)
                    if m_msg:
                        msg = m_msg.group(1)

                    m_content = re.search(r'content:"([^"]+)"', body)
                    if m_content:
                        raw_pattern = m_content.group(1).encode("utf-8")

                    m_pcre = re.search(r'pcre:"([^"]+)"', body)
                    if m_pcre:
                        pcre_pattern = m_pcre.group(1)

                    m_prio = re.search(r'priority:(\d+)', body)
                    if m_prio:
                        try:
                            prio = int(m_prio.group(1))
                            # Map priority to severity (inverse-ish)
                            severity = max(1, 10 - prio)
                        except Exception:
                            pass

                    group = DPIGroup.GENERIC
                    body_lower = body.lower()
                    if "http" in body_lower:
                        group = DPIGroup.HTTP
                    if "dns" in body_lower:
                        group = DPIGroup.DNS
                    if "tls" in body_lower or "ssl" in body_lower:
                        group = DPIGroup.TLS

                    name = msg or "SURICATA_RULE"
                    rule = DPIRule(
                        name=name,
                        group=group,
                        severity=severity,
...
# ============================================================
# NETWORKING – REAL PCAP PARSER
# ============================================================

class PCAPIngestor:
    """
    Minimal libpcap file parser:
    - Reads global header
    - Iterates packet records
    - Returns raw frame bytes (packet data)
    """
    ...
    def ingest_file(self, path: str) -> List[bytes]:
        ...
        try:
            ...
            # We don't deeply validate; just skip header
            offset = 24
