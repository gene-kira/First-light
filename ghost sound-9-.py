...
def update_trust_score(proc_key, outcome: str):
    # outcome: "benign", "hostile", "blocked"
...
def get_trust_adjustment(proc_key):
    ...
    # More benign hits → lower score; more hostile/blocked → higher score
...
def detect_audio_spikes(audio_sessions):
    # Compare current audio levels to last snapshot
...
class SecurityBridgeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Security Bridge - v8 (Operator Mode: Adaptive Trust + Audio Correlation + MITRE Views)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            top_frame,
            text="Security Bridge v8 - Operator Mode",
    ...
    def mark_cluster_benign(self):
        ...
        for c in clusters:
            if c.get("pid") == pid:
                # mark all with same parent or external IPs as benign
...
def main():
    log_event("INFO", "Security Bridge (Full Upgrade v8) started.")
    start_honeypot()
    root = tk.Tk()
    app = SecurityBridgeGUI(root)

    # Add context menu for cluster actions
