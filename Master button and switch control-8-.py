#!/usr/bin/env python
# Codex Purge Shell v4.2 (Safe Edition) — Win11 Security Bridge & Adjustment Console
# - Auto-elevation
...
# - Hourly baseline snapshot logic (no snapshot spam)
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4.2] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4_2"
        else:
            self.model = None
...
# =========================
# Main GUI / Codex Purge Shell v4.2
# =========================

class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4.2 (Safe Edition) — Win11 Security Bridge & Adjustment Console")
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4.2 (Safe Edition) — Windows 11 Security Bridge & Adjustment Console",
            font=("Arial", 16)
        )
        ...
        self.log_alert("Codex Purge Shell v4.2 (Safe Edition) started with auto-elevation and hourly baseline snapshots.")
        ...
        # Right notebook: Threat Matrix, Suricata, Overlays, Honeypot, Event Bus
    ...
    # =========================
    # Monitoring loops (hourly baseline)
    # =========================
    ...
    def _monitor_loop(self):
        ...
        if self.baseline_fw is None:
            ...
            snap = create_snapshot({"summary": "Initial baseline snapshot"})
            self.log_alert(f"[Baseline] Snapshot created: {snap}")
        ...
        while True:
            ...
            now = time.time()
            one_hour_passed = (now - self.last_snapshot_time) >= 3600
            ...
            if one_hour_passed or wu_running:
                changes = []
                ...
                if prof_hash != self.baseline_profiles:
                    changes.append("Profiles")

                if set_hash != self.baseline_settings:
                    changes.append("Settings Store")

                if changes:
                    self.log_alert(f"[Baseline Compare] Changes detected after 1 hour or during updates: {changes}")
                    meta = {
                        "summary": f"Hourly snapshot: {changes}",
                        "changes": changes,
                        "timestamp": now,
                        "wu_running": wu_running,
                    }
                    snap = create_snapshot(meta)
                    self.log_alert(f"[Snapshot] Hourly snapshot created: {snap}")
