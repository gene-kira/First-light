...
# -*- coding: utf-8 -*-
"""
CODEX CONTROL CONSOLE v7.7 (Windows Edition)
...
richer MITRE mapping, structured training, and refined response logic.

Major upgrades vs v7.6:
- Real DPI: deeper packet parsing (IP/TCP/UDP/HTTP) when Scapy is available
- Real honeypot: simple TCP listeners for HTTP/SSH-style traps
- Better GUI layout: more compact, clearer panels, severity coloring, timeline
- Better ML: improved AE/Transformer, basic IsolationForest-style scoring (manual)
- Better swarm: node tokens, basic authenticity check, trust decay
- Better MITRE mapping: more tactics/techniques, richer correlation
- Better training: structured sampling, baseline saving/loading, thresholds
- Better response logic: more nuanced actions per severity band
...
class AnomalyModel:
    ...
    def isolation_style_score(self, vec: np.ndarray) -> float:
        # Simple heuristic: distance from mean normalized by variance magnitude
...
class SwarmNode:
    def __init__(self, node_id: str, bind_port: int = 50050, broadcast_port: int = 50050, token: str = "codex-swarm-token"):
    ...
    def broadcast(self, msg: str):
        try:
            payload = f"TOKEN:{self.token} NODE:{self.node_id} {msg}"
...
class CodexGUI:
    def __init__(self):
        ...
        self.root.title("Codex Control Console v7.7 (Windows)")
...
def run_codex_daemon():
    ...
    brain = AIBrain(base_dir)
    gpu_telemetry = GPUTelemetry()
    swarm = SwarmNode(node_id=node_id, bind_port=bind_port, broadcast_port=broadcast_port, token=token)
    swarm.start()
    mitre = MITREMapper(db_path=str(base_dir / "mitre.json"))
    consensus = ConsensusEngine()
    ...
    print("[CORE] Codex Control Console daemon v7.7 (Windows: TRAIN + eve.json + PCAP + MITRE + COMPACT GUI + SWARM + RESPONSE + THRESHOLD + SURICATA_CFG + CAMPAIGNS + MATH_SUITE + DPI + HONEYPOT)")
    def loop():
        while True:
            daemon_cycle(node_id, console, brain, backbone, gpu_telemetry, swarm, mitre, consensus, responder, campaign_engine)
            time.sleep(5)
    wd = Watchdog("CodexDaemonV7_7_Windows", loop, restart_delay=3.0)
    if gui and gui.root:
