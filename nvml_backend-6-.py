...
# -*- coding: utf-8 -*-
"""
Universal GPU Control Center v4.3
Full Controls + Persistence Edition

Upgrades over v4.2:
- Full GUI control panel:
  - Per‑GPU power limit slider
  - Per‑GPU fan speed slider
  - Global aggression selector
  - Save button (writes config to disk)
- Persistent tuning:
  - All tuning changes (aggression, per‑GPU overrides) saved to system_tuner_config.json
  - Restored automatically on next start
- REST API extensions:
  - /tuning/gpu_override (set per‑GPU power/fan override)
  - /config/save (force save)
  - /config/get (read current config)
- Auto mode uses persisted overrides and aggression.
...
class GuiShell:
    ...
    def start(self) -> None:
        ...
        for idx, sample in sorted(latest_by_gpu.items()):
            ...
            power_slider.setValue(int(CONFIG["tuning"]["gpu_overrides"].get(str(idx), {}).get("power_target_percent",
            ...
            fan_slider.setValue(int(CONFIG["tuning"]["gpu_overrides"].get(str(idx), {}).get("fan_percent",
