...
- Pruning + TFLite dynamic-range quantization.
"""
...
def apply_pruning(model, end_step):
    ...
    Wrapping individual Conv3D/Dense layers with prune_low_magnitude()
    can fail with newer Keras / tensorflow_model_optimization combinations.
    Pruning the complete model is the supported and more robust path.
    """
    ...
    try:
        ...
        logging.exception("Direct model pruning failed: %s", exc)
        raise RuntimeError(
            "TensorFlow Model Optimization could not prune the complete "
            "Keras model. Make sure TensorFlow and "
            "tensorflow-model-optimization are compatible."
        ) from exc
...
class TemporalTrainingEngine:
    ...
    def run_cycle(self, progress_cb=None, stop_check_cb=None):
        ...
        for model_key in model_keys:
            ...
            check_stop()
            emit(f"[{model_key}] Applying magnitude pruning...")
            ...
            emit(f"[{model_key}] Stripping pruning wrappers and saving models...")
            ...
            emit(f"[{model_key}] Evaluating pruned model...")
            ...
            check_stop()
            emit(f"[{model_key}] Converting model to quantized TFLite...")
...
class TrainingMonitor(QtWidgets.QMainWindow):
    ...
    def on_completed(self, result):
        self.consecutive_failures = 0
        # result is a dict keyed by model_key
