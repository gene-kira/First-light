...
"""
BORG OS v18 – Gaming-Safe Unified Accelerator (CPU/GPU/NPU) + RL/DPI/Threat + Mesh + Command Center
+ One-Time Full Game Scan + Incremental New Game Detection + Manifest Watcher

Changes vs v17:
- One-time full game scan on first run (Steam/Epic/drives).
- borg_settings table with first_scan_done flag.
- Incremental game detection on later runs:
  - Only scans for new Steam/Epic manifests and new game folders.
  - Uses borg_games as baseline to avoid re-adding existing games.
- Background manifest watcher:
  - Periodically checks Steam/Epic manifest folders for new installs.
  - Auto-adds new games to whitelist and mesh SAFE votes.
"""
    ...
    def _kill_process(self, p, score, gpu_load, hard_protected, learned_safe, sigs, gpu_mem, overloaded):
        try:
            ...
            if ("steam" in lname or "epic" in lname or "battle.net" in lname or
                ...
                self._monitor_process(p, score, hard_protected, learned_safe, sigs, gpu_mem, overload=overloaded)
                return
...
if FASTAPI_AVAILABLE:
    app = FastAPI(title="Borg OS Node", version="18.0.0")
...
class BorgGUI:
    def __init__(self, root, core: BorgCore):
        ...
        root.title("BORG OS NODE v18 – Command Center (Accelerator + Threat + Deep RL + Mesh + DPI + Games)")
...
def run_cli():
    print("BORG OS NODE (CLI MODE, Autonomous v18, Accelerator + Threat + Deep RL + Mesh + DPI, Gaming-Safe + One-Time Scan + Incremental Detection)")
    print(f"OS: {OS_NAME}")
