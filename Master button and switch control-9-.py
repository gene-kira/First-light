#!/usr/bin/env python
# Codex Purge Shell v4.3 (Safe Edition) — Win11 Security Bridge & Adjustment Console
# - Auto-elevation
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4.3] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4_3"
        else:
            self.model = None
...
class GUIWatchdog:
    ...
    def _handle_violation(self, name, widget, exists, visible):
        ...
        # snapshot is still subject to cooldown in main app
...
# =========================
# Main GUI / Codex Purge Shell v4.3
# =========================

class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4.3 (Safe Edition) — Win11 Security Bridge & Adjustment Console")
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4.3 (Safe Edition) — Windows 11 Security Bridge & Adjustment Console",
            font=("Arial", 16)
        )
        ...
        self.log_alert("Codex Purge Shell v4.3 (Safe Edition) started with auto-elevation, cooldown snapshots, and stability fixes.")
    ...
    def _monitor_loop(self):
        ...
        while True:
            ...
            if (one_hour_passed and persistent_changes) or allow_update_snap:
                ...
                if snap:
                    # Update baseline only if snapshot actually happened
