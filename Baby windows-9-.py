#!/usr/bin/env python3
"""
System Governor v2.2
Resilient, self-learning, physics-inspired system governor with AI-style + DL-style prediction

New in v2.2:
- Windows Usage Breakdown Analyzer (passive, read-only)
- Tiered usage classification: 1 (always), 2 (≈50%), 3 (≈25%), 4 (never)
- Full enable/disable control from GUI
- No automatic setting changes — analyzer only observes and reports
- Persistent analyzer settings across reboot (tiers, enabled state, preferences)
"""
...
LOG_PATH = os.path.join(LOG_DIR, "system_governor_v2_2.log")
...
class WindowsUsageAnalyzer:
    ...
    def reset_to_normal(self):
        """
        Reset analyzer tiers to empty, but does NOT change Windows settings.
        Only clears analyzer's internal classification.
        """
        with self.lock:
            self.tiers = {"1": [], "2": [], "3": [], "4": []}
            CONFIG["windows_usage_analyzer"]["tiers"] = self.tiers
...
def create_tray_icon(on_show_callback, on_exit_callback):
    ...
    icon = pystray.Icon("SystemGovernor", img, "System Governor v2.2", menu)
    return icon
...
class GovernorWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("System Governor v2.2")
