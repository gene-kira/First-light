...
# Base layout from v12 + full Security Bridge backend + SecurityBridgeGUI tabs
# Auto‑scale GUI width, Lockdown + Reset + Audio Lockdown first in line (left‑aligned)
# Audio Lockdown: green → red, stays red until RESET, deep scan + intruder tracking
...
def ensure_libs():
    ...
    globals()["requests"] = importlib_import_module("requests") if False else importlib.import_module("requests")
    globals()["pefile"] = importlib.import_module("pefile")
    globals()["pythoncom"] = importlib_import_module("pythoncom") if False else importlib.import_module("pythoncom")
...
# -----------------------------
# Sandbox / auto-block / snapshot / clusters
# -----------------------------
...
# -------------------------------------------------------------------------
# Codex Control Console GUI (base layout) + SecurityBridge tabs
# -------------------------------------------------------------------------
class CodexControlConsole(tk.Tk):
    def __init__(self):
        ...
        self.audio_lockdown_button = None
        ...
        start_honeypot()
        log_event("INFO", "Codex Control Console v13 started (Security Bridge integrated).")
    ...
    def _build_layout(self, target_w, target_h):
        ...
        # ROW 2: Core controls (Codex + Security Bridge)
        ...
        # Right: intruder details + actions + SecurityBridge notebook
        right_frame = ttk.LabelFrame(main, text="Intruder Details / Actions + Security Bridge")
        ...
        # SecurityBridge notebook embedded in right_frame
        self.notebook = ttk.Notebook(right_frame)
        self.notebook.grid(row=3, column=0, sticky="nsew", padx=6, pady=(0, 6))
        ...
        menu.add_command(label="Kill selected process", command=self.kill_selected_process)
        menu.add_command(label="Sandbox selected", command=self.sandbox_selected)
        menu.add_command(label="Resume selected", command=self.resume_selected)
        menu.add_separator()
        menu.add_command(label="Mark selected safe-to-ignore", command=self.mark_safe_selected)
        menu.add_separator()
        menu.add_command(label="Mark cluster benign", command=self.mark_cluster_benign)
        menu.add_command(label="Mark cluster hostile", command=self.mark_cluster_hostile)
    ...
    def _add_row2_buttons(self, parent):
        buttons = [
            ...
            ("Auto‑refresh", self.on_auto_refresh_toggle),
            ("Mode", self.on_mode_button),
            ("Profile", self.on_profile_button),
            ("Generate report", self.generate_report_gui),
            ("Incident pack", self.incident_pack_gui),
            ("Freeze session", self.freeze_session_gui),
        ]
    ...
    def _add_row3_buttons(self, parent):
        buttons = [
            ("Kill high‑risk", self.kill_all_high_risk),
            ("Suspend remote", self.suspend_remote_high),
            ("Mute suspicious audio", self.mute_suspicious_audio),
            ("Silence non‑media", self.silence_non_media_audio),
            ("Quarantine remote tools", self.quarantine_remote_tools),
            ("Snapshot + pack", self.snapshot_and_pack),
        ]
        for i, (label, cmd) in enumerate(buttons):
            b = ttk.Button(parent, text=label, style="Control.TButton", command=cmd)
            b.grid(row=0, column=i, padx=(0, 6))
    ...
    # -------------------------------------------------------------------------
    # Codex top-row handlers (integrated with backend)
    # -------------------------------------------------------------------------
    def on_lockdown_pressed(self):
        self.lockdown_full_kill_gui()

    def on_reset_pressed(self):
        self.reset_system_gui()

    def on_audio_lockdown_pressed(self):
        self.audio_lockdown_gui()

    # -------------------------------------------------------------------------
    # Deep scan / intruder logic (now backed by Security Bridge snapshot)
    # -------------------------------------------------------------------------
    def _start_deep_scan(self, source="UNKNOWN"):
        if self.deep_scan_running:
            self._log("Deep scan already running.")
            return

        self.deep_scan_running = True
        self._log(f"Deep scan started (source: {source}). Searching for intruder and problem...")

        t = threading.Thread(target=self._deep_scan_thread, args=(source,), daemon=True)
        t.start()

    def _deep_scan_thread(self, source):
        try:
    ...
    def _on_scan_complete(self):
        ...
        self.lbl_intruder_detail.config(
            text=f"Intruder detected:\n  {self.intruder_name}\n\nSource trace:\n  Audio chain → process → module."
        )
        self.lbl_problem_detail.config(
            text=f"Problem detected:\n  {self.problem_desc}\n\nLockdown evidence retained."
        )

        self._log(f"Intruder found: {self.intruder_name}")
        self._log(f"Problem detected: {self.problem_desc}")
        self._log("System is ready for confirmation and auto‑kill.")
    ...
    # -------------------------------------------------------------------------
    # Intruder actions (Codex + backend)
    # -------------------------------------------------------------------------
    ...
    def on_auto_kill(self):
        ...
        try:
            name = self.intruder_name
            if "PID" in name:
                pid_str = name.split("PID")[-1].strip(" )")
                pid = int(pid_str)
                kill_process(pid)
                self._log("Intruder terminated. Audio source chain severed.")
                self.lbl_intruder_detail.config(
                    text=f"Intruder terminated:\n  {self.intruder_name}\n\nAudio chain neutralized."
                )
            else:
                self._log("Intruder PID not parsable; kill simulated only.")
        except Exception as e:
            self._log(f"Intruder kill failed: {e}")

    def on_rescan(self):
        if not self.intruder_found:
            self._log("No intruder to rescan.")
            return

        self._log("ONE MORE SCAN requested to confirm this is the only problem.")
        self._start_deep_scan(source="RESCAN")

    # -------------------------------------------------------------------------
    # Row 2 / Row 3 handlers (Codex + Security Bridge)
    # -------------------------------------------------------------------------
    def on_refresh(self):
        self._log("Manual refresh triggered.")
        self.refresh_snapshot()

    def on_auto_refresh_toggle(self):
        self.auto_refresh.set(not self.auto_refresh.get())
        self._log(f"Auto‑refresh toggled: {self.auto_refresh.get()}")

    def on_mode_button(self):
        current = self.mode_var.get()
        new = "SILENT" if current == "AGGRESSIVE" else "AGGRESSIVE"
        self.mode_var.set(new)
        self.on_mode_change()

    def on_profile_button(self):
        current = self.profile_var.get()
        keys = list(AUTO_BLOCK_PROFILES.keys())
        try:
    ...
    def on_auto_refresh(self):
        self._log("Auto‑refresh toggled (placeholder).")

    def on_mode(self):
        self._log("Mode button pressed (placeholder).")

    def on_profile(self):
        self._log("Profile button pressed (placeholder).")

    def on_generate_report(self):
        self._log("Generate report (placeholder).")

    def on_incident_pack(self):
        self._log("Incident pack (placeholder).")

    def on_freeze_session(self):
        self._log("Freeze session (placeholder).")

    def on_kill_high_risk(self):
        self._log("Kill high‑risk (placeholder).")

    def on_suspend_remote(self):
        self._log("Suspend remote (placeholder).")

    def on_mute_suspicious_audio(self):
        self._log("Mute suspicious audio (placeholder).")

    def on_silence_non_media(self):
        self._log("Silence non‑media (placeholder).")

    def on_quarantine_remote_tools_placeholder(self):
        self._log("Quarantine remote tools (placeholder).")

    def on_snapshot_pack(self):
        self._log("Snapshot + pack (placeholder).")

    # -------------------------------------------------------------------------
    # SecurityBridge GUI methods (adapted)
    # -------------------------------------------------------------------------
    ...
    def on_mitre_view_change(self, event=None):
        self.update_timeline()
        self.update_remote_panel(getattr(self, "last_snapshot", []))
    ...
    def audio_lockdown_gui(self):
        ...
        save_state(STATE)
        self.btn_audio_lockdown.configure(
        ...
        if result == "protected":
            self.status_var.set("Audio lockdown: user denied kill, process protected.")
            messagebox.showinfo(
                "AUDIO LOCKDOWN",
                "You said NO.\n\nThis process is now protected:\n"
                "- Auto-block disabled\n"
                "- Repeated-hit disabled\n"
                "- Trust reset\n"
...
def main():
    ...
    app.mainloop()
    log_event("INFO", "Codex Control Console v13 closed.")
