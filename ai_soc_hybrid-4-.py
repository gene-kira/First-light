...
class AssetIncident:
    ...
    safe: bool = False      # explicitly safe (games, trusted apps)
...
# Game / launcher / engine safe lists
SAFE_GAME_PROCESSES = [
...
# Sensitive / personal / biometric directories (high-level hints)
SENSITIVE_DIR_HINTS = [
...
def scan_system(stats: Dict) -> (List[AssetIncident], Dict, float):
    ...
    # Processes (AI / rogue / suspicious / games / sensitive)
    for proc in psutil.process_iter(['pid', 'name']):
        ...
        if game:
            base_risk = min(base_risk, 40)  # games are low risk by default
        if sensitive_touch and not game:
            base_risk = max(base_risk, 85)  # unknown touching sensitive paths is high risk
...
def risk_to_color(risk: int, safe: bool) -> QColor:
    if safe:
        return QColor(0, 120, 215)  # blue for safe/game
    if risk < 20:
...
def criticality_to_border(crit: str, isolated: bool, safe: bool) -> QPen:
    if safe:
        return QPen(QColor(0, 120, 215), 3)  # safe/game highlight
    if isolated:
...
class SocGuardianWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("AI System Guardian v3 – Hybrid SOC")
    ...
    def run_guardian_actions(self):
        # If any safe/game process is running, auto-soften behavior
