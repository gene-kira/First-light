...
# ============================================================
# GPU GOVERNOR v8.5
# Multi-GPU, AI/HC/Gaming/Connector-Safe profiles, FLOP governor,
...
# NEW v8.5:
# - Hard safety caps (never exceed 500W)
# - Connector-risk scoring (power + temp + predicted load)
# - DLSS-aware power throttling (heuristic)
# - "Connector Safe Mode" profile
# - Per-second power spike smoothing
# - PSU telemetry integration (if supported, stubbed)
# ============================================================
...
def log_telemetry_csv(row: Dict[str, Any]):
    header = [
        ...
        "psu_12v_voltage", "psu_12v_current"
    ]
    with CSV_LOCK:
...
# Hard caps
MAX_SAFE_POWER_W = 500  # global hard cap
...
class FanCurveLearner:
    ...
    def fit_regression(self):
        if len(self.history) < 10:
            return
        temps = [t for t, _ in self.history]
        fans = [f for _, f in self.history]
        mean_t = sum(temps) / len(temps)
        mean_f = sum(fans) / len(fans)
        num = sum((t - mean_t) * (f - mean_f) for t, f in self.history)
        den = sum((t - mean_t) ** 2 for t in temps)
        if den <= 1e-6:
            return
        slope = num / den
        intercept = mean_f - slope * mean_t
        new_points = []
        for base_t in [30, 50, 65, 80, 90]:
            fan = int(max(0, min(100, slope * base_t + intercept)))
            new_points.append(FanCurvePoint(base_t, fan))
        self.gov.gui_set_fan_curve_points(new_points)
        self.gov._save_persistent_settings()
        print(f"[FAN-LEARN] Updated fan curve via regression: slope={slope:.3f} intercept={intercept:.3f}")

class UndervoltLearner:
    def __init__(self, governor: "GpuGovernor"):
        self.gov = governor
    ...
    def suggest_offset(self) -> int:
        if len(self.samples) < 20:
            return self.gov.pending_voltage_offset
        cool_samples = [s for s in self.samples if s["temp"] < self.gov.cfg.temp_limit_c - 5 and s["power"] > 0]
        if not cool_samples:
            return self.gov.pending_voltage_offset
        avg_offset = sum(s["offset_mv"] for s in cool_samples) / len(cool_samples)
        target = int(max(-100, min(0, avg_offset - 5)))
        print(f"[UV-LEARN] Suggested undervolt offset={target} mV from avg={avg_offset:.1f} mV")
        return target

# PSU telemetry stub (extend with real integration if available)
class PsuTelemetry:
    def __init__(self, gpu_index: int):
        self.gpu_index = gpu_index
    ...
    def read(self) -> Tuple[float, float]:
        # Stub: in real integration, query PSU via vendor SDK or PMBus
        # Here we approximate from GPU power (rough heuristic)
        telem = get_gpu_telemetry(self.gpu_index)
        power = telem["power"]
        voltage = 12.0
...
class GpuGovernor:
    def __init__(self, gpu_index: int = 0):
        ...
        self.fan_learner = FanCurveLearner(self)
        self.uv_learner = UndervoltLearner(self)
        self.psu = PsuTelemetry(self.gpu_index)
    ...
    def _load_persistent_settings(self):
        ...
        try:
            ...
            if mode not in ("AI", "HC", "Gaming", "ConnectorSafe"):
                mode = "AI"
    ...
    def set_profile_mode(self, mode: str):
        ...
        self.pending_power = min(base.power_limit_w, MAX_SAFE_POWER_W)
    ...
    def apply_ai_safe(self):
        self.cfg.power_limit_w = min(int(self.pending_power), MAX_SAFE_POWER_W)
    ...
    def _dlss_aware_throttle(self, telem: Dict[str, Any]):
        # Heuristic: high GPU util + high mem util + sudden power spike => treat as DLSS-like load
        util = telem["gpu_util"]
    ...
    def _compute_connector_risk(self, telem: Dict[str, Any], predicted_temp: float) -> float:
        # Risk score 0–100 based on power, temp, predicted temp, PSU current
        power = telem["power"]
        temp = telem["temp"]
        ...
        current_ratio = min(1.0, current / 50.0)  # assume 50A as high
    ...
    def _smooth_power_spikes(self, telem: Dict[str, Any]):
        ...
        if len(self.last_power_samples) >= 3:
            ...
            if power > avg + 50:
                new_limit = max(300, min(self.pending_power - 30, MAX_SAFE_POWER_W))
    ...
    def _ai_auto_tune(self, telem: Dict[str, Any]):
        ...
        if predicted_temp > self.cfg.temp_limit_c + 3 or risk > 80.0:
            new_power = max(300, self.pending_power - 15)
            print(f"[AI-TUNE] Predicted temp {predicted_temp:.1f}C or risk {risk:.1f} high, reducing power to {new_power}W")
            self.pending_power = new_power
        elif util < 40 and power < self.pending_power * 0.7 and risk < 50.0 and self.profile_mode != "ConnectorSafe":
            ...
            self.pending_power = new_power
        suggested_uv = self.uv_learner.suggest_offset()
    ...
    def _safe_tune_loop(self):
        ...
        print(
            ...
            f"power_limit={self.cfg.power_limit_w} W (hard cap {MAX_SAFE_POWER_W}W) "
        ...
        while self.running:
            ...
            self.fan_learner.record(temp, fan_speed)
            self.uv_learner.record(temp, power, self.pending_voltage_offset)
            ...
            if temp > self.cfg.temp_limit_c:
                ...
                self.cfg.power_limit_w = min(new_limit, MAX_SAFE_POWER_W)
                set_power_limit_w(self.gpu_index, self.cfg.power_limit_w)
                self._save_persistent_settings()
            if self.pending_fan_curve_enabled:
                fan_target = self.fan_curve.compute(temp)
                set_fan_speed_percent(self.gpu_index, fan_target)
            if len(self.fan_learner.history) % 200 == 0:
                self.fan_learner.fit_regression()
            for plugin in PLUGINS:
                try:
...
class GovernorGUI(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.setWindowTitle("GPU Governor v8.5 — Multi-GPU AI/HC/Gaming/ConnectorSafe")
    ...
    def _build_telemetry_tab(self):
        ...
        for w in (self.gauge_temp, self.gauge_power, self.gauge_util,
                  self.gauge_flops, self.gauge_fan, self.gauge_vram, self.gauge_risk):
        ...
        layout.addWidget(QtWidgets.QLabel(f"Pending Power limit (W, hard cap {MAX_SAFE_POWER_W}W)"))
        ...
        self.chk_fan_curve = QtWidgets.QCheckBox("Enable automatic fan curve (safe ramp + learning)")
    ...
    def _build_fan_tab(self):
        ...
        panel = CollapsiblePanel("Custom Fan Curve Editor (with learning)", fan_editor)
    ...
    def _build_graph_tab(self):
        ...
        layout = QtWidgets.QVBoxLayout(tab)
        if HAS_QTCHARTS:
            charts_group = QtWidgets.QGroupBox("Dashboard — Temp / Power / Util / VRAM / Fragmentation / Risk")
    ...
    def _on_export(self):
        ...
        any_gov.export_curve(str(BASE_DIR / "curve_v8_5.json"))
