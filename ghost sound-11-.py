...
def reset_system_state():
    """
    Reset Security Bridge internal state after Lockdown,
    while keeping all lockdown evidence files.
    """
    log_event("RESET", "Reset Mode triggered (state normalization, evidence preserved).")

    # Clear volatile / incident-focused state
    volatile_keys = [
        "audio_spikes",
        "last_audio_levels",
        "event_stream",
    ...
    # Reset detection counters and trust scores to baseline
    STATE["detections"] = {}
    STATE["trust_scores"] = {}

    # No change to RULES, allowlist, blocklist, or evidence files
...
class SecurityBridgeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Security Bridge - v10 (Lockdown + Reset Mode + Adaptive Trust + Audio Correlation + MITRE Views)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            top_frame,
            text="Security Bridge v10 - Operator Mode + LOCKDOWN + RESET",
        ...
        # RESET BUTTON (state normalization)
        ttk.Button(
            top_frame,
    ...
    def reset_system_gui(self):
        """
        GUI handler for Reset Mode (state normalization, evidence preserved).
        """
        answer = messagebox.askyesno(
...
def main():
    log_event("INFO", "Security Bridge (Full Upgrade v10) started.")
    start_honeypot()
    root = tk.Tk()
