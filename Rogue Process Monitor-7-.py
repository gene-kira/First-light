...
# ============================================================
# Rogue Process Monitor v13.5 (SAFE, NON-DESTRUCTIVE)
# Apex Predator Edition – Borg Tech
#
# Concepts:
#   - Borg Queen: central orchestration brain
#   - Borg Workers: many scanning agents
#
# Features:
#   - Full sandbox isolation
#   - Borg Queen coordinating worker threads
#   - Worker pool scanning subsets of processes
...
LOG_FILE = "rogue_monitor_v13_5_log.txt"
...
def sandbox_append_history(event):
    ...
    with sandbox_lock:
        ...
        if len(sandbox_history_events) > 2000:
...
def prune_scan_cache(max_age=120.0):
    now = time.time()
    with scan_cache_lock:
...
class BorgWorker(threading.Thread):
    ...
    def run(self):
        ...
        self.queen.worker_report(local_results)
...
class BorgQueen:
    ...
    def worker_report(self, worker_results):
        with self.results_lock:
...
class SandboxScanner:
    ...
    def scan_loop(self):
        log("SandboxScanner v13.5 Borg scan loop started")
        while self.running:
            try:
    ...
    def tree_loop(self):
        log("SandboxScanner v13.5 tree loop started")
        while self.running:
            try:
                self.build_tree()
            except Exception as e:
    ...
    def borg_scan_cycle(self):
        ...
        if len(procs) < num_workers:
            num_workers = max(1, len(procs))

        chunk_size = max(1, len(procs) // max(1, num_workers))
        workers = []
        for i in range(num_workers):
            chunk = procs[i * chunk_size:(i + 1) * chunk_size]
            if not chunk:
                continue
            w = BorgWorker(self.queen, chunk)
            w.start()
            workers.append(w)
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v13.5 (SAFE, Borg Queen Sandbox)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v13.5 (SAFE, Borg Queen)", font=("Segoe UI", 14, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))

        desc = ttk.Label(
            frame,
            text=(
                "Borg Queen orchestrates multiple worker agents in a sandbox.\n"
                "Workers scan subsets of processes, Queen aggregates threat and history.\n"
...
def main():
    log("Rogue Process Monitor v13.5 (SAFE, Borg Queen Sandbox) starting")
    root = tk.Tk()
