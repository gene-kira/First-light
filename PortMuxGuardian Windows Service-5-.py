...
BASE_DIR = os.path.join(os.path.expanduser("~"), "CodexSecurityBridgeV4Hybrid")
DB_PATH = os.path.join(BASE_DIR, "codex_v4_hybrid.db")
SURICATA_EVE = os.path.join(BASE_DIR, "eve.json")  # point to real eve.json if you have Suricata
PEERS = ["http://127.0.0.1:6001/peer"]  # simple multi-node sync stub

AGGRESSIVE_ENABLED = True  # hybrid: aggressive only when high-risk + high score
...
class Persistence:
    ...
    def _init_db(self) -> None:
        ...
        conn.commit()
        conn.close()
        log("[DB] Initialized codex_v4_hybrid.db")
...
class ThreatScorer:
    ...
    def score(self, kind: str, payload: str) -> float:
        text = payload.lower()
        base = 0.0
        bad_words = ["crack", "warez", "keygen", "malware", "trojan", "ransom", "exploit", "phishing"]
        for w in bad_words:
            if w in text:
                base += 3.0
        ...
        if "download" in text or "http" in text:
            base += 1.0
        if kind in ("ai_text", "suricata", "process"):
            base *= 1.2
...
class PeerSync:
    ...
    def _loop(self) -> None:
        while self.running:
            ...
            try:
                for p in PEERS:
                    self.requests.post(p, json={"ping": "codex_v4_hybrid"})
            except Exception as e:
...
class Supervisor:
    ...
    def start(self) -> None:
        if self.running:
            return
        self.running = True
        log("[SUP] Starting Codex Security Bridge v4 Hybrid")
        self.ai.start()
        self.bridge.start()
        self.suricata.start()
        self.procmon.start()
    ...
    def handle_highrisk(self, kind: str, payload: str) -> None:
        ...
        if score >= 3.0:
            ...
            if AGGRESSIVE_ENABLED and mode == "highrisk_lockdown" and score >= 5.0:
                log("[SANDBOX] aggressive path would trigger here (stub)")
                action = "aggressive_stub"
...
def main() -> None:
    sup = Supervisor()
    sup.start()
    log("[MAIN] Codex Security Bridge v4 Hybrid running. Ctrl+C to exit.")
    try:
        while True:
            time.sleep(5.0)
