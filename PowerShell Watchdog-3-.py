...
FAST_SCAN_INTERVAL_MS = 250

def is_powershell_process(name: str) -> bool:
    """Recognize Windows PowerShell and PowerShell 7+."""
    n = (name or "").lower()
    return n in {"powershell.exe", "pwsh.exe"} or n.startswith("powershell")


def fast_kill_process(proc: psutil.Process, reason=""):
    """
    Immediately kill a PowerShell process and its children.
    Uses kill() rather than terminate() to avoid waiting for a graceful exit.
    """
    pid = proc.pid
    name = "unknown"
    try:
    ...
    # Kill children first so a child cannot outlive the PowerShell parent.
    ...
    try:
        ...
        log_event("ERROR", "FAST kill failed", {
            "pid": pid,
            "name": name,
            "reason": reason,
            "error": str(e),
    ...
    def update_data(self):
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'ppid']):
            try:
                ...
                if not is_powershell_process(name):
                    continue
                pid = proc.info['pid']
...
class PowerShellWatchdog(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PowerShell Watchdog — FAST KILL")
        ...
        self.status.showMessage(f"Ready — FAST KILL / {FAST_SCAN_INTERVAL_MS} ms scan")

        self.timer = QtCore.QTimer(self)
        self.timer.setInterval(FAST_SCAN_INTERVAL_MS)
    ...
    def kill_selected(self):
        ...
        if fast_kill_process(proc, reason="manual kill"):
            self.status.showMessage(f"FAST-killed PID {pid} ({name})")
            self.toast(f"FAST KILL: {pid} ({name})")
        else:
            self.status.showMessage(f"Failed to kill PID {pid} ({name})")
        self.refresh()
        ...
        for idx, info in enumerate(self.model.proc_infos):
            ...
            if sev >= 9.0 and suri_hits:
                ...
                if name in self.WHITELIST:
                    continue
                now = time.monotonic()
                if now - self._recent_kills.get(pid, 0.0) < 2.0:
                    continue

                if fast_kill_process(
                    proc,
                    reason=f"auto-block severity={sev:.1f}, rules={[r['id'] for r in suri_hits]}"
                ):
                    self._recent_kills[pid] = now
                    
                    self.toast(f"AUTO-BLOCK FAST KILL: {pid} ({name}) sev={sev:.1f}")
                else:
                    continue
        self.model.update_data()
