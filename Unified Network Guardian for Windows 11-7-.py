...
#     * Isolation Forest (with safe training/prediction)
#     * LSTM/autoencoder stubs (hook points)
    ...
    def launch(self):
        ...
        devices_box.setPlainText("\n".join(
            [f"{ip} - {info.get('role')} ({info.get('type')}) risk={info.get('risk')} "
             f"os={info.get('os_hint')} ttl={info.get('ttl_hint')} mac={info.get('mac_vendor')}"
