...
"""
DominionDeck_CommandCenter_FullSystem_NextTier.py
...
- Real swarm networking:
    ...
    - Encrypted WAN packets (AES-256 in CTR mode, shared key in swarm_key.json)
    - Swarm command propagation (JSON commands between peers)
- Drive health monitoring:
    - Heuristic (psutil usage)
    - True SMART-like via WMI (Windows only, basic attributes)
...
SWARM_KEY_PATH = DATA_DIR / "swarm_key.json"       # shared encryption key
        ...
        try:
            for disk in psutil.disk_partitions(all=False):
                ...
                for dev_id, info in smart.items():
                    if dev_id.endswith(dev.replace("\\", "")) or dev in dev_id:
                        smart_entry = info
                        break
...
def load_swarm_key():
    """
    swarm_key.json format:
    {
      "key_hex": "00112233..."
    }
    If missing, generate a new random key.
    """
    ...
    if not key_hex:
        ...
        log("Generated new swarm AES key.")
...
# ---------------------------------------------------------------------------
# REAL SWARM NETWORKING (LAN + WAN + encrypted + commands)
# ---------------------------------------------------------------------------
...
class SwarmNetwork(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                msg = None
                # Try encrypted first
                msg = decrypt_payload(self.key, data)
                if msg is None:
                    # Fallback: plain JSON
                    try:
    ...
    def broadcast_status(self):
        ...
        msg_enc = encrypt_payload(self.key, payload)
        # LAN broadcast (plain for local simplicity)
        try:
    ...
    def send_command(self, command, args=None):
        ...
        for host, port in self.wan_peers:
            try:
                ...
                log(f"SwarmNetwork command send error to {host}:{port} -> {e}")
...
def launch_gui():
    ...
    for col, txt in [("id", "Peer ID"), ("addr", "Address"), ("load", "Load")]:
        ...
        tree_swarm_net.column(col, width=220, anchor="center")
    ...
    def send_swarm_command_gui():
        ...
        if not content:
            return
        # simple format: COMMAND arg1=value1 arg2=value2
        parts = content.split()
        cmd = parts[0]
        args = {}
        for p in parts[1:]:
