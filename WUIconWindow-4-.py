#!/usr/bin/env python3
# codex_honeypot_gui_v8_s1_ai_deception.py
...
def ai_deception_strategy(event: Dict[str, Any]) -> List[str]:
    """
    Decide which deception assets to generate based on an event.
    Event can be Suricata, process, or honeypot event.
    Returns a list of asset kinds to generate.
    """
    kinds: List[str] = []
            ...
            try:
                for p in psutil.process_iter(attrs=["pid", "name", "cmdline"]):
                    ...
                    if pid not in known_pids:
                        ...
                        # AI-driven deception trigger for process events
...
def start_gui():
    ...
    root = tk.Tk()
    root.title("Codex Honeypot v8 (S1 + AI Deception)")
