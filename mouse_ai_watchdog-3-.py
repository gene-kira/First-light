...
# Track USB selective suspend state ourselves (since powercfg /q is unreliable on your build)
...
def get_usb_selective_suspend():
    """
    Returns last known USB selective suspend state.
    We track it ourselves because powercfg /q is invalid on this build.
    """
    return USB_SUSPEND_STATE

def set_usb_selective_suspend(disable=True):
    """
    Tries to set USB selective suspend via powercfg.
    Even if querying fails, setting usually works.
    """
