...
# -*- coding: utf-8 -*-
"""
CODEX CONTROL CONSOLE v7.9 (Windows Edition)
AI-driven, Suricata-aware, MITRE-mapped, swarm-enabled SOC brain
with real DPI, real honeypot stubs, improved GUI, upgraded ML, better swarm,
richer MITRE mapping, structured training, refined response logic,
Borg Swarm Tech (Queens + Workers),
and NEW Predictive & Missing-Detail Intelligence:

New v7.9 upgrades:
- Predictive Intelligence:
  - Bernoulli-style event likelihood modeling
  - Temporal probability forecasts for future alert states
  - Best-guess severity projection
- Missing-Detail Detection:
  - Feature completeness scoring
  - Missing-field detector for alerts/packets/MITRE entries
  - “Gap pressure” metric to highlight incomplete telemetry
- Intelligent Water Data Physics Engine:
  - Fluid-inspired smoothing of anomaly/drift over time
  - “Flow” and “pressure” metrics for data streams
  - Shock-absorber behavior for noisy spikes
- Altered States Engine:
  - System “states of consciousness” (CALM, FOCUSED, ALERT, OVERDRIVE)
  - State transitions driven by anomaly/drift/AE error/probabilities
  - State-aware response hints

Everything from v7.8 retained:
- Borg Swarm (Queens + Workers)
- Real DPI (Scapy/DPKT/PyShark fallback)
- Real honeypot stubs (HTTP/SSH)
- Compact GUI with severity coloring and timeline
- Better ML (AE/Transformer + isolation-style scoring)
- Better swarm (token, trust decay)
- Better MITRE mapping and campaign correlation
- Training mode with baseline thresholds
- Suricata auto-config generator
- Auto-creation of all required files/directories
"""
...
SYMPY_AVAILABLE = AUTO.has("sympy")
...
def is_windows_admin() -> bool:
    ...
    try:
        ...
        print(f"[ELEVATION] IsUserAnAdmin failed: {e}")
        return False

def relaunch_as_admin_if_needed():
    if os.name != "nt":
        print("[ELEVATION] Non-Windows OS, skipping elevation check")
        return False

    if is_windows_admin():
        print("[ELEVATION] Already running with administrative privileges")
        return False

    print("[ELEVATION] Not elevated. Attempting auto-relaunch with admin rights...")
    try:
        ...
        print("[ELEVATION] Relaunch requested. Exiting current process.")
        return True
    except Exception as e:
        print(f"[ELEVATION] Auto-relaunch failed: {e}")
        return False

ELEVATED = is_windows_admin()
if not ELEVATED:
    # Try relaunch; if it succeeds, current process will exit soon.
    relaunched = relaunch_as_admin_if_needed()
    if not relaunched:
        print("[ELEVATION] Running in LIMITED mode (no admin). Some actions may fail.")
...
def initialize_codex_files():
    ...
    if not pcap_file.exists():
        try:
            # Create an empty file. PacketParser safely treats an empty capture as no data.
            pcap_file.write_bytes(b"")
            print(f"[INIT] Created empty live.pcap")
        except Exception as e:
    ...
    if not cfg_path.exists():
        try:
            cfg_path.write_text(json.dumps({
                ...
                "swarm_token": "codex-swarm-token",
                "borg_role": "queen",  # "queen" or "worker"
                "borg_tasks": ["SCAN", "WATCH", "DEFEND", "LURE"],
                "train_samples": 500,
                "train_epochs": 5
...
print(f"[CONFIG] Swarm token = {CONFIG.get('swarm_token')}")
print(f"[CONFIG] Borg role = {CONFIG.get('borg_role')}")
print(f"[CONFIG] Borg tasks = {CONFIG.get('borg_tasks')}")
...
class TransformerEncoder(nn.Module if TORCH and nn is not None else object):
    def __init__(self, dim=256, nhead=4, num_layers=3):
        if not TORCH or nn is None:
            return
        super().__init__()
...
class AnomalyModel:
    ...
    def isolation_style_score(self, vec: np.ndarray) -> float:
        if self.mean is None or self.var is None:
            return 0.0
        vec = vec.astype(np.float32)
        diff = vec - self.mean
        var_mag = np.linalg.norm(self.var) + 1e-8
        return float(np.linalg.norm(diff) / var_mag)
...
class MathReasoningSuite:
    def __init__(self):
        self.enabled = True

    def area_under_curve_constant(self, value: float, a: float, b: float) -> float:
        return value * (b - a)

    def triangle_pythagoras(self, a: float, b: float) -> float:
        return (a**2 + b**2) ** 0.5

    def shape_anomaly_with_integral(self, anomaly: float) -> float:
        area = self.area_under_curve_constant(anomaly, 1.0, 2.0)
        return area * 1.1

    def shape_drift_with_geometry(self, drift: float) -> float:
        hyp = self.triangle_pythagoras(drift, drift)
        return hyp

# ===========================================================
# PREDICTIVE ENGINE (BERNOULLI + BEST GUESS)
# ===========================================================

class PredictiveEngine:
    """
    Simple Bernoulli-style predictive engine:
    - Tracks probability of ALERT vs NO_ALERT over time
    - Uses anomaly/drift/AE error to adjust likelihood
    - Produces best-guess future severity and alert probability
    """
    def __init__(self):
        self.alert_count = 0
        self.total_count = 0
        self.last_prob_alert = 0.5

    def update(self, decision: str, anomaly: float, drift: float, ae_err: float):
        self.total_count += 1
        if decision == "ALERT":
            self.alert_count += 1
        base_p = self.alert_count / max(self.total_count, 1)
        # Bernoulli-style adjustment: anomaly/drift/AE push probability up
        boost = (anomaly + drift + ae_err) / (1.0 + anomaly + drift + ae_err)
        self.last_prob_alert = min(1.0, max(0.0, base_p * 0.7 + boost * 0.3))
        return self.last_prob_alert

    def best_guess_severity(self, max_sev: int) -> float:
        # Predict next severity as a smoothed function of current severity and alert probability
        return float(max_sev) * (0.5 + 0.5 * self.last_prob_alert)

# ===========================================================
# MISSING DETAIL DETECTOR
# ===========================================================

class MissingDetailDetector:
    """
    Looks for missing fields in alerts, packets, and MITRE entries.
    Produces a "gap pressure" metric indicating how incomplete the telemetry is.
    """
    def __init__(self):
        pass

    def _score_alert(self, alert: Dict[str, Any]) -> float:
        required = ["alert", "timestamp", "src_ip", "dest_ip", "proto"]
        missing = 0
        for r in required:
            if r not in alert:
                missing += 1
    ...
    def _score_packet(self, pkt: Dict[str, Any]) -> float:
        required = ["len", "src", "dst", "proto"]
        missing = 0
        for r in required:
            if r not in pkt:
                missing += 1
    ...
    def _score_mitre(self, entry: "MITREEntry") -> float:
        missing = 0
        if not entry.technique_id:
            missing += 1
        if not entry.technique_name:
            missing += 1
        if not entry.tactic:
            missing += 1
        return missing / 3.0

    def compute_gap_pressure(self, alerts: List[Dict[str, Any]], packets: List[Dict[str, Any]], mitre_entries: List["MITREEntry"]) -> float:
        scores = []
        for a in alerts:
            scores.append(self._score_alert(a))
        for p in packets:
            scores.append(self._score_packet(p))
        for e in mitre_entries:
            scores.append(self._score_mitre(e))
        if not scores:
            return 0.0
...
class WaterPhysicsEngine:
    """
    Fluid-inspired smoothing engine:
    - Treats anomaly/drift as "pressure" and "flow"
    - Smooths spikes using exponential decay
    - Produces smoothed anomaly/drift and a "shock" metric
    """
    def __init__(self, alpha: float = 0.7):
        self.alpha = alpha
        self.smoothed_anomaly = 0.0
        self.smoothed_drift = 0.0

    def update(self, anomaly: float, drift: float) -> Dict[str, float]:
        self.smoothed_anomaly = self.alpha * self.smoothed_anomaly + (1 - self.alpha) * anomaly
        self.smoothed_drift = self.alpha * self.smoothed_drift + (1 - self.alpha) * drift
        shock = abs(anomaly - self.smoothed_anomaly) + abs(drift - self.smoothed_drift)
        return {
            "smoothed_anomaly": self.smoothed_anomaly,
...
class AlteredStatesEngine:
    ...
    - CALM: low anomaly/drift, low alert probability
    - FOCUSED: moderate anomaly/drift, moderate probability
    - ALERT: high anomaly/drift or high probability
    - OVERDRIVE: extreme anomaly/drift or shock
    """
    def __init__(self):
        self.state = "CALM"

    def update(self, anomaly: float, drift: float, ae_err: float, prob_alert: float, shock: float) -> str:
        score = anomaly + drift + ae_err + prob_alert * 2.0 + shock
        if score < 1.0:
            self.state = "CALM"
        elif score < 3.0:
            self.state = "FOCUSED"
...
class BorgSwarm:
    def __init__(self, role: str = "queen", tasks: Optional[List[str]] = None):
        self.role = role.lower()  # "queen" or "worker"
        self.tasks: Dict[str, BorgTask] = {}
        default_tasks = tasks or ["SCAN", "WATCH", "DEFEND", "LURE"]
        for t in default_tasks:
            self.tasks[t] = BorgTask(name=t, weight=1.0)
        self.assigned_task: Optional[str] = None

    def assign_task(self, task_name: str):
        if task_name in self.tasks:
            self.assigned_task = task_name
            print(f"[BORG] Role={self.role.upper()} assigned task={task_name}")
        else:
            print(f"[BORG] Unknown task={task_name}, ignoring")

    def rebalance_tasks(self):
        total = len(self.tasks) or 1
        for i, (name, task) in enumerate(self.tasks.items()):
            task.weight = 1.0 + (i / total)
        print("[BORG] Rebalanced task weights:", {k: v.weight for k, v in self.tasks.items()})

    def borg_header(self) -> str:
        return f"BORG_ROLE:{self.role.upper()} TASK:{self.assigned_task or 'NONE'}"
...
class SwarmNode:
    def __init__(self, node_id: str, bind_port: int = 50050, broadcast_port: int = 50050, token: str = "codex-swarm-token", borg: Optional[BorgSwarm] = None):
        self.node_id = node_id
        self.bind_port = bind_port
        self.broadcast_port = broadcast_port
        self.token = token
        self.borg = borg or BorgSwarm(role="worker")
    ...
    def broadcast(self, msg: str):
        try:
            borg_header = self.borg.borg_header()
            payload = f"TOKEN:{self.token} NODE:{self.node_id} {borg_header} {msg}"
    ...
    def _listen_loop(self):
        while self.running:
            try:
                ...
                if f"TOKEN:{self.token}" in decoded:
                    self.last_messages.append(decoded)
            except Exception:
                time.sleep(0.1)
    def start(self):
        self.running = True
...
class ConsensusEngine:
    def decide(self, local_node_id: str, local_decision: str, swarm: SwarmNode) -> str:
        ...
        queen_votes: Dict[str, float] = {}
        worker_votes: Dict[str, float] = {}
        ...
        for m in swarm.last_messages:
            try:
                ...
                borg_role_part = [p for p in parts if p.startswith("BORG_ROLE:")][0]
                borg_role = borg_role_part.split("BORG_ROLE:")[1]
                ...
                if borg_role == "QUEEN":
                    queen_votes[dec] = queen_votes.get(dec, 0.0) + weight * 1.5
                else:
                    worker_votes[dec] = worker_votes.get(dec, 0.0) + weight
            except Exception:
                continue

        for dec, w in queen_votes.items():
            votes[dec] = votes.get(dec, 0.0) + w
        for dec, w in worker_votes.items():
            votes[dec] = votes.get(dec, 0.0) + w
...
class PacketParser:
    ...
    def parse_packets(self, max_packets: int = 128) -> List[Dict[str, Any]]:
        ...
        if not self.pcap_path:
            return packets
        try:
            pcap_file = pathlib.Path(self.pcap_path)
            ...
            if SCAPY_AVAILABLE and scapy is not None:
                for pkt in scapy.PcapReader(self.pcap_path):
                    ...
                    if pkt.haslayer("Raw"):
                        ...
                        info["payload_sample"] = payload[:32].hex()
                    packets.append(info)
                ...
                with open(self.pcap_path, "rb") as f:
                    ...
                    for ts, buf in pcap:
                        info = {"len": len(buf), "ts": ts}
                        try:
                            eth = dpkt.ethernet.Ethernet(buf)
                            if isinstance(eth.data, dpkt.ip.IP):
                                ip = eth.data
                                info["src"] = socket.inet_ntoa(ip.src)
                                info["dst"] = socket.inet_ntoa(ip.dst)
                                info["proto"] = ip.p
                        except Exception:
                            pass
                        packets.append(info)
                ...
                for pkt in cap:
                    info = {"len": len(pkt), "layers": len(pkt.layers)}
                    packets.append(info)
                    if len(packets) >= max_packets:
                        break
            else:
...
def build_input_from_real_data(backbone: BackboneAdapter, mitre: MITREMapper) -> Tuple[List[float], List[Dict[str, Any]], List[Dict[str, Any]], List[MITREEntry]]:
    ...
    for p in packets:
        ...
        if "src" in p and "dst" in p:
            stream.append(float(hash(p["src"]) % 1024))
            stream.append(float(hash(p["dst"]) % 1024))
    if not stream:
        stream = [0.0] * 64
...
class AIBrain:
    def __init__(self, base_dir: pathlib.Path):
        ...
        self.math = MathReasoningSuite()
        self.base_dir = base_dir
        self.deep.load_weights(base_dir)
        self.ml.load_baseline(base_dir)

        # New predictive and resilience engines
        self.predictive = PredictiveEngine()
        self.missing_detector = MissingDetailDetector()
        self.water_physics = WaterPhysicsEngine(alpha=0.7)
        self.altered_states = AlteredStatesEngine()
    ...
    def infer(self, data, target=None, alerts=None, packets=None, mitre_entries=None):
        ...
        isolation_score = self.ml.isolation_style_score(global_vec)
        self.memory.add(global_vec)
        drift_score = self.memory.drift_score(global_vec)

        anomaly_shaped = self.math.shape_anomaly_with_integral(anomaly_score)
        drift_shaped = self.math.shape_drift_with_geometry(drift_score)
        ...
        result["anomaly_score"] = anomaly_shaped
        result["drift_score"] = drift_shaped
        result["ae_error"] = ae_err
        result["seq_enc"] = seq_enc
        result["isolation_score"] = isolation_score

        # Missing detail detection
        gap_pressure = self.missing_detector.compute_gap_pressure(alerts or [], packets or [], mitre_entries or [])
        result["gap_pressure"] = gap_pressure

        # Water physics smoothing
        water_stats = self.water_physics.update(anomaly_shaped, drift_shaped)
        result["water_smoothed_anomaly"] = water_stats["smoothed_anomaly"]
        result["water_smoothed_drift"] = water_stats["smoothed_drift"]
        result["water_shock"] = water_stats["shock"]

        # Predictive engine update
        prob_alert = self.predictive.update(result["decision"], anomaly_shaped, drift_shaped, ae_err)
        result["prob_alert"] = prob_alert
        result["predicted_severity"] = self.predictive.best_guess_severity(
            max_sev=max((e.severity for e in (mitre_entries or [])), default=0)
        )

        # Altered states engine
        state = self.altered_states.update(
            anomaly_shaped,
            drift_shaped,
            ae_err,
            prob_alert,
    ...
    def run_forever(self):
        ...
        while True:
            try:
                ...
                print(f"[WATCHDOG:{self.name}] Exception: {e}")
                traceback.print_exc()
                print(f"[WATCHDOG:{self.name}] Restarting in {self.restart_delay}s")
...
class HoneypotServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 8080, name: str = "HTTP-Honeypot"):
        self.host = host
        self.port = port
        self.name = name
        self.running = False
    ...
    def _serve(self):
        ...
        try:
            ...
            s.listen(5)
            print(f"[HONEYPOT:{self.name}] Listening on {self.host}:{self.port}")
            while self.running:
                try:
                    conn, addr = s.accept()
                    print(f"[HONEYPOT:{self.name}] Connection from {addr}")
                    try:
                        data = conn.recv(1024)
                        print(f"[HONEYPOT:{self.name}] Payload: {data[:64]!r}")
            ...
            print(f"[HONEYPOT:{self.name}] Error: {e}")
        finally:
            s.close()
...
class ResponseEngine:
    def __init__(self, os_type: str = "windows"):
        ...
        self.http_honeypot = HoneypotServer(port=8080, name="HTTP-Honeypot")
        self.ssh_honeypot = HoneypotServer(port=2222, name="SSH-Honeypot")
    ...
    def start_honeypots(self):
        ...
        print("[HONEYPOT] HTTP and SSH honeypots started")
    ...
    def apply(self, decision: str, mitre_entries: List[MITREEntry], anomaly: float, drift: float, ae_err: float, isolation_score: float, altered_state: str, prob_alert: float, gap_pressure: float):
        max_sev = max((e.severity for e in mitre_entries), default=0)
        if decision != "ALERT":
            print("[RESPONSE] Decision not ALERT, no action")
            return

        print(f"[RESPONSE] Decision=ALERT Sev={max_sev} Anom={anomaly:.3f} Drift={drift:.3f} AE={ae_err:.3f} Iso={isolation_score:.3f} State={altered_state} ProbAlert={prob_alert:.3f} Gap={gap_pressure:.3f}")

        if max_sev < SEVERITY_THRESHOLD and altered_state in ("CALM", "FOCUSED"):
            print(f"[RESPONSE] Severity {max_sev} < threshold {SEVERITY_THRESHOLD} and state={altered_state}, logging only")
            return

        if max_sev >= 5 or altered_state == "OVERDRIVE":
            self.firewall_block("0.0.0.0/0")
            self.honeypot_redirect("0.0.0.0/0")
            self.start_honeypots()
        elif max_sev == 4 or altered_state == "ALERT":
            self.deception_route("0.0.0.0/0")
            self.start_honeypots()
        elif max_sev == 3:
            print("[RESPONSE] Moderate severity — honeypots only")
            self.start_honeypots()
        else:
            print("[RESPONSE] Low severity — logging only")
...
# ===========================================================
# COMPACT SOC GUI (Improved Layout)
# ===========================================================

class CodexGUI:
    def __init__(self):
        self.root = None
        if TK_AVAILABLE and tk is not None:
            try:
                self.root = tk.Tk()
            except Exception as e:
                print(f"[GUI] Tk initialization failed: {e}")
                self.root = None
        self.timeline_points: List[Tuple[float, float, int]] = []
        if not self.root or not TTK_AVAILABLE or ttk is None:
            return

        self.root.title("Codex Control Console v7.9 (Windows)")
        self.root.geometry("1200x720")

        self.frame_main = tk.Frame(self.root)
        self.frame_main.pack(fill=tk.BOTH, expand=True)

        self.frame_left = tk.Frame(self.frame_main, width=260)
        self.frame_left.pack(side=tk.LEFT, fill=tk.Y)
        self.frame_center = tk.Frame(self.frame_main)
        self.frame_center.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.frame_right = tk.Frame(self.frame_main, width=260)
        ...
        self.label_pred_sev = tk.Label(self.frame_left, text="Pred Sev: N/A", font=("Consolas", 10))
        self.label_pred_sev.pack(anchor="w")
        ...
        self.label_iso = tk.Label(self.frame_left, text="Iso Score: N/A", font=("Consolas", 10))
        self.label_iso.pack(anchor="w")
        self.label_prob = tk.Label(self.frame_left, text="ProbAlert: N/A", font=("Consolas", 10))
        self.label_prob.pack(anchor="w")
        self.label_gap = tk.Label(self.frame_left, text="GapPressure: N/A", font=("Consolas", 10))
        self.label_gap.pack(anchor="w")
        self.label_state = tk.Label(self.frame_left, text="State: N/A", font=("Consolas", 10))
        ...
        self.label_swarm_title = tk.Label(self.frame_right, text="SWARM / BORG", font=("Consolas", 12, "bold"))
    ...
    def update_main(self, decision: str, consensus: str, anomaly: float, drift: float, ae_err: float, iso_score: float, gpu: Dict[str, float], max_sev: int, pred_sev: float, prob_alert: float, gap_pressure: float, state: str):
        if not self.root:
            return
        ...
        self.label_pred_sev.config(text=f"Pred Sev: {pred_sev:.2f}")
        ...
        self.label_iso.config(text=f"Iso Score: {iso_score:.3f}")
        self.label_prob.config(text=f"ProbAlert: {prob_alert:.3f}")
        self.label_gap.config(text=f"GapPressure: {gap_pressure:.3f}")
...
class CodexControlConsole:
    ...
    def push_event(self, payload: Dict[str, Any]):
        ...
        print("[CODEX] Decision:", payload.get("decision"),
              ...
              "Iso:", payload.get("iso_score"),
              "GPU:", payload.get("gpu"),
              "MaxSev:", payload.get("max_sev"),
              "PredSev:", payload.get("predicted_severity"),
              "ProbAlert:", payload.get("prob_alert"),
              "GapPressure:", payload.get("gap_pressure"),
              "State:", payload.get("altered_state"),
              "BorgRole:", payload.get("borg_role"),
              "BorgTask:", payload.get("borg_task"))
        if self.gui and self.gui.root:
            try:
                self.gui.root.after(0, lambda: self._update_gui(payload))
            except Exception as e:
                print(f"[GUI] Update scheduling failed: {e}")

    def _update_gui(self, payload: Dict[str, Any]):
        if not self.gui or not self.gui.root:
            return
        try:
            self.gui.update_main(
                ...
                payload.get("iso_score", 0.0),
                payload.get("gpu", {"gpu_util": 0, "gpu_mem": 0, "gpu_temp": 0}),
                payload.get("max_sev", 0),
                payload.get("predicted_severity", 0.0),
                payload.get("prob_alert", 0.0),
                payload.get("gap_pressure", 0.0),
                payload.get("altered_state", "CALM"),
            )
...
def daemon_cycle(node_id: str,
    ...
    result = brain.infer(data_stream, target, alerts=alerts, packets=packets, mitre_entries=mitre_entries)
    gpu_stats = gpu_telemetry.sample()

    # Borg task assignment logic
    if swarm.borg.role == "queen":
        swarm.borg.rebalance_tasks()
        chosen_task = random.choice(list(swarm.borg.tasks.keys()))
        swarm.borg.assign_task(chosen_task)
    else:
        if not swarm.borg.assigned_task:
            chosen_task = random.choice(list(swarm.borg.tasks.keys()))
            swarm.borg.assign_task(chosen_task)

    borg_header = swarm.borg.borg_header()
    msg = f"{borg_header} DECISION:{result['decision']} TOP:{result.get('top')} SCORE:{result.get('score')}"
    swarm.broadcast(msg)
    swarm.decay_trust()
    ...
    responder.apply(
        ...
        result.get("ae_error", 0.0),
        result.get("isolation_score", 0.0),
        result.get("altered_state", "CALM"),
        result.get("prob_alert", 0.0),
        result.get("gap_pressure", 0.0),
    )
    ...
    payload = {
        ...
        "iso_score": result.get("isolation_score"),
        "gpu": gpu_stats,
        ...
        "max_sev": max_sev,
        "campaigns": campaigns,
        "borg_role": swarm.borg.role,
        "borg_task": swarm.borg.assigned_task,
        "predicted_severity": result.get("predicted_severity", 0.0),
        "prob_alert": result.get("prob_alert", 0.0),
        "gap_pressure": result.get("gap_pressure", 0.0),
        "altered_state": result.get("altered_state", "CALM"),
    }
...
def run_codex_daemon():
    ...
    token = cfg.get("swarm_token", "codex-swarm-token")
    borg_role = cfg.get("borg_role", "queen")
    borg_tasks = cfg.get("borg_tasks", ["SCAN", "WATCH", "DEFEND", "LURE"])

    borg_swarm = BorgSwarm(role=borg_role, tasks=borg_tasks)
    ...
    brain = AIBrain(base_dir)
    gpu_telemetry = GPUTelemetry()
    swarm = SwarmNode(node_id=node_id, bind_port=bind_port, broadcast_port=broadcast_port, token=token, borg=borg_swarm)
    swarm.start()
    mitre = MITREMapper(db_path=str(base_dir / "mitre.json"))
    consensus = ConsensusEngine()
    ...
    print("[CORE] Codex Control Console daemon v7.9 (Windows: TRAIN + eve.json + PCAP + MITRE + COMPACT GUI + SWARM + BORG + RESPONSE + THRESHOLD + SURICATA_CFG + CAMPAIGNS + MATH_SUITE + DPI + HONEYPOT + PREDICTIVE + MISSING + WATER_PHYSICS + ALTERED_STATES)")
    def loop():
        while True:
            daemon_cycle(node_id, console, brain, backbone, gpu_telemetry, swarm, mitre, consensus, responder, campaign_engine)
            time.sleep(5)
    wd = Watchdog("CodexDaemonV7_9_Windows", loop, restart_delay=3.0)
    if gui and gui.root:
