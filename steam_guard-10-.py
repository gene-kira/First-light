#!/usr/bin/env python3
"""
Steam Malware Guard v9 – Queue-Based Prompt + Ultra-Tamper-Resistant Agent

Key changes vs v8:
- Fix crash when choosing whitelist/blacklist/kill/ignore:
  - Worker thread no longer touches Qt at all
  - kill_suspicious_processes pushes prompts into PROCESS_PROMPT_QUEUE
  - GuardGUI owns a QTimer that drains the queue in the GUI thread
  - Dialogs and decisions are handled entirely in the main thread

All previous features preserved:
- Auto-elevation, unified event bus
- FS watcher, entropy/string/name/YARA/VT stub
- GPU ML skeleton (3D-ResNet + Steam anomaly)
- Process→network correlation
- Network anomaly detection
- Kernel EventLog loop
- Steam backup/rollback/diff viewer
- Suricata eve.json + live socket stub
- Fluent Ribbon GUI + threat/tamper matrix
- Backbone telemetry
- Tamper-resistance (integrity, anti-debug, watchdog)
- Persistent config (auto_kill, auto_fs_watch, whitelist/blacklist)
"""
...
SURICATA_EVE_PATH = os.path.join(os.getcwd(), "eve.json")
SURICATA_SOCKET_HOST = "127.0.0.1"
SURICATA_SOCKET_PORT = 5555
...
AUTH_SECRET = b"super_secret_shared_key"
...
CONFIG_PATH = os.path.join(os.getcwd(), "guard_config.json")

DEFAULT_CONFIG = {
    "auto_kill": True,
    "auto_fs_watch": True,
    "whitelist_procs": WHITELIST_PROCS,
    "blacklist_tokens": BLACKLIST_TOKENS,
}

# Queue for process prompts (worker → GUI)
PROCESS_PROMPT_QUEUE = queue.Queue()
...
def load_config():
    global AUTO_KILL, AUTO_FS_WATCH, WHITELIST_PROCS, BLACKLIST_TOKENS
    ...
    try:
        with open(CONFIG_PATH, "r", encoding="utf-8") as f:
            cfg = json.load(f)
        AUTO_KILL = bool(cfg.get("auto_kill", True))
        AUTO_FS_WATCH = bool(cfg.get("auto_fs_watch", True))
        WHITELIST_PROCS = cfg.get("whitelist_procs", WHITELIST_PROCS)
        BLACKLIST_TOKENS = cfg.get("blacklist_tokens", BLACKLIST_TOKENS)
    except Exception:
        AUTO_KILL = DEFAULT_CONFIG["auto_kill"]
        AUTO_FS_WATCH = DEFAULT_CONFIG["auto_fs_watch"]
        WHITELIST_PROCS = DEFAULT_CONFIG["whitelist_procs"]
        BLACKLIST_TOKENS = DEFAULT_CONFIG["blacklist_tokens"]
...
# TAMPER
# ==========================
def compute_main_hash():
    return sha256_file(MAIN_SCRIPT_PATH)

def init_integrity(gui, bus):
    global MAIN_SCRIPT_HASH
    MAIN_SCRIPT_HASH = compute_main_hash()
    gui.log(f"[INTEGRITY] Main script hash={MAIN_SCRIPT_HASH}")
    bus.emit("tamper_event", {"type": "init_hash", "hash": MAIN_SCRIPT_HASH})

def check_integrity(gui, bus):
    current = compute_main_hash()
    if MAIN_SCRIPT_HASH and current != MAIN_SCRIPT_HASH:
        gui.log(f"[TAMPER] Main script hash changed! {MAIN_SCRIPT_HASH} -> {current}")
        bus.emit("tamper_event", {"type": "hash_mismatch", "old": MAIN_SCRIPT_HASH, "new": current})
        add_behavior_score(gui, bus, 10, "Main script tampered")

def anti_debug_check(gui, bus):
    try:
        ...
        ctypes.windll.kernel32.CheckRemoteDebuggerPresent(hProcess, ctypes.byref(is_remote))
        if is_debugger or is_remote.value:
            gui.log("[TAMPER] Debugger detected.")
            bus.emit("tamper_event", {"type": "debugger", "local": bool(is_debugger), "remote": bool(is_remote.value)})
            add_behavior_score(gui, bus, 5, "Debugger detected")
    except Exception as e:
        gui.log(f"[TAMPER ERROR] Anti-debug check failed: {e}")
        bus.emit("tamper_event", {"type": "debug_error", "msg": str(e)})

def tamper_monitor_loop(gui, bus):
    init_integrity(gui, bus)
    while True:
        check_integrity(gui, bus)
        anti_debug_check(gui, bus)
        time.sleep(15)

def spawn_watchdog(guard_pid):
    args = [sys.executable, MAIN_SCRIPT_PATH, "--watchdog", str(guard_pid)]
    subprocess.Popen(args, creationflags=0)

def watchdog_main():
    if len(sys.argv) < 3:
        print("[WATCHDOG] Missing guard PID.")
        return
    guard_pid = int(sys.argv[2])
    print(f"[WATCHDOG] Monitoring guard PID={guard_pid}")
    while True:
        try:
            psutil.Process(guard_pid)
        except psutil.NoSuchProcess:
            print("[WATCHDOG] Guard process missing, restarting...")
...
class GuardGUI(QtWidgets.QMainWindow):
    def __init__(self, bus):
        super().__init__()
        self.bus = bus
        self.setWindowTitle("Steam Malware Guard v9 (Queue-Based Prompt)")
        ...
        self.btn_toggle_fs = QtWidgets.QPushButton(f"FS Watch: {'ON' if AUTO_FS_WATCH else 'OFF'}")
        self.btn_toggle_kill = QtWidgets.QPushButton(f"Auto-Kill: {'ON' if AUTO_KILL else 'OFF'}")
        self.btn_show_diff = QtWidgets.QPushButton("Show Steam Diff")
        self.score_label = QtWidgets.QLabel(f"Behavior Score: {BEHAVIOR_SCORE}")
        ...
        right_layout.addWidget(QtWidgets.QLabel("Threat / Tamper Matrix"))
        ...
        # Timer to drain process prompt queue in GUI thread
        self.prompt_timer = QtCore.QTimer(self)
        self.prompt_timer.setInterval(500)  # 0.5s
        self.prompt_timer.timeout.connect(self._drain_process_prompts)
    ...
    def _drain_process_prompts(self):
        global PROCESS_PROMPT_QUEUE, GLOBAL_BUS
        try:
            while True:
                payload = PROCESS_PROMPT_QUEUE.get_nowait()
                pid = payload.get("pid")
                name = payload.get("name")
                exe = payload.get("exe")
                cmd = payload.get("cmd")
                decision = self.ask_process_action(pid, name, exe, cmd)

                if decision == "kill":
                    try:
                        p = psutil.Process(pid)
                        p.kill()
                        self.log(f"[AUTO-KILL] PID={pid} NAME={name}")
                        GLOBAL_BUS.emit("process_killed", {"pid": pid, "name": name})
                        add_behavior_score(self, GLOBAL_BUS, 2, "Unknown process killed")
                    except Exception as e:
                        self.log(f"[AUTO-KILL ERROR] {e}")
                elif decision == "whitelist":
                    WHITELIST_PROCS.append(name)
                    self.log(f"[WHITELIST] Added {name}")
                    save_config()
                elif decision == "blacklist":
                    BLACKLIST_TOKENS.append(name)
                    self.log(f"[BLACKLIST] Added {name}")
                    save_config()
                elif decision == "ignore":
                    self.log(f"[IGNORE] Process {name} (PID={pid}) ignored.")
        except queue.Empty:
            pass
    ...
    def ask_process_action(self, pid, name, exe, cmdline):
        dlg = QtWidgets.QDialog(self)
        dlg.setWindowTitle("Suspicious Process Detected")
        layout = QtWidgets.QVBoxLayout(dlg)
        ...
        btn_whitelist = QtWidgets.QPushButton("Whitelist")
        btn_blacklist = QtWidgets.QPushButton("Blacklist")

        btn_layout.addWidget(btn_kill)
        btn_layout.addWidget(btn_ignore)
        btn_layout.addWidget(btn_whitelist)
        btn_layout.addWidget(btn_blacklist)
        ...
        btn_kill.clicked.connect(lambda: set_choice("kill"))
        btn_ignore.clicked.connect(lambda: set_choice("ignore"))
        btn_whitelist.clicked.connect(lambda: set_choice("whitelist"))
        btn_blacklist.clicked.connect(lambda: set_choice("blacklist"))
...
def tamper_loop(gui, bus):
    tamper_monitor_loop(gui, bus)
...
def start_threads(gui, bus, ml_manager):
    ...
    threading.Thread(target=tamper_loop, args=(gui, bus), daemon=True).start()
...
def guard_main():
    ...
    gui.log("[INIT] Steam Malware Guard v9 started.")
    start_threads(gui, bus, GLOBAL_ML)
    spawn_watchdog(psutil.Process().pid)
    gui.show()
    sys.exit(app.exec())
