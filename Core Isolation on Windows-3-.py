...
def attempt_disable_device(device_id: str):
    """
    Heuristic remediation: use PowerShell to disable device via PnpDevice cmdlets.
    Requires Windows 10+ and admin.
    """
    try:
        cmd = [
            "powershell",
...
# ---------- HVCI Watchdog Thread ----------

class HVCIWatchdog(threading.Thread):
    ...
    def run(self):
        while not self._stop.is_set():
            try:
                ...
                current = (reg_state, hvci_enabled)
                if self._last_state is None:
                    self._last_state = current
                else:
                    if current != self._last_state:
                        self._last_state = current
                        self.callback(reg_state, hvci_enabled)
            except Exception:
                pass
...
class SecurityGovernorGUI:
    def __init__(self, root):
        ...
        self.label_warning.pack(padx=10, pady=5, fill="x")
        ...
        self.notebook.add(self.frame_drivers, text="Drivers")
        ...
        self.notebook.add(self.frame_tpm, text="TPM")
        ...
        self.notebook.add(self.frame_bitlocker, text="BitLocker")

        self.hvci_current = False

        # HVCI Watchdog
        self.watchdog = HVCIWatchdog(self.on_hvci_change, interval=5.0)
        self.watchdog.start()
    ...
    def on_hvci_change(self, reg_state, hvci_enabled):
        # Called from watchdog thread; marshal to main thread via after
        def notify():
            state_str = f"Registry Enabled={reg_state}, DeviceGuard HVCI={'ON' if hvci_enabled else 'OFF'}"
            messagebox.showwarning(
                "HVCI Change Detected",
                f"Memory Integrity state changed:\n{state_str}"
            )
            self.refresh_state()
    ...
    def remediate_drivers(self):
        text = self.text_drivers.get("1.0", tk.END).strip()
        if not text or "Potential non-Microsoft/OEM drivers" not in text:
            messagebox.showinfo("Info", "No driver list available to remediate.")
            return
        ...
        # Ask user if they want to attempt disabling all listed devices
