#!/usr/bin/env python3
# ============================================================
# TargetingComputer_DominionDeck_BorgProbes.py
...
class Threat:
    def __init__(self, name, level, port=None):
        ...
        self.assimilated = False  # Borg tech flag
...
class ThreatEngine:
    ...
    def update_all(self):
        if not self.threats:
            return
        self.score_cb(self.threats, self.borg_mode)
        ...
        while True:
            ...
            if self.ai_enabled and self.threats:
                ...
                if active:
                    ...
                    self.log(f"[AI] Auto-targeting: {target.name} (Predicted {target.predicted_risk:.2f})")
...
# ------------------------------------------------------------
# VRAM-Accelerated Threat Scoring (GPU-backed if available)
# Borg tech: collective assimilation scoring
# ------------------------------------------------------------
def vram_accelerated_scoring(threats, borg_mode=False):
    ...
    for t, s in zip(threats, scores):
        t.score = float(s + collective_boost)
...
class TargetingGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Targeting Computer — DominionDeck Borg Probes")
    ...
    def update_heatmap(self):
        if not self.heatmap_fig or not self.engine.threats or not np:
            return
