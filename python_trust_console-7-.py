#!/usr/bin/env python3
"""
Python Trust Console v6 (Compact Dark, Advanced)
...
- Sandbox trust mode (restrict to safe directories conceptually)
...
# ==========================
BACKUP_PATH = os.path.join(os.getcwd(), "trust_backup.json")
DIFF_PATH = os.path.join(os.getcwd(), "trust_diff.json")
...
PROFILE_STORE_PATH = os.path.join(os.getcwd(), "trust_profiles.json")
...
def register_py_association(python_exe, gui_log=None, sandbox=False):
    ...
    if sandbox:
        cmd = f'"{python_exe}" "%1" %*'
        # Conceptual sandbox: user must keep scripts in SAFE_SANDBOX_DIRS
    else:
        cmd = f'"{python_exe}" "%1" %*'
    ok2, err2 = set_reg(
        winreg.HKEY_CLASSES_ROOT,
...
def apply_trust_profile(profile_name, python_exe_list, profiles_cfg, gui_log=None):
    ...
    if gui_log:
        if ok_all:
            gui_log(f"[TRUST] Profile '{profile_name}' applied to {len(python_exe_list)} interpreter(s).")
        else:
...
class IntegrityMonitor(threading.Thread):
    def __init__(self, gui_log=None, interval=3600, auto_repair=False, python_exe_list=None):
        ...
        self.interval = interval  # seconds (1 hour default)
        ...
        while self.running:
            try:
                ...
                if self.last_status is not None and status != self.last_status:
                    msg = "[INTEGRITY] Trust status changed."
                    log_warn(msg)
                    if self.gui_log:
                        self.gui_log(msg)
                    if self.auto_repair:
...
# ==========================
# COMPACT PySide6 GUI (Dark, Advanced)
# ==========================
class TrustConsoleWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Trust Console v6 (Compact Dark)")
        self.resize(950, 600)
    ...
    def _init_ui(self):
        ...
        # Top row: profile + python selector
        ...
        self.dev_diag_view.setPlaceholderText("Developer Diagnostics (paths, profiles, sandbox dirs, diff file)")
    ...
    def refresh_profiles_view(self):
        # Show current profile config in diagnostics
    ...
    def on_start_monitor(self):
        if self.integrity_monitor and self.integrity_monitor.running:
            ...
            return
        py_list = self.get_selected_python_list()
        self.gui_log("[MON] Starting integrity monitor (hourly, auto-repair ON)...")
