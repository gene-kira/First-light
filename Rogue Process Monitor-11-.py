...
# ============================================================
# Rogue Process Monitor v13.9 (SAFE, NON-DESTRUCTIVE)
# Anomaly Borg + ML Anomaly + Matplotlib Threat / Timeline Plots
#
# New:
#   - Optional IsolationForest-based anomaly scoring per PID
#   - Feature vector per process (CPU, RSS, score, history stats)
#   - Matplotlib plots:
#       * Threat level over time (bucketed)
#       * Bucketed activity (count vs avg score)
#   - ML gracefully disabled if sklearn not present
#   - Plots gracefully disabled if matplotlib not present
#
# Still SAFE:
#   - No killing, blocking, or destructive actions
# ============================================================

LOG_FILE = "rogue_monitor_v13_9_log.txt"
...
def compute_ml_anomaly_scores(cache_snapshot):
    ...
    if len(X) < 10:
        # Not enough data to train IsolationForest meaningfully
        return {}
    try:
        ...
        # Convert to anomaly score 0-100 (lower decision_function -> higher anomaly)
        min_s = min(scores)
        max_s = max(scores)
        if max_s == min_s:
            return {}
...
class BorgWorker(threading.Thread):
    ...
    def score_proc(self, proc):
        try:
            ...
            # ML anomaly will be added later by queen; here we set placeholder
            ml_anomaly = 0.0
...
class BorgQueen:
    ...
    def finalize_cycle(self):
        ...
        # Inject ML anomaly into live_items
    ...
    def build_timeline(history, threat_level, bucket_seconds=300):
        ...
        for bk in sorted(buckets.keys()):
            ...
            result.append({
                ...
                "count": b["count"],
                "avg_score": avg_score,
                "avg_threat": threat_level,  # simple overlay
            })
...
class SandboxScanner:
    ...
    def scan_loop(self):
        log("SandboxScanner v13.9 Borg swarm scan loop started")
        while self.running:
            try:
    ...
    def tree_loop(self):
        log("SandboxScanner v13.9 tree loop started")
        while self.running:
            try:
                self.build_tree()
            except Exception as e:
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v13.9 (SAFE, Borg + ML + Plots)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v13.9 (SAFE, Borg + ML + Plots)", font=("Segoe UI", 14, "bold"))
        ...
        desc = ttk.Label(
            ...
            text=(
                ...
                "Optional IsolationForest ML adds another anomaly dimension.\n"
...
def main():
    log("Rogue Process Monitor v13.9 (SAFE, Borg + ML + Plots) starting")
    root = tk.Tk()
