...
#     * Multi-node logic (peer list + consensus voting stub)
...
class SwarmSync:
    ...
    def _consensus_vote(self, local: set, remote: set) -> set:
        """
        Simple consensus: union of both sets.
        Stub for more advanced voting.
        """
    ...
    def _handle_packet(self, pkt):
        try:
            if IP in pkt:
                ...
                malicious = self.threat_intel.is_malicious(src)
                if malicious or anomalous_ml or anomalous_nn:
                    sev = 80 if malicious else 60
                    event_type = "intrusion" if malicious else "per_ip_anomaly"
                    event = {
                        "type": event_type,
                        "src_ip": src,
                        "dst_ip": dst,
                        "summary": f"Suspicious IP {src} targeting {dst} (malicious={malicious}, ml={anomalous_ml}, nn={anomalous_nn})",
                        "severity": sev
                    }
                    self.threat_matrix.add_event(event)
...
class RemoteAPIHandler(BaseHTTPRequestHandler):
    ...
    """
    guardian_ctx = None  # set externally
