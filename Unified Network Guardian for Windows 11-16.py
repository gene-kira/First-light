...
# - Distributed mesh routing (alert broadcast)
# - Zero-trust identity enforcement:
#     * Identity fingerprint (IP/MAC/OS/role)
#     * Trust score
#     * Policy bound to identity, not just IP
# - MITRE ATT&CK mapping for events
# - Agentic reasoning threat triage + autonomous remediation
...
# - GUI dashboard with:
#   * Threats pane at top (with audio/visual controls)
...
# - NDIS / kernel-level telemetry suppression (safe stubs)
# - TLS visibility stubs (no full interception, but metadata extraction)
...
DEFAULT_CONFIG = {
    ...
    "settings_persist_interval": 30,
    "agentic_remediation_enabled": True,
    "tls_metadata_enabled": True,
    "kernel_layer_enabled": False
}
...
class GuardianConfig:
    ...
    def _apply(self):
        c = self.config
        self.gui_enabled = c.get("gui_enabled", True)
        self.audio_alerts = c.get("audio_alerts", True)
        self.visual_alerts = c.get("visual_alerts", True)
        self.scan_min_seconds = c.get("scan_min_seconds", 5)
        self.scan_max_seconds = c.get("scan_max_seconds", 60)
        self.self_repair_mode = c.get("self_repair_mode", "logged")
        self.ciarmy_enabled = c.get("ciarmy_enabled", True)
        self.et_enabled = c.get("et_enabled", True)
        self.watchdog_enabled = c.get("watchdog_enabled", True)
        self.firewall_block_enabled = c.get("firewall_block_enabled", True)
        self.firewall_rule_ttl_seconds = c.get("firewall_rule_ttl_seconds", 3600)
        self.manual_override_enabled = c.get("manual_override_enabled", False)
        self.rogue_ai_detection_enabled = c.get("rogue_ai_detection_enabled", True)
        self.swarm_sync_enabled = c.get("swarm_sync_enabled", False)
        self.swarm_endpoint = c.get("swarm_endpoint", "")
        self.swarm_encryption_key = c.get("swarm_encryption_key", "")
        self.swarm_shared_secret = c.get("swarm_shared_secret", "guardian_shared_secret")
        self.honeypot_enabled = c.get("honeypot_enabled", True)
        self.honeypot_ssh_port = c.get("honeypot_ssh_port", 2222)
        self.honeypot_http_port = c.get("honeypot_http_port", 8081)
        self.honeypot_rdp_port = c.get("honeypot_rdp_port", 3390)
        self.honeypot_ftp_port = c.get("honeypot_ftp_port", 2121)
        self.honeypot_telnet_port = c.get("honeypot_telnet_port", 2323)
        self.packet_inspection_enabled = c.get("packet_inspection_enabled", True)
        self.abuseipdb_api_key = c.get("abuseipdb_api_key", "")
        self.abuseipdb_base_url = c.get("abuseipdb_base_url", DEFAULT_CONFIG["abuseipdb_base_url"])
        self.otx_api_key = c.get("otx_api_key", "")
        self.otx_base_url = c.get("otx_base_url", DEFAULT_CONFIG["otx_base_url"])
        self.spamhaus_enabled = c.get("spamhaus_enabled", True)
        self.spamhaus_drop_url = c.get("spamhaus_drop_url", DEFAULT_CONFIG["spamhaus_drop_url"])
        self.swarm_peers = c.get("swarm_peers", [])
        self.remote_api_enabled = c.get("remote_api_enabled", True)
        self.remote_api_port = c.get("remote_api_port", 8089)
        self.remote_api_bind = c.get("remote_api_bind", "127.0.0.1")
        self.suricata_rules_path = c.get("suricata_rules_path", "suricata.rules")
        self.mesh_enabled = c.get("mesh_enabled", True)
        self.mesh_node_id = c.get("mesh_node_id", "guardian-node-1")
        self.settings_persist_interval = c.get("settings_persist_interval", 30)
        self.agentic_remediation_enabled = c.get("agentic_remediation_enabled", True)
        self.tls_metadata_enabled = c.get("tls_metadata_enabled", True)
        self.kernel_layer_enabled = c.get("kernel_layer_enabled", False)

    def save(self):
        # Persist current runtime settings back to YAML
        self.config["audio_alerts"] = self.audio_alerts
        self.config["visual_alerts"] = self.visual_alerts
        self.config["manual_override_enabled"] = self.manual_override_enabled
        self.config["gui_enabled"] = self.gui_enabled
        self.config["agentic_remediation_enabled"] = self.agentic_remediation_enabled
        self.config["tls_metadata_enabled"] = self.tls_metadata_enabled
        self.config["kernel_layer_enabled"] = self.kernel_layer_enabled
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
...
class GuardianDB:
    ...
    def _init_schema(self):
        with db_lock:
            ...
            add_column("events", "mitre_tech", "TEXT")
            add_column("events", "triage", "TEXT")
            add_column("ip_history", "mitre_tech", "TEXT")
            add_column("ip_history", "triage", "TEXT")
    ...
    def add_event(self, event: dict):
        with db_lock:
            ...
            ts = int(time.time())
            cur.execute("""
                INSERT INTO events (ts, type, src_ip, dst_ip, summary, severity, mitre_tech, triage)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ts,
                event.get("type", ""),
            ...
            if ip:
                cur.execute("""
                    INSERT INTO ip_history (ip, ts, event_type, summary, severity, mitre_tech, triage)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (ip, ts, event.get("type", ""), event.get("summary", ""), event.get("severity", 0),
                      event.get("mitre_tech", ""), event.get("triage", "")))
                self.conn.commit()
    ...
    def get_recent_events(self, limit=100):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("""
                SELECT ts, type, src_ip, dst_ip, summary, severity, mitre_tech, triage
                FROM events
                ORDER BY ts DESC
                LIMIT ?
            """, (limit,))
            rows = cur.fetchall()
        events = []
        for ts, t, src, dst, summary, sev, mitre, triage in rows:
            events.append({
                "ts": ts,
                "type": t,
                "src_ip": src,
    ...
    def get_ip_history(self, ip: str, limit: int = 100):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("""
                SELECT ts, event_type, summary, severity, mitre_tech, triage
                FROM ip_history
                WHERE ip = ?
                ORDER BY ts DESC
                LIMIT ?
            ...
            rows = cur.fetchall()
        history = []
        for ts, etype, summary, sev, mitre, triage in rows:
            history.append({
                "ts": ts,
                "event_type": etype,
                "summary": summary,
...
class SuricataRuleEngine:
    """
    Extended Suricata v6-style parser (safe subset).
    Supports:
    - alert/pass/drop actions
    - proto, src/dst, ports
    - options: msg, sid, rev, classtype, content, pcre, flow, tls.sni (as content match)
    """
    def __init__(self, config: GuardianConfig):
        self.config = config
        self.rules = []
        ...
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    if not line or line.startswith("#"):
                        continue
                    if line.startswith(("alert ", "drop ", "pass ")):
    ...
    def match_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                     proto: str, payload: bytes, tls_sni: str | None = None):
        matches = []
        for rule in self.rules:
            ...
            if content:
                try:
                    ...
                    continue
            tls_content = opts.get("tls.sni")
            if tls_content and tls_sni:
                if tls_content not in tls_sni:
                    continue
            matches.append(rule)
        return matches
...
class MITREMapper:
    """
    Simple MITRE ATT&CK mapping based on event type and context.
    """
    def __init__(self):
        self.map = {
            "intrusion": "T1046",          # Network Service Scanning
            "honeypot_hit_ssh": "T1110",   # Brute Force
            "honeypot_hit_http": "T1190",  # Exploit Public-Facing Application
            "honeypot_hit_rdp": "T1133",   # External Remote Services
            "honeypot_hit_ftp": "T1048",   # Exfiltration Over Alternative Protocol
            "honeypot_hit_telnet": "T1046",
            "rogue_ai_suspected": "T1071", # Application Layer Protocol
            "per_ip_anomaly": "T1082",     # System Information Discovery
            "replay_intrusion": "T1200",   # Hardware Additions (simulated)
            "replay_anomaly": "T1082",
            "anomaly_rate": "T1029",       # Scheduled Transfer
            "deep_anomaly": "T1036"        # Masquerading (behavioral)
        }

    def map_event(self, event_type: str) -> str:
        return self.map.get(event_type, "")

class AgenticTriageEngine:
    """
    Agentic reasoning for threat triage + autonomous remediation.
    """
    def __init__(self, config: GuardianConfig, db: GuardianDB, firewall, mesh_router):
        self.config = config
        self.db = db
        self.firewall = firewall
        self.mesh_router = mesh_router

    def classify(self, severity: int, malicious: bool, suricata_hits: int,
                 honeypot: bool, ml_anomaly: bool) -> str:
        if malicious or suricata_hits > 0 or honeypot:
            if severity >= 80:
                return "critical"
            elif severity >= 60:
                return "high"
                ...
                return "medium"
        if ml_anomaly and severity >= 60:
            return "high"
        if severity >= 40:
            return "medium"
        return "low"

    def remediate(self, event: dict):
        if not self.config.agentic_remediation_enabled:
            return
        triage = event.get("triage", "low")
        ip = event.get("src_ip", "")
        if not ip:
            return

        if triage == "critical":
            logger.info(f"[Agentic] Critical event for {ip}, enforcing BLOCK + mesh alert.")
            self.db.set_ip_policy(ip, POLICY_BLOCK, block_ttl=3600, allow_ttl=None, monitor_ttl=None)
            self.firewall.block_ip(ip, manual=True)
            self.mesh_router.broadcast_alert(event)
        elif triage == "high":
            logger.info(f"[Agentic] High event for {ip}, monitor + temporary block.")
            self.db.set_ip_policy(ip, POLICY_BLOCK, block_ttl=900, allow_ttl=None, monitor_ttl=None)
            self.firewall.block_ip(ip, manual=True)
        elif triage == "medium":
            logger.info(f"[Agentic] Medium event for {ip}, monitor policy.")
            self.db.set_ip_policy(ip, POLICY_MONITOR, block_ttl=None, allow_ttl=None, monitor_ttl=1800)
        else:
            logger.debug(f"[Agentic] Low event for {ip}, no auto remediation.")
...
class ThreatMatrix:
    def __init__(self, db: GuardianDB, mitre: MITREMapper, triage_engine: AgenticTriageEngine):
        self.db = db
        self.mitre = mitre
    ...
    def add_event(self, event: dict):
        ...
        mitre_tech = self.mitre.map_event(etype)
        event["mitre_tech"] = mitre_tech
        self.db.add_event(event)
        logger.info(f"[ThreatMatrix] Event: {event.get('summary', 'unknown')} (MITRE={mitre_tech}, triage={event.get('triage','')})")
        self.triage_engine.remediate(event)
...
class DeepMLAnomalyDetector:
    def __init__(self, base: AnomalyDetector):
        ...
        if SKLEARN_AVAILABLE:
            ...
            self.mlp = MLPClassifier(hidden_layer_sizes=(32, 16), max_iter=200)

    def record_event(self, event: dict):
        rate = self.base.get_rate()
        sev = event.get("severity", 0)
        self.samples.append([rate, sev])
        ...
        if self.iforest is not None and len(self.samples) > 50:
            try:
                ...
                logger.error(f"[DeepML] IF training failed: {e}")

        if self.mlp is not None and len(self.samples) > 100:
            try:
                labels = [1 if s[1] >= 60 else 0 for s in self.samples]
                self.mlp.fit(self.samples, labels)
            except Exception as e:
                logger.error(f"[DeepML] MLP training failed: {e}")
    ...
    def is_deep_anomaly(self) -> bool:
        ...
        if self.iforest is not None and hasattr(self.iforest, "estimators_"):
            try:
                pred = self.iforest.predict([features])[0]
                if pred == -1:
                    return True
            except Exception as e:
                logger.error(f"[DeepML] IF prediction failed: {e}")

        if self.mlp is not None and hasattr(self.mlp, "coefs_"):
            try:
                pred = self.mlp.predict([features])[0]
                return pred == 1
            except Exception as e:
                logger.error(f"[DeepML] MLP prediction failed: {e}")
...
class SignatureEngine:
    def __init__(self, db: GuardianDB):
        ...
        self.signatures = [
            ...
            ("ftp_probe", "honeypot_hit_ftp"),
            ("telnet_probe", "honeypot_hit_telnet")
        ]
...
class KernelTelemetryLayer:
    """
    Stub for NDIS / kernel-level telemetry suppression.
    No real driver loading here; just a placeholder for future integration.
    """
    def __init__(self, config: GuardianConfig):
        self.config = config
        self.enabled = config.kernel_layer_enabled

    def suppress_noise(self, pkt_meta: dict) -> bool:
        if not self.enabled:
            return False
        # Example: drop low-risk broadcast noise
        if pkt_meta.get("dst_ip", "").startswith("224.") or pkt_meta.get("dst_ip", "").startswith("255."):
            return True
        return False
...
class AlertEngine:
    def __init__(self, config: GuardianConfig):
        ...
        self.first_alert_only = False
        self.severity_increase_only = True

    @staticmethod
    def simple_alert(message: str):
        print(f"[ALERT] {message}")

    def _should_emit(self, ip: str, event_type: str, severity: int) -> bool:
        now = time.time()

        # Burst suppression: ignore if too many alerts in last second
        self.burst_window.append(now)
        self.burst_window = [t for t in self.burst_window if now - t < 1.0]
        if len(self.burst_window) > self.burst_threshold:
            logger.debug("[Alert] Burst suppression: too many alerts, skipping.")
            return False

        key = (ip, event_type)
        last_ts = self.last_alert_ts.get(key, 0)
        last_sev = self.last_alerts.get(key, 0)

        if self.first_alert_only and key in self.last_alerts:
            logger.debug(f"[Alert] First-alert-only: already alerted for {key}, skipping.")
            return False

        if now - last_ts < self.cooldown_seconds:
            logger.debug(f"[Alert] Cooldown active for {key}, skipping.")
            return False

        if self.severity_increase_only and severity <= last_sev:
            logger.debug(f"[Alert] Severity not increased for {key}, skipping.")
            return False

        self.last_alert_ts[key] = now
        self.last_alerts[key] = severity
        return True
    ...
    def intruder_alert(self, src_ip: str, dst_ip: str, severity: int, event_type: str):
        if not self._should_emit(src_ip, event_type, severity):
            return
...
def extract_tls_sni(payload: bytes) -> str | None:
    """
    Very rough TLS ClientHello SNI extraction stub.
    Not a full TLS parser, just metadata.
    """
    try:
        data = payload
        if len(data) < 5:
        ...
        if data[0] != 0x16:  # Handshake
            return None
        # This is intentionally minimal; real TLS parsing is complex.
        return None
    except Exception:
        return None
...
class PacketInspector(threading.Thread):
    def __init__(self, config: GuardianConfig, threat_intel: ThreatIntel,
                 ...
                 kernel_layer: KernelTelemetryLayer, triage_engine: AgenticTriageEngine):
    ...
    def _handle_packet(self, pkt):
        try:
            if IP in pkt:
                ...
                meta = {"src_ip": src, "dst_ip": dst, "proto": proto, "src_port": src_port, "dst_port": dst_port}
                if self.kernel_layer.suppress_noise(meta):
                    return

                tls_sni = extract_tls_sni(payload) if self.config.tls_metadata_enabled else None
                ...
                suricata_matches = self.suricata_engine.match_packet(src, dst, src_port, dst_port, proto, payload, tls_sni)
                ...
                if malicious or anomalous_ml or anomalous_nn or suricata_matches:
                    ...
                    triage = self.triage_engine.classify(sev, malicious, len(suricata_matches), False, anomalous_ml or anomalous_nn)
                    summary = f"Suspicious IP {src} targeting {dst} (malicious={malicious}, ml={anomalous_ml}, nn={anomalous_nn}, rules={len(suricata_matches)}, triage={triage})"
                    event = {
                        "type": event_type,
                        "src_ip": src,
                        "dst_ip": dst,
                    ...
                    self.threat_matrix.add_event(event)
                    self.alerts.intruder_alert(src, dst, sev, event_type)
    ...
    def replay_packet(self, src_ip: str, dst_ip: str, payload: bytes, meta: dict):
        try:
            ...
            tls_sni = None
            if self.config.tls_metadata_enabled:
            ...
            suricata_matches = self.suricata_engine.match_packet(src_ip, dst_ip, src_port, dst_port, proto, payload, tls_sni)
            ...
            if malicious or anomalous_ml or anomalous_nn or suricata_matches:
                ...
                triage = self.triage_engine.classify(sev, malicious, len(suricata_matches), False, anomalous_ml or anomalous_nn)
                summary = f"[Replay] Suspicious IP {src_ip} targeting {dst_ip} (malicious={malicious}, ml={anomalous_ml}, nn={anomalous_nn}, rules={len(suricata_matches)}, triage={triage})"
                event = {
                    "type": event_type,
                    "src_ip": src_ip,
                    "dst_ip": dst_ip,
...
class Honeypot(threading.Thread):
    def __init__(self, config: GuardianConfig, threat_matrix: ThreatMatrix,
                 firewall: FirewallManager, db: GuardianDB, sig_engine: SignatureEngine,
                 alerts: AlertEngine):
        super().__init__(daemon=True)
        self.config = config
        self.threat_matrix = threat_matrix
    ...
    def run(self):
        ...
        self._bind_port(self.config.honeypot_ftp_port, "FTP")
        self._bind_port(self.config.honeypot_telnet_port, "TELNET")
                ...
                try:
                    ...
                    try:
                        if proto == "SSH":
                            ...
                            try:
                                data = conn.recv(1024)
                                logger.info(f"[Honeypot] Cowrie-style SSH command from {ip}: {data!r}")
                            except Exception:
                                pass
                        elif proto == "HTTP":
                            ...
                            banner = b"220 Guardian FTP Honeypot\r\n"
                            conn.sendall(banner)
                        elif proto == "TELNET":
                            banner = b"Guardian Telnet Honeypot\r\nlogin: "
                            conn.sendall(banner)
                            try:
                                data = conn.recv(1024)
                                logger.info(f"[Honeypot] Telnet input from {ip}: {data!r}")
                            except Exception:
                                pass
                    except Exception:
                        pass
                    event_type = f"honeypot_hit_{proto.lower()}"
                    event = {
                        "type": event_type,
                        "src_ip": ip,
                    ...
                    self.threat_matrix.add_event(event)
                    self.alerts.intruder_alert(ip, CRITICAL_NODE_IP or "", 60, event_type)
...
class GuardianService(threading.Thread):
    def __init__(self, config: GuardianConfig, net_engine: NetworkDiscoveryEngine,
                 ...
                 mesh_router: MeshRouter, triage_engine: AgenticTriageEngine):
        super().__init__(daemon=True)
        self.config = config
        ...
        self.triage_engine = triage_engine
        self.running = True
        self.last_settings_save = time.time()
        ...
        while self.running:
            ...
            if self.anomaly.is_anomalous_rate():
                event = {
                    ...
                    "severity": 50,
                    "triage": self.triage_engine.classify(50, False, 0, False, True)
                }
                self.threat_matrix.add_event(event)
                self.anomaly.record_event(event)
                self.deep_ml.record_event(event)
            ...
            if self.deep_ml.is_deep_anomaly():
                event = {
                    ...
                    "severity": 70,
                    "triage": self.triage_engine.classify(70, False, 0, False, True)
                }
                self.threat_matrix.add_event(event)
            ...
            now = time.time()
            if now - self.last_settings_save > self.config.settings_persist_interval:
                self.config.save()
    ...
    def _simulate_traffic_and_detection(self):
        if random.random() < 0.1:
            ...
            triage = self.triage_engine.classify(80, True, 1, False, False)
            event = {
                "type": event_type,
                "src_ip": src_ip,
                "dst_ip": dst_ip,
            ...
            self.threat_matrix.add_event(event)
            self.alerts.intruder_alert(src_ip, dst_ip, 80, event_type)
    ...
    def __init__(self, ip: str, db: GuardianDB, firewall: FirewallManager, pcap_mgr: PacketCaptureManager, parent=None):
        ...
        def load_history():
            ...
            for h in history:
                ...
                lines.append(f"[{ts}] ({h['severity']}) {h['event_type']} (MITRE={h.get('mitre_tech','')}, triage={h.get('triage','')}): {h['summary']}")
...
class GuardianGUI:
    ...
    def launch(self):
        ...
        # Top row: threats + audio/visual controls
        top_layout = QtWidgets.QHBoxLayout()

        events_box = QtWidgets.QTextEdit()
        events_box.setReadOnly(True)
        events_box.setPlainText("\n".join(
            [f"[{e.get('severity',0)}] {e.get('summary', '')} (MITRE={e.get('mitre_tech','')}, triage={e.get('triage','')})"
             for e in self.threat_matrix.get_recent_events()]
        ))
        top_layout.addWidget(events_box)
        ...
        indicator_layout.addLayout(controls_layout)
...
def main():
    ...
    db = GuardianDB()
    mitre = MITREMapper()
    triage_engine = AgenticTriageEngine(config, db, None, None)  # firewall/mesh will be wired later
    ...
    triage_engine.firewall = firewall
    triage_engine.mesh_router = mesh_router

    threat_matrix = ThreatMatrix(db, mitre, triage_engine)
    ...
    kernel_layer = KernelTelemetryLayer(config)
