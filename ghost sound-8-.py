...
def emit_event(ev: dict):
    ...
    if len(STATE["event_stream"]) > 1000:
        STATE["event_stream"] = STATE["event_stream"][-1000:]
    save_state(STATE)
...
def freeze_incident_session(snapshot):
    session = {
        ...
        "event_stream_last_200": STATE.get("event_stream", [])[-200:],
    }
    try:
...
class SecurityBridgeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Security Bridge - v7 (Operator Mode: JSON Stream + MITRE Filters + Profiles + RSE)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            top_frame,
            text="Security Bridge v7 - Operator Mode",
        ...
        ttk.Label(mitre_bar, text="MITRE filter:").pack(side=tk.LEFT)
        ...
        for info in snapshot:
            if info.remote_index >= 40 or any(k in (info.name or "").lower() for k in REMOTE_TOOL_KEYWORDS):
                self.remote_text.insert(
                    ...
                    f"remote_index={info.remote_index} RSE={info.rse_verdict}({info.rse_score}%)\n"
                )
...
def main():
    log_event("INFO", "Security Bridge (Full Upgrade v7) started.")
    start_honeypot()
    root = tk.Tk()
