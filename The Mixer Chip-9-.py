"""
Golden Star MPU – AI Security Engine v12

Upgrades in this revision:

- Full(er) Suricata rule grammar (flowbits, byte_extract stubs, http modifiers, tls fingerprints)
- TCP stream reassembly (per-flow buffers, basic reassembly)
- Real HTTP parsing (request line, headers, method, path, host)
- Real TLS fingerprinting (ClientHello SNI + JA3-style hash stub)
- Real clustering foundation (NodeCluster with Raft/Paxos-style stubs, telemetry sync hooks)
- GPU inference via ONNX Runtime CUDA EP (if available)
- Sandbox VM integration stubs (QEMU/KVM command hooks, syscall trace placeholders)

NOTE:
- Still safe: no privileged live capture; PCAP is file-based, listener is local UDP only.
- VM sandbox integration is stubbed: commands/logging only, no destructive operations.
"""
...
# ============================================================
# TCP STREAM REASSEMBLY + HTTP/TLS PARSING
# ============================================================
...
class TCPStreamReassembler:
    """
    Very basic TCP stream reassembler:
    - Keyed by (src_ip, src_port, dst_ip, dst_port)
    - Appends payloads in arrival order (no seq/ack handling)
    """

    def __init__(self):
        self.streams: Dict[Tuple[str, int, str, int], bytearray] = {}
    ...
    def add_segment(self, packet: ParsedPacket):
        ...
        buf = self.streams.setdefault(key, bytearray())
        buf.extend(packet.payload)

    def get_stream(self, packet: ParsedPacket) -> bytes:
        key = (packet.src_ip, packet.src_port, packet.dst_ip, packet.dst_port)
        return bytes(self.streams.get(key, b""))
...
class TLSFingerprint:
    ...
    def parse_client_hello(payload: bytes) -> Optional[ParsedTLS]:
        # Very rough TLS ClientHello parsing for SNI + JA3-style hash stub
        if len(payload) < 5:
            return None
        ...
        if payload[1] != 0x03:
            return None
        # We won't fully parse; just extract SNI if possible and hash the payload
        sni = None
        try:
            # Search for "server_name" extension (0x00 0x00)
            idx = payload.find(b"\x00\x00")
            if idx != -1:
                # Very rough: assume SNI is shortly after
                sni_len_idx = idx + 9
                if sni_len_idx + 2 <= len(payload):
                    sni_len = struct.unpack("!H", payload[sni_len_idx:sni_len_idx+2])[0]
                    sni_start = sni_len_idx + 2
                    if sni_start + sni_len <= len(payload):
                        sni_bytes = payload[sni_start:sni_start+sni_len]
                        sni = sni_bytes.decode("utf-8", errors="ignore")
        except Exception:
            sni = None

        ja3_hash = hashlib.md5(payload).hexdigest()
        return ParsedTLS(sni=sni, ja3_hash=ja3_hash)
...
class DPIRule:
    ...
    def matches(self, packet: ParsedPacket) -> bool:
        ...
        if self.http_modifiers and packet.http:
            # Example: http_method, http_uri, http_host
...
class DPIEngine:
    ...
    def _parse_suricata_options(self, body: str) -> Dict[str, Any]:
        ...
        # Split by ';' but ignore trailing empty
        ...
        for p in parts:
            if p.startswith("msg:"):
                ...
                # flowbits:set,somebit or flowbits:is,somebit
                if "set," in p:
                ...
                # custom extension: tls.fingerprint:JA3HASH
        ...
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    if "http" in body_lower or opts["http_modifiers"]:
                        group = DPIGroup.HTTP
                    if "dns" in body_lower:
                        group = DPIGroup.DNS
                    if "tls" in body_lower or "ssl" in body_lower or opts["tls_fingerprint"]:
                        group = DPIGroup.TLS
...
class MLInferenceEngine:
    def __init__(self, npu: LegacyNPUAdapter, cfg: Dict[str, str], gpu_info: Dict[str, Any]):
        self.npu = npu
...
class SandboxEngine:
    ...
    def _run_vm_sandbox(self, payload: bytes) -> Dict[str, Any]:
        """
        Stub for VM sandbox integration:
        - Logs intended QEMU/KVM command
        - Returns simulated syscall trace summary
        """
        ...
        log.info(f"[SANDBOX] Would run VM: {qemu_path} -drive file={vm_image} (stub only)")
        # Simulated syscall trace
        syscalls = ["open", "read", "write", "connect", "send", "recv"]
...
class NodeCluster:
    """
    Stub for distributed node mesh:
    - Tracks node states
    - Provides hooks for telemetry/threat sync
    - Raft/Paxos-style placeholders
    """
...
class MPUGui:
    ...
    def refresh_panel(self):
        ...
        for n, s in view.items():
            ...
            self.info_box.insert(tk.END, f"  {n}: hb={hb}, telem={s['telemetry_count']}, threats={s['threat_count']}\n")
...
class ClusterWindow:
    ...
    def update_cluster(self):
        ...
        for n, s in view.items():
            ...
            self.text.insert(tk.END, f"  {n}: hb={hb}, telem={s['telemetry_count']}, threats={s['threat_count']}\n")
...
def command_shell(mpu: GoldenStarMPU):
    ...
    print("Commands: submit, status, devices, bios, log, cluster, exit")
    while True:
        ...
        if cmd == "exit":
            ...
            for n, s in view.items():
                hb = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(s["last_heartbeat"]))
                print(f"  {n}: hb={hb}, telem={s['telemetry_count']}, threats={s['threat_count']}")
        else:
