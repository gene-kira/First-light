...
- FIX: auto-register all current drives so extra drives always show up in mapped table
"""
...
class MappingManager:
    ...
    def full_sync(self):
        # 1) Discover all current physical mounts
        current_mounts = []
        try:
            for part in psutil.disk_partitions(all=False):
                # Skip CD-ROM / empty drives on Windows
                if os.name == "nt":
        ...
        # 2) Ensure each mount has an extended label
        ...
        # 3) Keep GUID ↔ mount mapping in sync
...
def launch_gui():
    ...
    def refresh_settings():
        ...
        mgr.full_sync()  # now auto-registers all current mounts
