...
LOG_PATH = os.path.join(LOG_DIR, "system_governor_v1_1.log")
...
class UsageTracker:
    ...
    def score_process(self, name, info):
        ...
        mem_hist = info.get("mem_history", [])
        io_hist = info.get("io_history", [])
        if cpu_hist:
...
class GovernorWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("System Governor v1.1")
