...
- Fault tolerance (health, retry)
- ZMQ swarm OFF by default (manual start from GUI)
- ZMQ "port stacking": first process binds (server), others connect (client)
- ZMQ client can submit jobs to server (cpu/io/gpu/npu/async/water/plugin)
- Persistent memory across reboots (swarm_state.json) with controlled saves
"""
...
# ---------------------------------------------------------------------------
# Persistent memory (settings + queen state)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# LAN/WAN Swarm via ZeroMQ (manual start, port stacking, client jobs)
# ---------------------------------------------------------------------------

class ZMQSwarm:
    """
    Port stacking behavior:
    - First process that starts ZMQSwarm will bind tcp://*:6000 (server).
    - Subsequent processes will connect to tcp://localhost:6000 (client).
    - Multiple programs share the same port logically (one server, many clients).
    Client mode can submit jobs to server.
    """
    ...
    def _loop(self):
        ...
        try:
            ...
            # Fall back to client mode (port stacking)
    ...
    def _client_loop(self, socket):
        # Simple heartbeat loop; jobs are sent via send_job()
        while not self._stop_event.is_set():
            try:
    ...
    # -------- client-side job submission API --------
...
class SwarmGUI(QtWidgets.QMainWindow):
    def __init__(self, swarm: BeastSwarm, tuner: SwarmTuner, queen: Queen, memory: MemoryManager, zmq_swarm: ZMQSwarm):
        ...
        self.setWindowTitle("Beast Swarm Engine v6.2 - Real-Time Queen")
    ...
    def send_zmq_demo_jobs(self):
        if self.zmq_swarm.mode != "client":
            ...
            return
        # Send a few jobs to server
        self.zmq_swarm.send_job("cpu", name="zmq_cpu_0", args=[5_000_00], kwargs={}, priority=5)
        ...
        self.zmq_swarm.send_job("water", name="zmq_water_0", args=[], kwargs={"steps": 150, "dt": 0.01}, priority=4)
