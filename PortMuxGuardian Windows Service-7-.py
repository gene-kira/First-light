#!/usr/bin/env python3
# codex_security_bridge_v4_3_hybrid_deception.py
# Headless AI security + deception daemon (v4.3):
...
BASE_DIR = os.path.join(os.path.expanduser("~"), "CodexSecurityBridgeV4_3")
DB_PATH = os.path.join(BASE_DIR, "codex_v4_3.db")
...
class Persistence:
    ...
    def _init_db(self) -> None:
        ...
        conn.commit()
        conn.close()
        log("[DB] Initialized codex_v4_3.db")
...
class Supervisor:
    ...
    def start(self) -> None:
        if self.running:
            return
        self.running = True
        log("[SUP] Starting Codex Security Bridge v4.3 Hybrid Deception")
...
def main() -> None:
    log("[MAIN] Codex Security Bridge v4.3 hybrid deception starting")
    SUPERVISOR.start()
    ...
    try:
        ...
        SUPERVISOR.stop()
        server.shutdown()
        log("[MAIN] Codex Security Bridge v4.3 stopped")
