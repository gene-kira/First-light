#!/usr/bin/env python
# Codex Purge Shell v4.8 (Safe Edition) — Win11 Security Bridge & Correlation Console
# - Auto-elevation
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4.8] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4_8"
        else:
            self.model = None
...
class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4.8 (Safe Edition) — Win11 Security Bridge & Correlation Console")
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4.8 (Safe Edition) — Windows 11 Security Bridge, Threat Timeline & Correlation Console",
            font=("Arial", 16)
        )
        ...
        self.log_alert("Codex Purge Shell v4.8 (Safe Edition) started with reboot-persistent config, green Suricata, and green Sentinel persona.")
