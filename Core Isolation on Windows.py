...
def get_device_guard_status():
    """
    Uses WMI Win32_DeviceGuard to approximate:
    - SecurityServicesConfigured
    - SecurityServicesRunning
    - VirtualizationBasedSecurityStatus
    - HVCIEnabled (Memory Integrity)
    """
    if wmi is None:
        return None
...
def get_secure_boot_status():
    """
    Reads SecureBoot registry state (approx).
    Official way is Confirm-SecureBootUEFI in PowerShell,
    but we can infer from registry.
    """
    try:
...
def get_bitlocker_status():
    """
    BitLocker is best queried via manage-bde or PowerShell,
    but we can approximate via registry.
    """
    try:
        key_path = r"SYSTEM\\CurrentControlSet\\Control\\BitLocker"
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, key_path) as key:
            # This is heuristic; exact values depend on config
...
# ---------- Driver scan (potential blockers) ----------

def get_driver_blockers():
    """
    Uses Win32_PnPSignedDriver to list non-Microsoft/non-major OEM drivers
    that could potentially conflict with Memory Integrity.
...
# ---------- Memory Integrity (HVCI) toggle via registry ----------

def set_memory_integrity(enable: bool):
    """
    Toggles Memory Integrity (HypervisorEnforcedCodeIntegrity) via registry.
...
def main():
    ...
    if not is_admin():
        print("This script should be run as Administrator for full functionality.")
        # Still allow GUI, but toggling may fail
