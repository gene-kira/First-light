...
# -*- coding: utf-8 -*-
"""
Universal GPU Control Center v4.1 (Monolithic Governor)

Upgrades in this build:
- Advanced GPU tuning organ (safe, vendor-limited, NVML-based where possible)
- Full GUI dashboard (multi-GPU table + live charts + profile controls)
- Expanded REST API (GPU, profiles, plugins, vram, sync, prediction)
- Swarm sync organ (peer telemetry exchange over HTTP)
- Prediction engine organ (simple trend-based forecasting)
- Plugin API organ (hooks, events, telemetry interceptors)
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "auto_profile": {
        ...
        "power_target_idle_percent": 85,
        "power_target_active_percent": 105,
        "fan_idle_percent": 35,
        "fan_active_percent": 75,
    },
    "beast_mode": {
        "worker_threads": 4,
...
class PluginAPI:
    def __init__(self, bus: "EventBus", telemetry: TelemetryEngine, vram_engine: VramComputeEngine) -> None:
        self.bus = bus
...
class ProfileManager:
    def __init__(self, cfg: dict[str, Any], bus: EventBus, nvml_backend: Optional[NvmlBackend]) -> None:
        self.cfg = cfg
        self.bus = bus
    ...
    def apply_mode(self, mode: str) -> None:
        ...
        self._apply_gpu_tuning_for_mode(mode)

    def _apply_gpu_tuning_for_mode(self, mode: str) -> None:
        ...
        try:
            ...
            return
        if mode == "base":
            p = self.cfg["base_profile"]["power_target_percent"]
            f = self.cfg["base_profile"]["fan_target_percent"]
        elif mode == "turbo":
            p = self.cfg["turbo_profile"]["power_target_percent"]
            f = self.cfg["turbo_profile"]["fan_target_percent"]
        else:
            return
        for i in range(count):
            self.nvml_backend.set_power_limit_percent(i, p)
            self.nvml_backend.set_fan_speed_percent(i, f)
...
class PredictionEngine:
    ...
    def forecast(self) -> dict[str, Any]:
        ...
        window = self.cfg.get("window", 60)
        hist = self.telemetry_engine.get_history()
        if not hist:
            return {"enabled": True, "status": "no_data"}
        recent = [s for s in hist if time.time() - s.timestamp <= window]
        if len(recent) < 3:
        ...
        if len(temps) < 3 or len(utils) < 3:
            return {"enabled": True, "status": "insufficient_data"}
        t_slope = self._simple_slope(temps, window)
        u_slope = self._simple_slope(utils, window)
        horizon = self.cfg.get("horizon_seconds", 60)
        temp_future = temps[-1] + t_slope * (horizon / window)
        util_future = utils[-1] + u_slope * (horizon / window)
        return {
            "enabled": True,
            "status": "ok",
            "window": window,
    ...
    def _simple_slope(self, values: List[float], window: float) -> float:
        if len(values) < 2:
            return 0.0
    ...
    def _update_dashboard(self) -> None:
        ...
        if pred.get("status") == "ok":
            self.pred_label.setText(
                ...
                f"(horizon {pred['horizon_seconds']}s)"
            )
        else:
...
class DaemonServer:
    ...
    async def _run_http(self) -> None:
        ...
        async def handle_sync(request: aiohttp.web.Request) -> aiohttp.web.Response:
            data = await request.json()
            node_id = data.get("node_id")
            print(f"[Sync] received payload from {node_id}")
...
def start_governor_runtime() -> None:
    ...
    profile_manager = ProfileManager(CONFIG, bus, nvml_backend)

    plugin_api = PluginAPI(bus, telemetry_engine, vram_engine)
    ...
    sync_manager = SyncManager(sync_cfg, telemetry_engine)
    sync_manager.start()
    ...
    gui_shell = GuiShell(telemetry_engine, profile_manager, prediction_engine, nvml_backend)
