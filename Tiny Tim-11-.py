#!/usr/bin/env python3
"""
Unified LLM Dissector System (Next Evolution + Persistent Vector Store + Multi-Agent Debate + GPU Batching)
...
+ Persistent Vector Store (JSON-based)
...
class PersistentVectorStore:
    ...
    def save(self):
        try:
            ...
            print("[VectorStore] Save error:", e)
...
def start_extension_server(cockpit: Optional[LLMCockpitGUI], dissector: Optional["LLMDissector"]):
    ...
    def extension_event():
        ...
        if dissector and text:
            # simple auto-tag + refinement hook
            try:
...
class AsyncBatcher:
    ...
    def _flush(self):
        ...
        outs = []
        # GPU-aware: if Torch backend and CUDA, we could batch, but here we just loop
...
class LLMDissector:
    def __init__(
        ...
        self.vector_store = PersistentVectorStore(vector_store_path)
    ...
    def multi_agent_debate(self, question: str, chapters: Dict[str, Any], num_agents: int = 3) -> Tuple[str, float]:
        ...
        # critic on best candidate (first)
    ...
    def self_repair(self):
        # simple checks: chapters file, graph file, vector store
        try:
            if not os.path.exists(self.chapters_path):
                if self.cockpit:
...
def main():
    ...
    if LLMCockpitGUI is not None:
        ...
        def run_llm_system():
            ...
            # start extension server bound to first dissector
            if dissectors:
