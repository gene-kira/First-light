...
# -*- coding: utf-8 -*-
"""
Universal GPU Control Center v4.5
ML Upgrades Edition

Upgrades over v4.4:
- ML-based thermal prediction:
  - Lightweight regression model trained online from telemetry
  - Predicts future temperature and risk more accurately than simple slopes
- ML-based workload classification:
  - Classifies workload as: idle / light / gaming / compute / mixed
  - Uses GPU util, memory usage, process info, and time patterns
- Anomaly detection:
  - Detects abnormal temperature/utilization patterns
  - Flags potential cooling issues or runaway workloads
- Enhanced auto mode:
  - Uses ML workload class + ML thermal prediction + PID + curves
- REST API extensions:
  - /ml/prediction
  - /ml/workload
  - /ml/anomaly
...
class MLWorkloadClassifier:
    ...
    def classify(self, sample: TelemetrySample) -> str:
        ...
        if util < idle_th and mem_ratio < 0.2:
            return "idle"
        if util < light_th:
            return "light"
        if util < gaming_th and mem_ratio < 0.7:
            return "gaming"
        if util >= compute_th and mem_ratio >= 0.7:
            return "compute"
        return "mixed"
...
class PredictionEngine:
    ...
    def forecast(self) -> dict[str, Any]:
        ...
        temp_future_ml = self.ml_thermal.predict(
            gpu_index=any_sample.gpu_index if latest_by_gpu else 0,
            util=util_now,
            temp=temp_now,
            mem_ratio=mem_ratio,
            horizon_sec=horizon,
        ...
        temp_future = 0.7 * temp_future_ml + 0.3 * temp_future_slope
...
class TuningEngine:
    ...
    def apply_auto_tuning(self) -> None:
        ...
        for i in range(count):
            ...
            if anomaly_info.get("anomaly_temp"):
                base_power *= 0.9
                base_fan *= 1.1
...
class GuiShell:
    ...
    def start(self) -> None:
        ...
        self.table.setHorizontalHeaderLabels(
            ["GPU", "Name", "Util %", "Temp C", "Mem Used", "Mem Total", "Power %", "Fan %", "Workload"]
        )
        ...
        self.ml_label = QtWidgets.QLabel("ML: waiting...")
    ...
    def _update_dashboard(self) -> None:
        ...
        if pred.get("status") == "ok":
            self.pred_label.setText(
                ...
                f"(ML {pred['temp_future_ml']:.1f}C), util {pred['util_now']:.1f}→{pred['util_future']:.1f}% "
        ...
        if anomaly.get("status") == "ok":
            self.ml_label.setText(
                f"ML: z_temp={anomaly['z_temp']:.2f} z_util={anomaly['z_util']:.2f} "
                f"anomaly_temp={anomaly['anomaly_temp']} anomaly_util={anomaly['anomaly_util']}"
            )
        else:
            self.ml_label.setText(f"ML: {anomaly.get('status')}")
...
def start_governor_runtime() -> None:
    ...
    ml_anomaly = MLAnomalyDetector(ml_cfg)
    ...
    prediction_engine = PredictionEngine(CONFIG.get("prediction", {}), telemetry_engine, ml_thermal)
