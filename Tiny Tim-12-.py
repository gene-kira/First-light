#!/usr/bin/env python3
"""
Unified LLM Dissector System (Next Evolution ++ Agentic Platform)
...
+ Semantic Search
+ Chapter Merging (centroid-based)
+ Auto-Tagging
+ Browser Extension Hooks / API (file + local HTTP integration)
...
+ Retrieval Caching
+ Contextual Memory + Decay + Persistence
+ Multi-Agent Reasoning (3–5 agents, debate, critic, verifier)
...
+ Graph-Based Routing
+ Semantic Pruning
+ Async Inference Batching (GPU-aware)
+ Embedding Caching with TTL
+ Chapter Preloading (via refinement)
+ Memory-Based Warm Starts + Persistence
+ Routing Heatmaps
+ Web → Chapter Auto-Tagging
+ Web-Triggered Chapter Refinement
+ Persistent Vector Store (JSON/SQLite/FAISS)
+ Self-Repair Routines (basic)
+ Debate Transcripts
+ Cross-Agent Critique
+ Verification Chains
+ Tool-Using Agents
+ Persistent Chapter Embeddings
+ Distributed Mode (Coordinator Stub)
+ Full Web Scraping Integration
"""
...
class AutoLoader:
    def __init__(self):
        ...
        self.required_packages: Dict[str, str] = {
            ...
            "flask": "flask",
            "bs4": "beautifulsoup4",
            "sqlite3": "sqlite3",  # builtin, but kept for clarity
        }
        ...
        try:
            ...
            if module_name == "sqlite3":
                ...
                try:
                    ...
                    print("[AutoLoader] sqlite3 load error:", e)
                    return None
    ...
    def load_all(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.loaded_modules["faiss"] = faiss
        except Exception:
            self.loaded_modules["faiss"] = None
...
flask = _modules.get("flask")
bs4 = _modules.get("bs4")
sqlite3 = _modules.get("sqlite3")
faiss = _modules.get("faiss")
...
DEFAULT_PROMPT_TEMPLATES = {
    ...
    "critic": (
        ...
        "Question:\n{question}\n\n"
        "Candidate answer:\n{answer}\n\n"
        "List potential issues, missing details, or errors.\n"
        "Then propose a corrected/improved answer.\n"
        "Format:\nIssues:\n- ...\nImproved Answer:\n<answer>"
    ),
    "verifier": (
        ...
        "Question:\n{question}\n\n"
        "Candidate answer:\n{answer}\n\n"
        "Rate correctness from 0 to 1 and explain briefly.\n"
        "Format:\nSCORE: <0-1>\nNOTE: <short note>"
    ),
    "drift_detect": (
    ...
    "tool_agent": (
        ...
        "Question:\n{question}\n\n"
        "You may request tools like:\n"
        "- WEB_SCRAPE(url)\n"
        "- SEARCH_CHAPTERS(query)\n"
        "Respond with either:\n"
        "TOOL: <tool_call>\nOR\n"
        "ANSWER: <final answer>"
    ),
...
# =========================
# SIMPLE CONTEXTUAL MEMORY WITH DECAY + PERSISTENCE
# =========================

class ContextMemory:
    def __init__(self, path: str = "context_memory.json", max_items: int = 200, decay_half_life: float = 600.0):
        self.path = path
    ...
    def save(self):
        try:
            ...
            print("[ContextMemory] Save error:", e)
...
# =========================
# VECTOR STORE (JSON / SQLite / FAISS)
# =========================

class VectorStoreBase:
    def upsert(self, key: str, embedding: np.ndarray, meta: Dict[str, Any]):
        raise NotImplementedError

    def get(self, key: str) -> Optional[np.ndarray]:
        raise NotImplementedError

    def search(self, query_emb: np.ndarray, top_k: int = 5) -> List[Tuple[str, float, Dict[str, Any]]]:
        raise NotImplementedError

class JSONVectorStore(VectorStoreBase):
        ...
        if os.path.exists(self.path):
            try:
                ...
                for key, val in raw.items():
                    emb = np.array(val["embedding"], dtype=np.float32)
                    self.store[key] = {
                        "embedding": emb,
                        "meta": val.get("meta", {}),
                    }
    ...
    def save(self):
        try:
            ...
            for key, val in self.store.items():
                out[key] = {
                    "embedding": val["embedding"].tolist(),
                    "meta": val.get("meta", {}),
                }
            with open(self.path, "w", encoding="utf-8") as f:
                json.dump(out, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print("[VectorStore(JSON)] Save error:", e)
    ...
    def search(self, query_emb: np.ndarray, top_k: int = 5) -> List[Tuple[str, float, Dict[str, Any]]]:
        ...
        for key, val in self.store.items():
            emb = val["embedding"]
            num = float(np.dot(query_emb, emb))
            den = float(np.linalg.norm(query_emb) * np.linalg.norm(emb))
            score = num / den if den != 0 else 0.0
            results.append((key, score, val.get("meta", {})))
...
class SQLiteVectorStore(VectorStoreBase):
    ...
    def upsert(self, key: str, embedding: np.ndarray, meta: Dict[str, Any]):
        ...
        c.execute("REPLACE INTO vectors (key, embedding, meta) VALUES (?, ?, ?)", (key, emb_bytes, meta_json))
        self.conn.commit()

    def get(self, key: str) -> Optional[np.ndarray]:
        c = self.conn.cursor()
        c.execute("SELECT embedding FROM vectors WHERE key = ?", (key,))
        row = c.fetchone()
        if not row:
            return None
        emb_bytes = row[0]
    ...
    def search(self, query_emb: np.ndarray, top_k: int = 5) -> List[Tuple[str, float, Dict[str, Any]]]:
        # naive: load all, compute cosine
        c = self.conn.cursor()
        c.execute("SELECT key, embedding, meta FROM vectors")
        rows = c.fetchall()
        results = []
        for key, emb_bytes, meta_json in rows:
            emb = np.frombuffer(emb_bytes, dtype=np.float32)
            num = float(np.dot(query_emb, emb))
            den = float(np.linalg.norm(query_emb) * np.linalg.norm(emb))
            score = num / den if den != 0 else 0.0
...
class FAISSVectorStore(VectorStoreBase):
    def __init__(self, dim: int, path: str = "vector_store.faiss"):
        self.path = path
        self.dim = dim
    ...
    def get(self, key: str) -> Optional[np.ndarray]:
        # FAISS doesn't store per-key embeddings easily; skip
        return None
    ...
    def search(self, query_emb: np.ndarray, top_k: int = 5) -> List[Tuple[str, float, Dict[str, Any]]]:
        ...
        scores, idxs = self.index.search(q, min(top_k, len(self.keys)))
        results = []
        for s, i in zip(scores[0], idxs[0]):
            if i < 0 or i >= len(self.keys):
                continue
            key = self.keys[i]
            results.append((key, float(s), self.meta.get(key, {})))
        return results

def build_vector_store(dim: int) -> VectorStoreBase:
    # preference: FAISS → SQLite → JSON
    if faiss is not None:
        print("[VectorStore] Using FAISS backend.")
        return FAISSVectorStore(dim=dim)
    if sqlite3 is not None:
        print("[VectorStore] Using SQLite backend.")
        return SQLiteVectorStore()
    print("[VectorStore] Using JSON backend.")
...
class DebateTranscriptStore:
    def __init__(self, path: str = "debate_transcripts.json"):
        self.path = path
    ...
    def save(self):
        try:
            ...
            print("[DebateTranscripts] Save error:", e)

    def add(self, question: str, agents: List[str], critic: str, verifier_note: str, final_answer: str, confidence: float):
...
# =========================
# TOOL REGISTRY + WEB SCRAPER
# =========================

class ToolRegistry:
    def __init__(self, dissector: "LLMDissector"):
    ...
    def web_scrape(self, url: str) -> str:
        ...
        try:
            ...
            return f"[WEB_SCRAPE_ERROR] {e}"

    def search_chapters(self, query: str) -> str:
        try:
            chapters = self.dissector.load_chapters()
            results = self.dissector.semantic_search(query, chapters, top_k=5)
            lines = []
            for cid, score in results:
                ch = chapters[cid]
                lines.append(f"{cid} ({score:.3f}): {ch['title']} - {ch['summary']}")
            return "\n".join(lines)
        except Exception as e:
            return f"[SEARCH_CHAPTERS_ERROR] {e}"

    def handle_tool_call(self, tool_call: str) -> str:
        tool_call = tool_call.strip()
        if tool_call.upper().startswith("WEB_SCRAPE(") and tool_call.endswith(")"):
            url = tool_call[len("WEB_SCRAPE("):-1].strip().strip('"').strip("'")
            return self.web_scrape(url)
        if tool_call.upper().startswith("SEARCH_CHAPTERS(") and tool_call.endswith(")"):
            q = tool_call[len("SEARCH_CHAPTERS("):-1].strip().strip('"').strip("'")
            return self.search_chapters(q)
        return f"[UNKNOWN_TOOL] {tool_call}"
...
class DistributedCoordinator:
    """
    Simple stub: in real use, this would coordinate multiple nodes.
    Here, it just logs and could be extended to RPC.
    """
    def __init__(self, cockpit=None):
        self.cockpit = cockpit
        self.nodes: Dict[str, Dict[str, Any]] = {}

    def register_node(self, name: str, role: str, info: Dict[str, Any]):
        self.nodes[name] = {"role": role, "info": info}
        if self.cockpit:
            self.cockpit.log_patterns(f"[DIST] Node registered: {name} role={role} info={info}")
...
# =========================
# WEB ACTIVITY MONITOR + REAL EXTENSION MESSAGING (HTTP)
# =========================
...
# Simple local HTTP server for browser extension messaging
def start_extension_server(cockpit: Optional[LLMCockpitGUI], dissector: Optional["LLMDissector"]):
    if flask is None:
        if cockpit:
            cockpit.log_web("[WEB] Flask not available; extension server disabled.")
        return

    app = flask.Flask("LLMExtensionServer")

    @app.route("/extension/event", methods=["POST"])
    def extension_event():
        data = flask.request.json or {}
        url = data.get("url", "")
        text = data.get("text", "")
        if cockpit:
            cockpit.log_web(f"[EXT] Event from extension: url={url}, text_len={len(text)}")
            cockpit.set_web_url(url)
        if dissector and text:
            try:
                chapters = dissector.load_chapters()
                q_emb = dissector._embed_text(text)
                scores = []
                for label, ch in chapters.items():
                    centroid = np.array(ch["centroid"], dtype=np.float32)
                    s = dissector._cosine_sim(q_emb, centroid)
                    scores.append((label, s))
                scores.sort(key=lambda x: x[1], reverse=True)
                if scores:
                    best_label = scores[0][0]
                    dissector.refine_chapter_over_time(best_label, chapters)
            except Exception as e:
                if cockpit:
                    cockpit.log_web(f"[EXT] Refinement error: {e}")
        return {"status": "ok"}

    threading.Thread(target=lambda: app.run(host="127.0.0.1", port=5005, debug=False, use_reloader=False), daemon=True).start()
    if cockpit:
        cockpit.log_web("[WEB] Extension server started on http://127.0.0.1:5005")
...
class KnowledgeGraph:
    ...
    def add_chapter(self, chapter_id: str, title: str, summary: str, topic_seed: str, tags: List[str], centroid: List[float]):
    ...
    def merge_similar_chapters_centroid(self, chapters: Dict[str, Any], threshold: float = 0.85) -> Dict[str, Any]:
        labels = list(chapters.keys())
        merged: Dict[str, Any] = {}
        used = set()
...
def build_chapters_from_index(index: Dict[str, Any], llm_generate) -> Dict[str, Any]:
    ...
    for label, data in index.items():
        ...
        chapters[label] = {
            ...
            "subtopics": [],
            "tags": tags,
            "embeddings": [],  # persistent chapter embeddings
        }
    return chapters
...
class LLMDissector:
    def __init__(
        ...
        vector_store_path: str = "vector_store.db",
        memory_path: str = "context_memory.json",
        debate_path: str = "debate_transcripts.json",
    ):
        ...
        # vector store dimension = primary + secondary
        dummy = self.embed_model_primary.encode(["test"], convert_to_numpy=True)[0]
        dummy2 = self.embed_model_secondary.encode(["test"], convert_to_numpy=True)[0]
        dim = dummy.shape[0] + dummy2.shape[0]
        self.vector_store = build_vector_store(dim)
        ...
        self.memory = ContextMemory(path=memory_path)
        self.debate_store = DebateTranscriptStore(path=debate_path)
    ...
    def _embed_text(self, text: str) -> np.ndarray:
        ...
        self.vector_store.upsert(f"text::{hash(text)}", emb, {"text": text})
        return emb
    ...
    def dissect(
        ...
        for label, ch in chapters.items():
            ...
            for e in ch["examples"]:
                ...
                ch_embs.append(emb.tolist())
            ch["embeddings"] = ch_embs

        merged_chapters = self.graph.merge_similar_chapters_centroid(chapters, threshold=0.85)
        ...
        for cid, ch in pruned_chapters.items():
            ...
            self.graph.add_chapter(cid, ch["title"], ch["summary"], topic_seed, ch.get("tags", []), ch["centroid"])
    ...
    def critic_agent(self, question: str, answer: str) -> str:
        prompt = DEFAULT_PROMPT_TEMPLATES["critic"].format(question=question, answer=answer)
        try:
            return self.generate(prompt, max_new_tokens=256)
        except Exception:
            return "Issues:\n- critic failed\nImproved Answer:\n" + answer

    def verifier_agent(self, question: str, answer: str) -> Tuple[float, str]:
        prompt = DEFAULT_PROMPT_TEMPLATES["verifier"].format(question=question, answer=answer)
        try:
            resp = self.generate(prompt, max_new_tokens=128)
            score = 0.0
            note = ""
            for line in resp.splitlines():
                line = line.strip()
    ...
    def tool_using_agent(self, question: str, chapters: Dict[str, Any]) -> str:
        prompt = DEFAULT_PROMPT_TEMPLATES["tool_agent"].format(question=question)
        resp = self.generate(prompt, max_new_tokens=256)
        lines = resp.splitlines()
        for line in lines:
            line = line.strip()
            if line.upper().startswith("TOOL:"):
                tool_call = line.split(":", 1)[1].strip()
                tool_result = self.tools.handle_tool_call(tool_call)
                return f"[TOOL_RESULT]\n{tool_result}"
            if line.upper().startswith("ANSWER:"):
                return line.split(":", 1)[1].strip()
        return resp

    def multi_agent_debate(self, question: str, chapters: Dict[str, Any], num_agents: int = 3) -> Tuple[str, float]:
        agents_answers = []
        for i in range(num_agents):
            prompt = DEFAULT_PROMPT_TEMPLATES["multi_agent"].format(agent_id=i+1, question=question)
            ans = self.generate(prompt, max_new_tokens=256)
            agents_answers.append(ans)

        # cross-agent critique: each agent critiques others
        critique_log = []
        for i in range(num_agents):
            for j in range(num_agents):
                if i == j:
                    continue
                q = question
                candidate = agents_answers[j]
                crit = self.critic_agent(q, candidate)
                critique_log.append(f"Agent {i+1} critiqued Agent {j+1}:\n{crit}")

        # base candidate
        base_answer = agents_answers[0]
        critic_output = self.critic_agent(question, base_answer)
        improved_answer = base_answer
        for line in critic_output.splitlines():
            if line.lower().startswith("improved answer:"):
                improved_answer = line.split(":", 1)[1].strip()
                break

        # verification chain: verifier + self-consistency
        score, note = self.verifier_agent(question, improved_answer)
        sc = self.self_consistency_score(question, improved_answer)
        final_conf = (score + sc) / 2.0

        # store debate transcript
        self.debate_store.add(
            question=question,
            agents=agents_answers,
        ...
        if self.cockpit is not None:
            self.cockpit.log_patterns("[DEBATE] Transcript stored.")
            self.cockpit.log_patterns("\n".join(critique_log))

        return improved_answer, final_conf
    ...
    def answer_with_chapters(self, question: str, chapters: Dict[str, Any], use_multi_agent: bool = True, explain: bool = False, use_tools: bool = True) -> Tuple[str, float, Optional[str]]:
        if use_tools:
            tool_ans = self.tool_using_agent(question, chapters)
            # tool result is used as context or direct answer; here we just append
            question = question + "\n\n[TOOL_CONTEXT]\n" + tool_ans

        if use_multi_agent:
            ans, conf = self.multi_agent_debate(question, chapters, num_agents=3)
        else:
    ...
    def self_repair(self):
        try:
            if not os.path.exists(self.chapters_path):
                if self.cockpit:
                    self.cockpit.log_patterns("[SELF-REPAIR] Chapters missing; attempting empty init.")
                with open(self.chapters_path, "w", encoding="utf-8") as f:
                    json.dump({}, f)
            if not os.path.exists(self.graph.path):
                if self.cockpit:
                    self.cockpit.log_patterns("[SELF-REPAIR] Graph missing; reinitializing.")
                self.graph.save()
        except Exception as e:
            if self.cockpit:
                self.cockpit.log_patterns(f"[SELF-REPAIR] Error: {e}")
...
def ensemble_answer(question: str, dissectors: List[LLMDissector], explain: bool = False) -> str:
    ...
    for d in dissectors:
        try:
            ...
            ans, conf, explanation = d.answer_with_chapters(question, chapters, use_multi_agent=True, explain=explain, use_tools=True)
...
def main():
    ...
    if LLMCockpitGUI is not None:
        ...
        coordinator = DistributedCoordinator(cockpit=cockpit)
        ...
        def run_llm_system():
            ...
            for mid in model_ids:
                coordinator.register_node(mid, "llm", {"backend": "auto"})
            ...
            if dissectors:
                start_extension_server(cockpit, dissectors[0])
        ...
        dissectors: List[LLMDissector] = []
        coordinator = DistributedCoordinator(cockpit=None)
        for mid in model_ids:
            coordinator.register_node(mid, "llm", {"backend": "auto"})
        ...
        if dissectors:
            start_extension_server(None, dissectors[0])
