...
# ============================================================
# Aggressive Autoloader (Option A)
# - Tries to import required libs
# - If missing, runs pip install automatically
# - OS-agnostic: works on Windows/macOS/Linux (assuming Python + pip)
# ============================================================
...
def ensure_package(pkg_name, import_name=None):
    ...
    pkg_name: name for pip
    import_name: name for import (if different)
    """
...
def aggressive_autoload():
    ...
    # tk (on many systems this is part of Python; pip 'tk' may be a no-op or fail harmlessly)
    try:
        import tkinter  # noqa
    except ImportError:
        ensure_package("tk")
...
# After autoloader, import everything we need
import psutil
...
# ============================================================
# Simple Chrome / pdpro7 killer (destructive, optional)
# ============================================================
...
KILLER_LOG_FILE = "chrome_pdpro_killer_log.txt"
...
def killer_log(msg):
    ...
    print(line)
    try:
        with open(KILLER_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(line + "\n")
    except Exception:
        pass

def killer_is_target(name: str) -> bool:
    if not name:
        return False
    lname = name.lower()
    for t in TARGETS:
...
def kill_targets_once():
    ...
    for proc in psutil.process_iter(["name", "pid"]):
        try:
            ...
            if killer_is_target(name):
                try:
                    psutil.Process(pid).terminate()
                    killer_log(f"KILLED target process: {name} (PID={pid})")
                    killed += 1
                except Exception as e:
                    killer_log(f"Failed to kill {name} (PID={pid}): {e}")
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
    return killed

def killer_main_loop():
    killer_log("Chrome / pdpro7 killer starting (auto-kill instantly)")
    while True:
...
# ============================================================
# Rogue Process Monitor v14.0 (SAFE, NON-DESTRUCTIVE)
# Borg + ML + Plots + Aggressive Autoloader
# ============================================================

LOG_FILE = "rogue_monitor_v14_log.txt"
...
class BorgWorker(threading.Thread):
    ...
    def score_proc(self, proc):
        try:
            ...
            ml_anomaly = 0.0  # placeholder, queen will fill
...
class SandboxScanner:
    ...
    def scan_loop(self):
        log("SandboxScanner v14 Borg swarm scan loop started")
        while self.running:
            try:
    ...
    def tree_loop(self):
        log("SandboxScanner v14 tree loop started")
        while self.running:
            try:
                self.build_tree()
            except Exception as e:
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v14.0 (SAFE, Borg + ML + Plots + Autoloader)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v14.0 (SAFE, Borg + ML + Plots + Autoloader)", font=("Segoe UI", 14, "bold"))
    ...
    def _build_live_tab(self):
        ...
        title = ttk.Label(frame, text="Live Process View (Borg + ML Anomaly)", font=("Segoe UI", 12, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))
    ...
    def _update_status(self, text):
        ...
        self.status_label.config(
            text=f"Status: {text} | ML={ml_status} | Plots={plot_status}",
            foreground="#00aa00"
        )
...
def main():
    log("Rogue Process Monitor v14.0 (SAFE, Borg + ML + Plots + Autoloader) starting")
    # Optional: start killer loop in background if you want destructive mode alongside SAFE monitor
    # killer_thread = threading.Thread(target=killer_main_loop, daemon=True)
    # killer_thread.start()
