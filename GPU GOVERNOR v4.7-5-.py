...
# Clock/voltage control (NVAPI integration-ready), Qt6 GUI with:
# - Collapsible panels (accordion style)
# - Tabs (Telemetry / Fan Curve / Processes / Graphs)
# - Sidebar navigation layout
# - Auto-resizing responsive GUI
# - Dark mode toggle
# - Compact mode for small screens
# - Scrollable fan curve editor
# ML-based thermal prediction, GPU process viewer,
# VRAM fragmentation detector, Plugin system, Logging/CSV export
# ============================================================
...
def nvapi_set_fan_speed_percent(index: int, percent: int):
    if not HAS_NVAPI:
        ...
        return
    print(f"[NVAPI] Fan speed GPU {index} -> {percent}% (placeholder)")
...
class CurveVisualizer:
    ...
    def _prepare_ml_dataset(self, gpu_index: int, window_s: float = 300.0) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        if not HAS_TORCH:
            return None, None
        ...
        for s in samples:
            dt = s.timestamp - t0
            xs.append([dt, s.power, s.gpu_util, s.fan_speed, s.vram_used_mb])
            ys.append([s.temp])
        X = torch.tensor(xs, dtype=torch.float32)
        Y = torch.tensor(ys, dtype=torch.float32)
        return X, Y

    def train_ml_model(self, gpu_index: int, epochs: int = 200, lr: float = 1e-3):
        if not HAS_TORCH:
            print("[ML-TEMP] Torch not available, skipping ML training")
            return
        X, Y = self._prepare_ml_dataset(gpu_index)
        if X is None or Y is None:
            print("[ML-TEMP] Not enough data to train ML model")
            return
        in_dim = X.shape[1]
        ...
        for ep in range(epochs):
            ...
            opt.step()
            if ep % 50 == 0:
                print(f"[ML-TEMP] GPU {gpu_index} epoch={ep} loss={loss.item():.4f}")
        self.ml_models[gpu_index] = model
        print(f"[ML-TEMP] Trained ML thermal model for GPU {gpu_index}")

    def predict_next_temp_ml(self, gpu_index: int, horizon_s: float = 10.0) -> float:
        samples = self.get_recent_samples(gpu_index, window_s=60.0)
        if not samples:
            return 0.0
        if gpu_index not in self.ml_models:
            return samples[-1].temp
        model = self.ml_models[gpu_index]
        last = samples[-1]
        t0 = samples[0].timestamp
        dt_future = (last.timestamp - t0) + horizon_s
        x = torch.tensor([[dt_future, last.power, last.gpu_util, last.fan_speed, last.vram_used_mb]], dtype=torch.float32)
        with torch.no_grad():
            pred = model(x).item()
        return float(pred)
...
class GpuGovernor:
    ...
    def _ai_auto_tune(self, telem: Dict[str, Any]):
        ...
        util = telem["gpu_util"]
        power = telem["power"]
        predicted_temp = self.curve_vis.predict_next_temp_ml(self.gpu_index, horizon_s=10.0)
        if predicted_temp > self.cfg.temp_limit_c + 3:
            new_power = max(150, self.pending_power - 10)
            print(f"[AI-TUNE] ML predicted temp {predicted_temp:.1f}C > limit, reducing power to {new_power}W")
    ...
    def _safe_tune_loop(self):
        ...
        set_power_limit_w(self.gpu_index, self.cfg.power_limit_w)
        self.curve_vis.train_ml_model(self.gpu_index, epochs=100, lr=1e-3)
        start = time.time()
...
class GovernorGUI(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.setWindowTitle("GPU Governor v8.2 — Multi-GPU AI/HC/Gaming")
    ...
    def _build_telemetry_tab(self):
        ...
        self.lbl_pred_temp = QtWidgets.QLabel("ML Predicted Temp (+10s): - C")
    ...
    def _build_graph_tab(self):
        ...
        layout = QtWidgets.QVBoxLayout(tab)
        if HAS_QTCHARTS:
            charts_group = QtWidgets.QGroupBox("Dashboard — Temp / Power / Util / VRAM")
    ...
    def _on_export(self):
        ...
        any_gov.export_curve(str(BASE_DIR / "curve_v8_2.json"))
    ...
    def _update_telemetry(self):
        ...
        pred_temp = self.current_gov.curve_vis.predict_next_temp_ml(self.current_index, horizon_s=10.0)
        self.lbl_pred_temp.setText(f"ML Predicted Temp (+10s): {pred_temp:.1f} C")
