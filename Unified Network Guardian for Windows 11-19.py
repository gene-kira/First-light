...
# guardian_core.py
#
# Unified Network Guardian for Windows 11 (Option A — FULL INTEGRATION)
...
# - Full Suricata rule import (enhanced Suricata v6-style parser)
...
#     * Optional GPU deep learning (PyTorch) with CPU fallback
# - Per-IP ML anomaly detection + risk scoring
# - Per-IP heatmaps (time vs severity/rate)
# - Honeypot protocol emulation (Cowrie-style):
#     * Fake SSH
#     * Fake HTTP
...
# - Distributed mesh routing (enhanced consensus stub)
# - Zero-trust identity enforcement
# - TLS interception proxy (stubbed, optional)
# - NDIS/WinDivert interception stubs + kernel-level telemetry suppression (safe placeholders)
...
# - GUI dashboard with:
#   * Threat events + controls on top
#   * Audio OFF by default (toggleable)
...
# - AI Defender Agent:
#   * ForkliftLinear-wrapped HF model
#   * Agentic reasoning + threat triage
...
# PyTorch / Transformers for AI Defender Agent
try:
    import torch
...
DEFAULT_CONFIG = {
    "gui_enabled": True,
    "audio_alerts": False,  # audio OFF by default (per your request)
    "visual_alerts": True,
    "scan_min_seconds": 5,
    "scan_max_seconds": 60,
    ...
    "ai_primary_model_name": "gpt2",
    "kernel_telemetry_suppression_enabled": False,
    "ndis_interceptor_enabled": False
}
...
class GuardianConfig:
    def __init__(self):
        ...
        self.audio_alerts = self.config.get("audio_alerts", False)
        ...
        self.honeypot_telnet_port = self.config.get("honeypot_telnet_port", 2323)
        self.honeypot_ftp_port = self.config.get("honeypot_ftp_port", 2121)
        ...
        self.tls_proxy_enabled = self.config.get("tls_proxy_enabled", False)
        self.tls_proxy_port = self.config.get("tls_proxy_port", 8443)
        self.ai_agent_enabled = self.config.get("ai_agent_enabled", True)
        self.ai_primary_model_name = self.config.get("ai_primary_model_name", "gpt2")
        self.kernel_telemetry_suppression_enabled = self.config.get("kernel_telemetry_suppression_enabled", False)
        self.ndis_interceptor_enabled = self.config.get("ndis_interceptor_enabled", False)
    ...
    def save(self):
        # Persist current runtime toggles back to YAML
        ...
        self.config["scan_min_seconds"] = self.scan_min_seconds
        self.config["scan_max_seconds"] = self.scan_max_seconds
        self.config["self_repair_mode"] = self.self_repair_mode
        self.config["watchdog_enabled"] = self.watchdog_enabled
        self.config["firewall_block_enabled"] = self.firewall_block_enabled
        self.config["firewall_rule_ttl_seconds"] = self.firewall_rule_ttl_seconds
        self.config["manual_override_enabled"] = self.manual_override_enabled
        self.config["rogue_ai_detection_enabled"] = self.rogue_ai_detection_enabled
        self.config["swarm_sync_enabled"] = self.swarm_sync_enabled
        self.config["swarm_endpoint"] = self.swarm_endpoint
        self.config["honeypot_enabled"] = self.honeypot_enabled
        self.config["tls_proxy_enabled"] = self.tls_proxy_enabled
        self.config["ai_agent_enabled"] = self.ai_agent_enabled
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
...
class GuardianDB:
    ...
    def _init_schema(self):
        with db_lock:
            ...
            add_column("events", "mitre_technique", "TEXT")
    ...
    def add_event(self, event: dict):
        with db_lock:
            ...
            ts = int(time.time())
            cur.execute("""
                INSERT INTO events (ts, type, src_ip, dst_ip, summary, severity, mitre_technique)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                ts,
                event.get("type", ""),
    ...
    def get_recent_events(self, limit=100):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("""
                SELECT ts, type, src_ip, dst_ip, summary, severity, mitre_technique
                FROM events
                ORDER BY ts DESC
                LIMIT ?
            """, (limit,))
            rows = cur.fetchall()
        events = []
        for ts, t, src, dst, summary, sev, mitre in rows:
            events.append({
                "ts": ts,
                "type": t,
                "src_ip": src,
...
class SuricataRuleEngine:
    """
    Enhanced Suricata-style rule engine (still simplified, but closer to v6-style).
    """
    def __init__(self, config: GuardianConfig):
        self.config = config
        self.rules = []
...
class ThreatMatrix:
    def __init__(self, db: GuardianDB):
        self.db = db
        self.ai_agent = None  # will be injected later

    def attach_ai_agent(self, agent):
        self.ai_agent = agent
    ...
    def add_event(self, event: dict):
        ...
        if self.ai_agent is not None:
            try:
                self.ai_agent.handle_event(event)
            except Exception as e:
                logger.error(f"[AI-Agent] Failed to handle event: {e}")
    ...
    def record_connection(self, src_ip: str):
        ...
        if self.connection_counts[key] > 50:
            event = {
                ...
                "severity": 70,
                "mitre_technique": "T1071"  # Application Layer Protocol (rough mapping)
            }
...
class DeepMLAnomalyDetector:
    """
    IsolationForest + optional simple PyTorch autoencoder (GPU if available).
    """
    def __init__(self, base: AnomalyDetector):
        ...
        self.device = DEFAULT_DEVICE if HAS_TORCH else "cpu"
        self.ae = None
    ...
    def _init_autoencoder(self):
        ...
        self.ae = SimpleAE().to(self.device)
    ...
    def record_event(self, event: dict):
        ...
        if self.iforest is not None and len(self.samples) > 50:
            try:
                ...
                logger.error(f"[DeepML] IsolationForest training failed: {e}")

        if self.use_torch and self.ae is not None and len(self.samples) > 50 and not self.ae_trained:
            try:
                x = torch.tensor(self.samples, dtype=torch.float32, device=self.device)
                ...
                logger.info("[DeepML] Autoencoder trained (GPU if available).")
            except Exception as e:
                logger.error(f"[DeepML] Autoencoder training failed: {e}")
    ...
    def is_deep_anomaly(self) -> bool:
        ...
        if self.iforest is not None and hasattr(self.iforest, "estimators_"):
            try:
                ...
                logger.error(f"[DeepML] IsolationForest prediction failed: {e}")

        if self.use_torch and self.ae is not None and self.ae_trained:
            try:
                x = torch.tensor([[rate]], dtype=torch.float32, device=self.device)
                recon = self.ae(x)
                loss = ((recon - x) ** 2).mean().item()
                return loss > 0.5
            except Exception as e:
                logger.error(f"[DeepML] Autoencoder prediction failed: {e}")
...
class SwarmSync:
    ...
    def _consensus_vote(self, local: set, remote: set) -> set:
        # Simple union; could be extended to Raft/Paxos-style majority voting
...
class AlertEngine:
    def __init__(self, config: GuardianConfig):
        self.config = config
        # Dedup + cooldown + burst suppression
        self.last_alerts = {}          # (ip, event_type) -> last_severity
        self.last_alert_ts = {}        # (ip, event_type) -> timestamp
        self.cooldown_seconds = 600    # per-IP + per-event-type cooldown
        self.burst_window = []         # timestamps of recent alerts
        self.burst_threshold = 50      # ignore if >50 alerts in 1s
        self.first_alert_only = False  # if True, only first alert per (ip,event_type)
        self.severity_increase_only = True  # only alert if severity increases
...
class ForkliftExecutor:
    """
    Minimal executor stub that wraps nn.Linear calls and collects stats.
    """
    def __init__(self):
        self.calls = 0
        self.router_data = {}
    ...
    def linear(self, layer_name, weight, bias, x, layer_depth):
        self.calls += 1
        # Simple pass-through; could add routing logic here
        return nn.functional.linear(x, weight, bias)

    def stats(self):
        return {
            "linear_calls": self.calls,
...
def load_model(model_name: str = PRIMARY_MODEL_NAME):
    ...
    if not HAS_TORCH:
        logger.warning("[Node] Torch not available, using TinyFallback.")
    ...
    logger.info(f"[Node] Loading model: {model_name}")
    try:
        ...
        IS_FALLBACK_MODEL = False
        logger.info(f"[Node] Loaded HF model: {model_name}")
    except Exception as e:
        logger.error(f"[Node] Failed to load {model_name}, falling back to TinyFallback: {e}")
        try:
        ...
        CURRENT_MODEL_NAME = "TinyFallback"
        IS_FALLBACK_MODEL = True
        logger.info("[Node] Using TinyFallback model.")

@torch.inference_mode()
def generate_text(prompt: str, max_new_tokens: int = 128) -> tuple[str, dict]:
    load_model()  # ensure model/tokenizer are ready
    ...
    if tok is None:
        # TinyFallback path: just echo prompt
        return prompt, {
            "model_name": CURRENT_MODEL_NAME,
            "is_fallback": IS_FALLBACK_MODEL,
            "latency_ms": 0.0,
            "linear_calls": EXECUTOR.calls
        }
...
class MITREMapper:
    """
    Very simple MITRE ATT&CK mapper based on event type / summary keywords.
    """
    def map_event(self, event: dict) -> str:
        etype = event.get("type", "")
        summary = event.get("summary", "").lower()
        if "honeypot" in summary or "ssh" in summary:
            return "T1110"  # Brute Force
        if "intrusion" in summary or "suspicious" in summary:
            return "T1071"  # Application Layer Protocol
        if "rogue ai" in summary:
            return "T1059"  # Command and Scripting Interpreter (rough)
        if "deep anomaly" in summary:
            return "T1046"  # Network Service Scanning
        return ""

class AIDefenderAgent:
    """
    Agentic reasoning + threat triage using the Forklift-wrapped HF model.
    """
    def __init__(self, config: GuardianConfig, firewall: FirewallManager, db: GuardianDB, mitre: MITREMapper):
        self.config = config
    ...
    def handle_event(self, event: dict):
        if not self.config.ai_agent_enabled:
            return

        mitre_id = self.mitre.map_event(event)
        if mitre_id:
            event["mitre_technique"] = mitre_id
            self.db.add_event(event)  # update with MITRE mapping

        prompt = (
            "You are an autonomous AI defender agent.\n"
            "You see the following network security event:\n"
            f"Type: {event.get('type')}\n"
            f"Source IP: {event.get('src_ip')}\n"
            f"Destination IP: {event.get('dst_ip')}\n"
            f"Summary: {event.get('summary')}\n"
            f"Severity: {event.get('severity')}\n"
            f"MITRE Technique: {event.get('mitre_technique', '')}\n\n"
            "Decide if the source IP should be blocked, monitored, or allowed.\n"
            "Respond with a short JSON: {\"action\": \"block|monitor|allow\", \"reason\": \"...\"}."
        )

        try:
            text, stats = generate_text(prompt, max_new_tokens=128)
            logger.info(f"[AI-Agent] Model stats: {stats}")
            action = "monitor"
            reason = "model response not parsed"
            ...
            try:
                ...
                if start != -1 and end != -1 and end > start:
                    ...
                    action = obj.get("action", "monitor")
                    reason = obj.get("reason", reason)
            except Exception as e:
                logger.error(f"[AI-Agent] Failed to parse model JSON: {e}")

            src_ip = event.get("src_ip")
            if src_ip:
                if action == "block":
                    logger.info(f"[AI-Agent] Autonomous remediation: BLOCK {src_ip} ({reason})")
                    self.firewall.block_ip(src_ip, manual=True)
                    self.db.set_ip_policy(src_ip, POLICY_BLOCK)
                elif action == "allow":
                    logger.info(f"[AI-Agent] Autonomous remediation: ALLOW {src_ip} ({reason})")
                    self.firewall.unblock_ip(src_ip)
                    self.db.set_ip_policy(src_ip, POLICY_ALLOW)
                else:
                    logger.info(f"[AI-Agent] Autonomous remediation: MONITOR {src_ip} ({reason})")
                    self.db.set_ip_policy(src_ip, POLICY_MONITOR)
        except Exception as e:
            logger.error(f"[AI-Agent] Error during agentic reasoning: {e}")
...
class Honeypot(threading.Thread):
    def __init__(self, config: GuardianConfig, threat_matrix: ThreatMatrix, firewall: FirewallManager, db: GuardianDB, sig_engine: SignatureEngine, alerts: AlertEngine):
        super().__init__(daemon=True)
        self.config = config
        self.threat_matrix = threat_matrix
        self.firewall = firewall
        ...
        while self.running:
            for s, proto, port in list(self.sockets):
                try:
                    ...
                    try:
                        if proto == "SSH":
                            banner = b"SSH-2.0-OpenSSH_8.9p1 CowrieGuardian\r\n"
                        elif proto == "HTTP":
                            banner = b"HTTP/1.1 200 OK\r\nServer: GuardianHoneypot\r\nContent-Length: 0\r\n\r\n"
                        elif proto == "RDP":
                            banner = b"RDP-Guardian-Stub\r\n"
                        elif proto == "TELNET":
                            banner = b"Debian GNU/Linux 11 CowrieGuardian tty1\r\nlogin: "
                        else:  # FTP
                            banner = b"220 Guardian FTP Honeypot ready\r\n"
                        conn.sendall(banner)
                    except Exception:
                        pass
...
class GuardianGUI:
    ...
    def launch(self):
        ...
        title = QtWidgets.QLabel("Unified Network Guardian (Suricata + Zero-Trust Mesh + AI Defender)")
        ...
        # Threat events + controls on top
        layout.addWidget(QtWidgets.QLabel("Threat Events (top priority):"))
...
def main():
    logger.info("[Guardian] Starting Unified Network Guardian (Option A FULL INTEGRATION)...")
    ...
    ai_agent = AIDefenderAgent(config, firewall, db, mitre_mapper)
    threat_matrix.attach_ai_agent(ai_agent)
