...
# - Firewall auto-blocking with expiration + manual override
# - Threat intelligence feeds (API-ready stubs)
# - Device fingerprinting + basic OS hints
# - Rogue AI detection (behavioral heuristics)
# - Deep-style ML anomaly detection (statistical + model stub)
# - Honeypot protocol emulation (fake SSH banner)
# - Swarm sync with encrypted payload (Fernet)
...
#   * Live event-rate chart (if QtCharts available)
...
class GuardianDB:
    ...
    def upsert_device(self, ip: str, role: str, dev_type: str, risk: int, os_hint: str = ""):
        cur = self.conn.cursor()
        cur.execute("""
            INSERT INTO devices (ip, role, type, risk, os_hint)
            VALUES (?, ?, ?, ?, ?)
        ...
        """, (ip, role, dev_type, risk, os_hint))
        self.conn.commit()

    def get_devices(self):
        cur = self.conn.cursor()
        cur.execute("SELECT ip, role, type, risk, os_hint FROM devices")
        rows = cur.fetchall()
        devices = {}
        for ip, role, dev_type, risk, os_hint in rows:
            devices[ip] = {
                "role": role,
                "type": dev_type,
                "risk": risk,
    ...
    def add_firewall_rule(self, name: str, ip: str):
        cur = self.conn.cursor()
        cur.execute("""
            INSERT INTO firewall_rules (name, ip, created_ts)
            VALUES (?, ?, ?)
        """, (name, ip, int(time.time())))
        self.conn.commit()

    def get_firewall_rules(self):
        cur = self.conn.cursor()
        cur.execute("SELECT name, ip, created_ts FROM firewall_rules")
        rows = cur.fetchall()
        rules = []
        for name, ip, ts in rows:
            rules.append({"name": name, "ip": ip, "created_ts": ts})
        return rules

    def delete_firewall_rule(self, name: str):
        cur = self.conn.cursor()
        cur.execute("DELETE FROM firewall_rules WHERE name = ?", (name,))
        self.conn.commit()
...
class NetworkDiscoveryEngine:
        ...
        try:
            ...
            for c in conns:
                if c.raddr and c.raddr.ip:
                    ...
                    self.db.upsert_device(ip, role, dev_type, risk, os_hint)
        except Exception as e:
    ...
    def mark_critical_node(self):
        ...
        if CRITICAL_NODE_IP:
            self.db.upsert_device(CRITICAL_NODE_IP, "CRITICAL_NODE", "pc", 0, "Windows (local)")
...
class FirewallManager:
    ...
    def block_ip(self, ip: str):
        ...
        rule_name = f"GuardianBlock_{ip}_{int(time.time())}"
        try:
            cmd = [
            ...
            self.db.add_firewall_rule(rule_name, ip)
        except Exception as e:
    ...
    def cleanup_expired_rules(self):
        ...
        for r in rules:
            if now - r["created_ts"] > ttl:
                logger.info(f"[Firewall] Expiring rule {r['name']} for IP {r['ip']}")
                try:
                    cmd = ["netsh", "advfirewall", "firewall", "delete", "rule", f"name={r['name']}"]
                    subprocess.run(cmd, shell=True, check=False)
                except Exception as e:
                    logger.error(f"[Firewall] Failed to delete rule {r['name']}: {e}")
                self.db.delete_firewall_rule(r["name"])
...
class DeepAnomalyDetector:
    ...
    def record_event(self, event: dict):
        self.base.record_event(event)

    def is_deep_anomaly(self) -> bool:
        rate = self.base.get_rate()
        return rate > self.threshold_multiplier
...
class SwarmSync:
    ...
    def push_blocklist(self, blocklist: set):
        if not self.config.swarm_sync_enabled or not self.config.swarm_endpoint:
            self.last_status_ok = False
            return
        logger.info("[Swarm] Pushing blocklist to swarm (encrypted).")
        try:
            payload = {"blocklist": list(blocklist), "ts": int(time.time())}
            enc = self._encrypt_payload(payload)
            requests.post(self.config.swarm_endpoint, data=enc, timeout=3)
...
class PacketInspector(threading.Thread):
    def __init__(self, config: GuardianConfig, threat_intel: ThreatIntel,
                 ...
                 anomaly: AnomalyDetector, deep_anomaly: DeepAnomalyDetector):
        super().__init__(daemon=True)
        self.config = config
        ...
        self.deep_anomaly = deep_anomaly
        self.running = True
        ...
        try:
            if IP in pkt:
                ...
                if self.threat_intel.is_malicious(src):
                    ...
                    self.deep_anomaly.record_event(event)
        except Exception as e:
...
class GuardianService(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.deep_anomaly.is_deep_anomaly():
                event = {
                    "type": "deep_anomaly",
                    "src_ip": "",
                    "dst_ip": "",
                    "summary": "Deep anomaly detected (rate beyond threshold)."
                }
                self.threat_matrix.add_event(event)
...
class GuardianGUI:
    ...
    def launch(self):
        ...
        if QT_CHARTS_AVAILABLE:
            ...
            chart.setTitle("Event Rate Over Time")
        ...
        devices_box.setPlainText("\n".join(
            [f"{ip} - {info.get('role')} ({info.get('type')}) risk={info.get('risk')} os={info.get('os_hint')}"
    ...
    def _start_gui_timer(self, events_box, devices_box):
        ...
        def refresh():
            ...
            rate = self.deep_anomaly.get_rate()
            ...
            if self.swarm.config.swarm_sync_enabled:
                if self.swarm.last_status_ok:
                    self.swarm_label.setText("Swarm Sync: OK (encrypted)")
            ...
            if QT_CHARTS_AVAILABLE and self.series is not None:
...
def main():
    ...
    deep_anomaly = DeepAnomalyDetector(anomaly)
    ...
    if config.gui_enabled:
        gui = GuardianGUI(config, db, net_engine, threat_matrix, alerts,
                          swarm, honeypot, anomaly, deep_anomaly, firewall,
