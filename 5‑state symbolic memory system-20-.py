...
"""
BORG OS v19 – Gaming-Safe Unified Accelerator (CPU/GPU/NPU) + RL/DPI/Threat + Mesh + Command Center
+ One-Time Full Game Scan + Incremental New Game Detection + Manifest Watcher
+ HARD NEVER-KILL for borg_games and launchers

Key upgrades vs v18:
- Explicit NEVER-KILL rule for any process whose exe path is in borg_games.
- Stronger protection for Steam/Epic/Battle.net/Riot/Origin/launchers/clients/game EXEs.
- Extra logging when a process is spared because it is a known game or launcher.
"""
...
# ============================================================
#  OS GOVERNOR (GAMING-SAFE, HARD NEVER-KILL FOR borg_games)
# ============================================================
...
class BorgOSGovernor:
    def __init__(self, logger=None, storage=None, event_bus=None,
        ...
        # HARD NEVER-KILL cache of game exe paths
            ...
            for i, state in enumerate(states):
                ...
                for p in procs[start:end]:
                    try:
                        ...
                        # HARD NEVER-KILL: check exe path against borg_games
                        exe_path = None
                        try:
                            exe_path = p.exe()
                        except Exception:
    ...
    def _kill_process(self, p, score, gpu_load, hard_protected, learned_safe, sigs, gpu_mem, overloaded):
        try:
            ...
            # Extra safety: if somehow launcher-like slipped here, still never kill
...
if FASTAPI_AVAILABLE:
    app = FastAPI(title="Borg OS Node", version="19.0.0")
...
class BorgGUI:
    def __init__(self, root, core: BorgCore):
        ...
        root.title("BORG OS NODE v19 – Command Center (Accelerator + Threat + Deep RL + Mesh + DPI + Games)")
...
def run_cli():
    print("BORG OS NODE (CLI MODE, Autonomous v19, Accelerator + Threat + Deep RL + Mesh + DPI, Gaming-Safe + One-Time Scan + Incremental Detection + NEVER-KILL borg_games)")
    print(f"OS: {OS_NAME}")
