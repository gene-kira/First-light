...
# -*- coding: utf-8 -*-
"""
Codex Purge Shell – Unified Python File v5
...
- Threat-Adaptive Mode (self-defending identity firewall)
- Local Machine-Learning Pattern Detector (simple, strictly local)
- Persona Forge (realistic synthetic personas)
- Telemetry Mirage (synthetic usage/health/error events)
- Subsystem Quarantine (local sandbox for suspicious components)
- Cold-Boot Persona Resurrection (identity continuity from swarm)
...
def compute_boot_info(state_store: JsonStateStore, logger: SafeLogger) -> BootInfo:
    ...
    if first_boot:
        ...
        for key in ["persona", "identity_shadow", "ml_patterns", "quarantine"]:
...
class PersonaForge:
    """
    Generates more realistic synthetic personas:
    - believable activity patterns
    - believable sync states
    - believable device fingerprints
    - believable metadata
    - believable region profiles
    """
    def __init__(self):
        self.regions = ["US", "EU", "APAC", "LATAM", "MEA"]
        self.roles = ["workstation", "lab-node", "honeypot", "gaming-rig", "dev-box"]
...
# ============================================================
# Persona Engine (Lock + Swarm + Reboot-Safe Rotation + Resurrection)
# ============================================================
...
class PersonaEngine:
    """
    Manages fake personas:
    - Persona Lock (freeze identity)
    - Persona Swarm (multiple personas rotating)
    - Reboot-Safe Persona Rotation
    - Cold-Boot Persona Resurrection
    """
    ...
    def _load_or_create_persona(self):
        ...
        if data:
            ...
            self.logger.log("Generated new fake persona via Persona Forge.")
    ...
    def rotate_persona(self):
        with self._lock:
            ...
            self.current_persona = self._generate_new_persona()
            self._persist_persona()
            self.logger.log("Persona rotated via Persona Forge.")
    ...
    def toggle_swarm(self):
        with self._lock:
            ...
            if self.swarm_enabled and not self.swarm_personas:
                self.swarm_personas = [self._generate_new_persona() for _ in range(4)]
                self.logger.log("Persona Swarm initialized with 4 forged personas.")
    ...
    def apply_boot_policies(self, boot_info: BootInfo):
        """
        Reboot-Safe Persona Rotation + Cold-Boot Persona Resurrection:
        - On first boot: keep persona but log.
        - On subsequent boots: resurrect from swarm if available, else rotate.
        """
        with self._lock:
            if boot_info.first_boot:
                ...
                if self.swarm_enabled and self.swarm_personas:
                    ...
                    self.logger.log(
                        f"[BootPolicy] Cold-Boot Persona Resurrection: resurrecting [{resurrected.id}] from swarm."
                    )
                    self.current_persona = resurrected
                    self._persist_persona()
                else:
                    if not self.persona_locked:
                        self.logger.log("[BootPolicy] Reboot detected: rotating persona safely via forge.")
                        self.current_persona = self._generate_new_persona()
                        self._persist_persona()
                    else:
...
# ============================================================
# IdentityCRL Shadow Layer (Kill Switch + Auto-Kill + Stealth + Leak Detector + Lockdown)
# ============================================================
...
# ============================================================
# Local ML Pattern Detector (Simple, Strictly Local)
# ============================================================
...
class PatternDetector:
    """
    Simple local pattern detector:
    - Tracks identity access frequency
    - Detects spikes
    - Flags suspicious subsystems
    """
    def __init__(self, state_store: JsonStateStore, logger: SafeLogger):
        self.state_store = state_store
        self.logger = logger
    ...
    def record_access(self, subsystem: str):
        with self._lock:
            ...
            stats.last_access_ts = time.time()
            self._persist_state()
    ...
    def is_suspicious(self, subsystem: str) -> bool:
        with self._lock:
            ...
            if not stats:
                return False
            if stats.leak_count >= 2 or stats.access_count >= 20:
                return True
            return False
...
class SubsystemQuarantine:
    """
    Local quarantine for suspicious subsystems:
    - If subsystem repeatedly tries to use real identity, it is quarantined.
    - Identity calls are redirected.
    - Only fake data is returned.
    """
    def __init__(self, state_store: JsonStateStore, logger: SafeLogger):
        self.state_store = state_store
        self.logger = logger
    ...
    def quarantine(self, subsystem: str):
        with self._lock:
            ...
            self.logger.log(f"[Quarantine] Subsystem [{subsystem}] quarantined.")
            self._persist_state()
...
# ============================================================
# Telemetry Mirage (Synthetic Usage / Health / Errors)
# ============================================================
...
class TelemetryMirage:
    ...
    def generate_mirage_events(self, subsystem: str, persona: FakePersona):
        events = []
        for _ in range(3):
            ...
            self.logger.log(f"[Mirage] Synthetic {event_type} event for [{subsystem}] persona [{persona.id}].")
        return events
...
class ThreatAdaptiveController:
    ...
    - Telemetry spike -> auto-stealth
    - Identity requests spike -> auto-swarm
    - Windows sync-like subsystem -> auto-kill
    - Repeated identity access -> auto-flood + quarantine
    """
    def __init__(
        self,
        logger: SafeLogger,
    ...
    def _evaluate(self, subsystem: str):
        snapshot = self.pattern_detector.get_snapshot().get(subsystem, {})
        access_count = snapshot.get("access_count", 0)
        leak_count = snapshot.get("leak_count", 0)
        ...
        if self.telemetry_counter >= 50:
            self.logger.log("[ThreatAdaptive] Telemetry spike detected -> auto-stealth.")
            self.identity_shadow.toggle_stealth_mode()
            self.telemetry_counter = 0

        if access_count >= 30:
            if not self.persona_engine.get_swarm_state():
                self.logger.log("[ThreatAdaptive] Identity request spike -> auto-swarm.")
        ...
        if leak_count >= 2:
            if not self.telemetry_engine.flooder_enabled:
                self.logger.log("[ThreatAdaptive] Repeated leak -> auto-flood.")
...
# ============================================================
# Fake Telemetry Engine (Flooder + Glyphs + Mirage + Threat-Adaptive)
# ============================================================
...
class FakeTelemetryEngine:
    ...
    def _handle_identity_access(self, event: Dict[str, Any]):
        ...
        self.mirage.generate_mirage_events(subsystem, persona)
...
class CodexGUI:
    ...
    def __init__(
        ...
        self.root.title("Codex Purge Shell – Identity Shadow Layer v5")
    ...
    def _build_layout(self):
        ...
        self.shadow_info_text = tk.Text(self.shadow_frame, height=7, width=140)
        ...
        self.persona_info_text = tk.Text(self.persona_frame, height=8, width=140)
        ...
        self.telemetry_text = tk.Text(self.telemetry_frame, height=15, width=140)
        ...
        # Pattern / Quarantine Panel
        self.pattern_frame = ttk.LabelFrame(self.main_frame, text="Pattern Detector & Quarantine")
        self.pattern_frame.pack(fill=tk.X, padx=5, pady=5)

        self.pattern_text = tk.Text(self.pattern_frame, height=6, width=140)
        self.pattern_text.grid(row=0, column=0, columnspan=2, padx=5, pady=5)

        self.quarantine_text = tk.Text(self.pattern_frame, height=4, width=140)
...
def main():
    ...
    telemetry_engine = FakeTelemetryEngine(
        persona_engine, identity_shadow, logger, pattern_detector, quarantine, mirage
    )

    threat_controller = ThreatAdaptiveController(
        logger, identity_shadow, persona_engine, telemetry_engine, pattern_detector, quarantine
    )
    ...
    root = tk.Tk()
    gui = CodexGUI(root, logger, identity_shadow, persona_engine, telemetry_engine, pattern_detector, quarantine)
