#!/usr/bin/env python3
"""
Beast Swarm Engine v8.2 — Command Center + Swarm Scheduler + REST + VRAM Visualizer + Auto-Spawn
...
- REST API for metrics + job submission (HTTP JSON)
"""
...
class BeastSwarm:
    ...
    def submit_plugin(self, plugin_name, job_name, *args, priority=0, name=None, **kwargs):
        # plugin job is resolved at node level; here we just pass identifiers
        def plugin_wrapper(plugin_name, job_name, *args, **kwargs):
            # placeholder; actual resolution happens in node.submit_plugin
...
# ---------------------------------------------------------------------------
# GUI — Command Center with VRAM fragmentation visualizer
# ---------------------------------------------------------------------------
...
class SwarmGUI(QtWidgets.QMainWindow):
    def __init__(self, swarm: BeastSwarm, queen: Queen,
        ...
        self.setWindowTitle("Beast Swarm Engine v8.2 — Command Center")
        ...
        for b in [self.btn_overview, self.btn_nodes, self.btn_gpu,
                  self.btn_vram, self.btn_queen, self.btn_zmq, self.btn_plugins]:
            b.setCheckable(True)
        ...
        self.stack.addWidget(self.overview_panel)  # index 0
        self.stack.addWidget(self.nodes_panel)     # index 1
        self.stack.addWidget(self.gpu_panel)       # index 2
        self.stack.addWidget(self.vram_panel)      # index 3
        self.stack.addWidget(self.queen_panel)     # index 4
        self.stack.addWidget(self.zmq_panel)       # index 5
        self.stack.addWidget(self.plugins_panel)   # index 6
    ...
    def _build_plugins_panel(self):
        ...
        layout.addWidget(QtWidgets.QLabel("Loaded plugins"))
    ...
    def run_selected_plugin_job(self):
        ...
        # Swarm-level plugin submission uses global scheduler
    ...
    def refresh_all(self):
        ...
        if mem_vals:
            self.global_mem_bar.setValue(int(sum(mem_vals) / len(mem_vals)))
        if disk_vals:
            self.global_disk_bar.setValue(int(sum(disk_vals) / len(disk_vals)))
...
def build_local_swarm():
    ...
    auto_spawner = AutoSpawner(nodes, queen, memory, gpu_telemetry, gpu_governor, plugin_manager, scheduler)
    zmq_swarm = ZMQSwarm(swarm, CONFIG["zmq_bind"]) if zmq is not None else None
    rest_api = SwarmREST(swarm, queen, memory) if Flask is not None else None

    return swarm, queen, gpu_governor, tuner, zmq_swarm, memory, plugin_manager, auto_spawner, scheduler, rest_api


def main():
    swarm, queen, gpu_governor, tuner, zmq_swarm, memory, plugin_manager, auto_spawner, scheduler, rest_api = build_local_swarm()
    ...
    gui = SwarmGUI(swarm, queen, gpu_governor, zmq_swarm, plugin_manager, memory)
    gui.show()
