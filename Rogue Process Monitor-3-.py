...
# ============================================================
# Rogue Process Monitor v13.1 (SAFE, NON-DESTRUCTIVE)
# - Full control sandbox (GUI fully isolated from heavy scanning)
# - Behavior engine + reputation scoring in sandbox
# - Threat meter fed by sandbox summaries
# - Live processes, history, tree, alerts all buffered
# ============================================================

LOG_FILE = "rogue_monitor_v13_1_log.txt"
...
# -------------------------
# Sandbox shared state (GUI reads only)
# -------------------------
...
sandbox_threat_level = 0      # int 0-100
...
def sandbox_pop_alerts():
    global sandbox_alerts
    with sandbox_lock:
        alerts = list(sandbox_alerts)
        sandbox_alerts.clear()
        return alerts

def sandbox_set_threat_level(level):
    global sandbox_threat_level
    with sandbox_lock:
        sandbox_threat_level = int(max(0, min(100, level)))

def sandbox_get_threat_level():
    with sandbox_lock:
        return sandbox_threat_level

# -------------------------
# Behavior engine / scoring (sandbox)
# -------------------------
...
# -------------------------
# Sandbox thread (all heavy work lives here)
# -------------------------
...
class SandboxThread:
    ...
    def loop(self):
        log("Sandbox thread v13.1 started (SAFE, non-destructive)")
        while self.running:
            try:
    ...
    def scan_and_update(self):
        ...
        # --- Process tree (simplified, cycle-proof) ---
        self.build_tree()
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v13.1 (SAFE, Sandbox)")
        self.root.geometry("1100x750")

        self._build_ui()

        # Start sandbox
        self.sandbox = SandboxThread()

        # Start lightweight GUI refresh loops
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v13.1 (SAFE, Sandbox)", font=("Segoe UI", 14, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))

        desc = ttk.Label(
            frame,
            text=(
                "All heavy scanning runs in a separate sandbox thread.\n"
                "GUI only reads summaries, so it stays responsive and safe.\n"
    ...
    def _build_live_tab(self):
        ...
        desc = ttk.Label(
            ...
            text=(
                ...
                "GUI does not scan directly; it only reads buffered data."
            ),
            justify="left"
        )
        desc.grid(row=1, column=0, sticky="w", pady=(0, 10))
    ...
    def _build_tree_tab(self):
        ...
        desc = ttk.Label(
            frame,
            text=(
                "Tree is built inside the sandbox and streamed here as text.\n"
    ...
    def _schedule_live_refresh(self):
        ...
        for item in items:
            ...
            path = item["path"]
            score = item["score"]
            rogue = "YES" if item["rogue"] else "NO"
    ...
    def _schedule_alert_refresh(self):
        ...
        if alerts:
            ...
            # Popup per alert (optional, can be noisy)
            for al in alerts:
    ...
    def _show_alert_popup(self, al):
        ...
        # Run popup in main thread via after
...
def main():
    log("Rogue Process Monitor v13.1 (SAFE, Sandbox) starting")
    root = tk.Tk()
