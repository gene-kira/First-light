...
- True 24/7 daemon behavior: immediate restart after each cycle, no 30s delay.
...
- Safe stop and restart controls.
...
class TrainingMonitor(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # No restart timer: immediate restart in on_finished when CONTINUOUS_MODE is True.
    ...
    def on_finished(self):
        ...
        if CONTINUOUS_MODE:
            ...
            self.status_label.setText("Status: Restarting immediately...")
            self.append_log("Worker finished. Restarting training immediately.")
            self.start_training()
        else:
