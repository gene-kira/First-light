...
# Security Bridge + Audio Lockdown + Full Kill + Reset
# FULL GAME IMMUNITY PATCH (Maximum Protection)
...
def ensure_libs():
    ...
    globals()["pefile"] = importlib_import_module("pefile") if False else importlib.import_module("pefile")
...
# Initialize new audio-lockdown-related state
...
def assess_risk(name, pid, volume, muted, connections, file_suspicious, mem_suspicious):
    ...
    # If user explicitly protected this process, treat as benign and never re-hunt
    ...
    if is_allowlisted(name_lower):
        ...
        rse_verdict = "None"
        rse_score = 0
        reasons.append("Process is allowlisted (FULL GAME IMMUNITY / Teams / media).")
        update_trust_score(proc_key, "benign")
    else:
        if score >= 80:
            risk_level = "High"
...
def auto_block_if_needed(info: ProcessSecurityInfo, gui_confirm_callback=None):
    ...
    # Skip auto-block for user-protected processes
    if proc_key in STATE.get("user_protected", []):
        return

    # Skip auto-block for confirmed audio intruder after it has been killed once
...
def audio_lockdown_trace():
    """
    Trace from speaker backward to find the loudest non-allowlisted audio source,
    build an intruder chain, and return candidate info.
    """
    audio_sessions = get_audio_sessions()
    if not audio_sessions:
        return None, None

    # Find loudest non-allowlisted session
    best = None
    for pid, name, volume, muted in audio_sessions:
        if pid is None:
            continue
...
def confirm_audio_intruder(chain, user_confirm: bool):
    """
    After user confirmation, either protect or kill the intruder,
    and mark state so it doesn't keep coming back.
    """
    pid = chain["pid"]
    name = chain["name"]
    name_lower = (name or "").lower()
    ...
    if not user_confirm:
        # User said NO: protect this process permanently
    ...
    # User confirmed intruder: kill once, mark as confirmed, and don't re-hunt
    try:
        kill_process(pid)
    except Exception as e:
...
# -----------------------------
# GUI (Codex Control Console + Security Bridge)
# -----------------------------
class SecurityBridgeGUI:
    def __init__(self, root):
        self.root = root

        # Auto-scale window similar to Codex Control Console layout
        screen_w = self.root.winfo_screenwidth()
        screen_h = self.root.winfo_screenheight()
        target_w = max(1200, int(screen_w * 0.92))
        target_h = max(800, int(screen_h * 0.80))
        self.root.geometry(f"{target_w}x{target_h}+50+50")

        self.root.title("Codex Control Console v13 — Security Bridge (Audio Lockdown + Full Kill + Reset)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            top_frame,
            text="Codex Control Console v13 — Security Bridge Operator Mode",
        ...
        # AUDIO LOCKDOWN BUTTON (green -> red, stays red until reset)
        ...
        self.audio_lockdown_button.pack(side=tk.RIGHT, padx=10)

        # FULL KILL LOCKDOWN BUTTON
        ttk.Button(
            top_frame,
        ...
        # MAIN NOTEBOOK (Codex-style main area)
    ...
    def audio_lockdown_gui(self):
        """
        AUDIO LOCKDOWN:
        - Button turns red and stays red until RESET
        - Trace from speaker backward
        - Show intruder + problem
        - Ask for confirmation
        - Auto-kill intruder once confirmed
        - After one more scan, stop re-hunting that intruder
        """
        # Mark audio-lockdown active and set button red
    ...
    def reset_system_gui(self):
        ...
        # Reset audio-lockdown button to green
        self.audio_lockdown_button.configure(
            text="AUDIO LOCKDOWN — READY",
            style="AudioLockdownGreen.TButton"
        )
...
# -----------------------------
def main():
    log_event("INFO", "Codex Control Console v13 — Security Bridge (Full Upgrade) started.")
    start_honeypot()
    root = tk.Tk()
    ...
    root.mainloop()
    log_event("INFO", "Codex Control Console v13 — Security Bridge closed.")
