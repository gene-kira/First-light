...
- Plugin system
- GUI dashboard with threat matrix, persona panel, animated overlays, VRAM entropy graph
- Right-click context menu for overrides, threat assignment, tagging, export, correlated view
...
class SecurityConfig:
    def __init__(self):
        ...
        # Per-profile modes
        # "gaming", "browsing", "high_security"
        self.profile_mode = "browsing"

        self.enable_auto_terminate = False
        self.enable_quarantine = True  # safe, non-destructive
    ...
    def apply_profile(self):
        """
        Adjust thresholds and response behavior based on profile_mode.
        """
        mode = self.profile_mode
        if mode == "gaming":
            # More tolerant, fewer incidents, semi AI
            ...
            # Aggressive detection, full auto
...
# ============================================================================
# Borg memory + overrides + threat labels + tags
# ============================================================================
...
class BorgMemory:
    ...
    def set_threat_label(self, fingerprint: str, label: Optional[str]):
        ...
        with self.lock, self._connect() as conn:
            if label is None:
                conn.execute(
                    "DELETE FROM incident_threat_labels WHERE fingerprint=?",
                    (fingerprint,),
                )
            else:
                conn.execute(
    ...
    def set_tags(self, fingerprint: str, tags: list[str]):
        ...
        with self.lock, self._connect() as conn:
            if not tags_str:
                conn.execute(
                    "DELETE FROM incident_tags WHERE fingerprint=?",
                    (fingerprint,),
                )
            else:
                conn.execute(
...
# ============================================================================
# Response engine with modes + auto-quarantine
# ============================================================================
...
class SecurityGUI:
    def __init__(self, cfg: SecurityConfig):
        ...
        self.profile_mode_var = tk.StringVar(value=self.cfg.profile_mode)
        self._build()
    ...
    def _timeline_tab(self, parent):
        ...
        ttk.Button(
            controls, text="Replay Timeline",
            command=self._replay_timeline
        ).pack(side=tk.LEFT, padx=4)
    ...
    def _settings_tab(self, parent):
        ...
        ttk.Radiobutton(
            ...
            command=self._apply_profile_mode,
        ).pack(anchor="w")
        ttk.Radiobutton(
            profile_frame,
            text="Browsing",
    ...
    def _about_tab(self, parent):
        text = (
            ...
            "Threat assignment via right-click lets you tell the AI which bucket an incident\n"
            "belongs to (process, network, file, GPU, etc.), feeding the threat matrix.\n\n"
            "Incident tagging and timeline replay let you build your own narrative of what\n"
            "happened over time, with AI explanations for why each event is suspicious."
        )
    ...
    def _open_incident_details(self, event=None):
        ...
        if ai_expl:
            ttk.Label(top_frame, text=f"AI Explanation: {ai_expl}", font=("Segoe UI", 10), wraplength=800).pack(anchor="w", pady=2)
