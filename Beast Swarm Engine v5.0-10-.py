#!/usr/bin/env python3
"""
Beast Swarm Engine v8.4 — Multi-Host Mesh + Consensus + DFS + GPU Control + Limits + Cancellation + Multi-Host GUI
...
- Multi-host swarm networking:
    - TCP JSON server (backward compatible)
    - Encrypted peer channels (TLS-lite wrapper)
    - ZeroMQ mesh (optional) for broadcast / fan-out
- Queen consensus engine:
    - Local risk aggregation
    - Risk decay + anomaly detection
    - Gossip-style risk sharing between hosts (eventual consistency)
- Distributed filesystem (DFS):
    - Shared storage namespace
    - Chunk replication metadata
    - Simple distributed caching abstraction
- GPU governor:
    - NVML telemetry
    - Soft policy (load/temp/memory)
    - Hooks for fan/clock/power limit (read-only suggestions; actual control gated)
- Plugin system:
    - Sandbox subprocess execution
    - Resource limits (CPU time, wall-clock timeout)
    - Optional memory limit hints
- Job system:
    - Job IDs
    - Cancellation signals
    - Preemption hooks (cooperative)
...
- GUI Command Center:
    ...
    - Real-time charts (CPU, memory, disk, VRAM fragmentation)
...
CONFIG = {
    ...
    "dfs_root": "dfs_root",
    "dfs_replication_factor": 2,
    "plugin_wall_timeout_sec": 60.0,
    "plugin_cpu_hint_sec": 30.0,
    "job_cancel_check_interval_sec": 0.2,
    "tls_enable": False,
    "tls_certfile": None,
    "tls_keyfile": None,
}
...
class JobHistory:
    ...
    def record(self, job_id, node_id, task_type, name, status, duration, info=None):
        with self.lock:
            conn = sqlite3.connect(self.path)
            try:
                c = conn.cursor()
                c.execute(
                    "INSERT INTO jobs (job_id, timestamp, node_id, task_type, name, status, duration, info) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (job_id, time.time(), node_id, task_type, name, status, duration, json.dumps(info or {})),
                )
                conn.commit()
            finally:
                conn.close()
    ...
    def recent(self, limit=100):
        with self.lock:
            ...
            try:
                c = conn.cursor()
                c.execute(
                    "SELECT job_id, timestamp, node_id, task_type, name, status, duration, info "
                ...
                conn.close()
        out = []
        for job_id, ts, node_id, ttype, name, status, dur, info in rows:
            try:
...
class MemoryManager:
    ...
    def update_dfs_metadata(self, meta):
        self.state["dfs_metadata"] = meta

    def get_dfs_metadata(self):
        return self.state.get("dfs_metadata", {})

    def update_peer_health(self, peer_health):
        self.state["peer_health"] = peer_health

    def get_peer_health(self):
        return self.state.get("peer_health", {})
...
class Task:
    ...
    def __post_init__(self):
        ...
        if not self.job_id:
            self.job_id = f"{self.name}-{int(time.time()*1000)}-{next(_JOB_COUNTER)}"
...
# ---------------------------------------------------------------------------
# Queen consensus engine with risk decay + anomaly detection + gossip
# ---------------------------------------------------------------------------

class Queen:
    def __init__(self, memory: MemoryManager = None, node_id: str = "local"):
        self.nodes = {}
        self.lock = threading.Lock()
        self.memory = memory
        self.risk_state = {}
    ...
    def gossip_import(self, payload):
        remote_risk = payload.get("risk_state", {})
        now = time.time()
        with self.lock:
            for entity, rs_remote in remote_risk.items():
                rs_local = self.risk_state.get(entity, {
                    "score": 0.0,
                    "last_update": now,
                    "history": [],
                })
                # simple merge: average scores, merge history
                s_local = rs_local["score"]
                s_remote = rs_remote.get("score", 0.0)
                rs_local["score"] = (s_local + s_remote) / 2.0
                rs_local["last_update"] = now
                hist_local = rs_local.get("history", [])
                hist_remote = rs_remote.get("history", [])
                merged = (hist_local + hist_remote)[-self.anomaly_window:]
                rs_local["history"] = merged
                self.risk_state[entity] = rs_local
        if self.memory:
            self.memory.update_queen_risk_state(self.risk_state)
...
# ---------------------------------------------------------------------------
# GPU telemetry + governor hooks + control suggestions
# ---------------------------------------------------------------------------
    ...
    def _compute_policy(self, snapshot):
        ...
        for dev in snapshot.get("devices", []):
            ...
            policy = {
                ...
                "suggest_fan_up": False,
                "suggest_clock_down": False,
                "suggest_power_limit_down": False,
            }

            if temp > target_temp + 5 or util > target_util + 15:
                policy["suggest_reduce_load"] = True
                policy["suggest_fan_up"] = True
                policy["suggest_clock_down"] = True
                policy["suggest_power_limit_down"] = True
...
def vram_bandwidth_job(size_mb=None, device_index=0, cancel_reg: Optional[CancellationRegistry] = None, job_id: str = ""):
    ...
    try:
        ...
        if cancel_reg and cancel_reg.is_cancelled(job_id):
...
def vram_fragmentation_stress_job(chunks=None, device_index=0, cancel_reg: Optional[CancellationRegistry] = None, job_id: str = ""):
    if torch is None or not torch.cuda.is_available():
        return {"error": "torch CUDA not available"}
...
# ---------------------------------------------------------------------------
# Distributed filesystem (DFS) abstraction
# ---------------------------------------------------------------------------

class DistributedFS:
    """
    Very simple DFS abstraction:
    - Local root directory
    - Metadata about chunks and replication
    - No actual remote replication (metadata only), but ready for extension
    """
    def __init__(self, root: str, memory: MemoryManager):
        self.root = root
        self.memory = memory
        os.makedirs(self.root, exist_ok=True)
        self.lock = threading.Lock()
        self.meta = self.memory.get_dfs_metadata()

    def _save_meta(self):
        self.memory.update_dfs_metadata(self.meta)
        self.memory.save()
...
# ---------------------------------------------------------------------------
# Plugin system (sandboxed + resource limits)
# ---------------------------------------------------------------------------
...
class PluginManager:
    ...
    def sandbox_run(self, plugin_name, job_name, *args, **kwargs):
        ...
        try:
            ...
            wall_timeout = CONFIG["plugin_wall_timeout_sec"]
            out, err = proc.communicate(json.dumps(payload), timeout=wall_timeout)
...
class BeastNode:
    def __init__(self,
                 ...
                 cancel_registry: CancellationRegistry = None):
        self.node_id = node_id
        ...
        self.cancel_registry = cancel_registry or CancellationRegistry()
    ...
    async def _async_task_wrapper(self, task: Task):
        ...
        try:
            ...
            if self.job_history:
                self.job_history.record(task.job_id, self.node_id, TaskType.ASYNC, task.name, status, duration, info)
    ...
    def submit_vram_bandwidth(self, size_mb=None, device_index=0, priority=0, name=None):
        if not self.enable_gpu:
            ...
            return None
        def wrapper(size_mb, device_index, job_id):
            return vram_bandwidth_job(size_mb, device_index, self.cancel_registry, job_id)
        task = Task(wrapper, (size_mb, device_index, ""), {}, task_type=TaskType.VRAM,
                    priority=priority, name=name or "vram_bandwidth")
    ...
    def submit_vram_fragmentation(self, chunks=None, device_index=0, priority=0, name=None):
        if not self.enable_gpu:
            print(f"[{self.node_id}][VRAM] GPU disabled/unavailable.")
            return None
        def wrapper(chunks, device_index, job_id):
            return vram_fragmentation_stress_job(chunks, device_index, self.cancel_registry, job_id)
        task = Task(wrapper, (chunks, device_index, ""), {}, task_type=TaskType.VRAM,
                    priority=priority, name=name or "vram_fragmentation")
        self.vram_queue.put(task)
        return task.job_id
    ...
    def _run_task(self, task: Task):
        ...
        info = None
        try:
            if task.cancellable and self.cancel_registry.is_cancelled(task.job_id):
                status = "cancelled"
                info = {"cancelled": True}
                return {"cancelled": True}
            # inject job_id into wrappers that expect it
            if task.task_type == TaskType.VRAM and len(task.args) == 3:
                args = list(task.args)
                args[2] = task.job_id
                result = task.func(*args, **task.kwargs)
            else:
            ...
            if self.job_history:
                self.job_history.record(task.job_id, self.node_id, task.task_type, task.name, status, duration, info)
            self.cancel_registry.clear(task.job_id)
...
class BeastSwarm:
    def __init__(self, nodes, scheduler: GlobalScheduler, cancel_registry: CancellationRegistry):
        self.nodes = nodes
        self.scheduler = scheduler
        self.cancel_registry = cancel_registry

    def submit_cpu(self, func, *args, priority=0, name=None, **kwargs):
        return self.scheduler.submit(Task(func, args, kwargs, TaskType.CPU, priority, name))

    def submit_io(self, func, *args, priority=0, name=None, **kwargs):
        return self.scheduler.submit(Task(func, args, kwargs, TaskType.IO, priority, name))

    def submit_gpu(self, func, *args, priority=0, name=None, **kwargs):
        return self.scheduler.submit(Task(func, args, kwargs, TaskType.GPU, priority, name))

    def submit_npu(self, func, *args, priority=0, name=None, **kwargs):
        return self.scheduler.submit(Task(func, args, kwargs, TaskType.NPU, priority, name))

    def submit_live(self, name="live", priority=0):
        return self.scheduler.submit(Task(live_data_job, (name,), {}, TaskType.LIVE, priority, name))

    def submit_plugin(self, plugin_name, job_name, *args, priority=0, name=None, **kwargs):
        def plugin_wrapper(plugin_name, job_name, *args, **kwargs):
            return {"plugin": plugin_name, "job": job_name}
        return self.scheduler.submit(Task(plugin_wrapper, (plugin_name, job_name, *args), kwargs,
                                          TaskType.PLUGIN, priority, name or job_name))

    def submit_vram_bandwidth(self, size_mb=None, device_index=0, priority=0, name=None):
        return self.scheduler.submit(Task(vram_bandwidth_job, (size_mb, device_index), {},
                                          TaskType.VRAM, priority, name or "vram_bandwidth"))

    def submit_vram_fragmentation(self, chunks=None, device_index=0, priority=0, name=None):
        return self.scheduler.submit(Task(vram_fragmentation_stress_job, (chunks, device_index), {},
...
class AutoSpawner:
    def __init__(self, nodes, queen: Queen, memory: MemoryManager,
                 ...
                 job_history: JobHistory, cancel_registry: CancellationRegistry):
        self.nodes = nodes
        self.queen = queen
        self.memory = memory
        self.gpu_telemetry = gpu_telemetry
...
# ---------------------------------------------------------------------------
# Multi-host swarm networking (TCP JSON + TLS + gossip)
# ---------------------------------------------------------------------------

class MultiHostSwarm:
    """
    Multi-host swarm:
    - Listens on TCP for JSON messages: {"type": "...", ...}
    - Optional TLS
    - Forwards jobs into local BeastSwarm
    - Gossip risk state to peers
    """
    def __init__(self, swarm: BeastSwarm, queen: Queen, memory: MemoryManager):
        self.swarm = swarm
        self.queen = queen
        self.memory = memory
        self.stop_event = threading.Event()
        ...
        self.peer_health = {}
        self.gossip_thread = threading.Thread(target=self._gossip_loop, daemon=True, name="MultiHostGossip")
        self.gossip_thread.start()

    def _wrap_tls(self, sock: socket.socket, server_side: bool) -> socket.socket:
        if not CONFIG["tls_enable"]:
            return sock
    ...
    def _server_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                continue
            try:
                conn = self._wrap_tls(conn, server_side=True)
            except Exception:
                pass
    ...
    def _handle_client(self, conn, addr):
        try:
            ...
            if mtype == "job":
                ...
                self.queen.gossip_import(msg.get("payload", {}))
                conn.sendall(b'{"status":"ok"}')
        except Exception:
            try:
    ...
    def send_job_to_peer(self, peer, task_type, *args, **kwargs):
        ...
        try:
            ...
            s.settimeout(2.0)
            s = self._wrap_tls(s, server_side=False)
            s.connect((host, port))
            ...
            try:
                ...
                r = {"status": "unknown"}
            self.peer_health[peer] = {"last_ok": time.time(), "status": r.get("status", "unknown")}
            self.memory.update_peer_health(self.peer_health)
            return r
        except Exception as e:
            self.peer_health[peer] = {"last_error": time.time(), "error": str(e)}
            self.memory.update_peer_health(self.peer_health)
    ...
    def _gossip_loop(self):
        while not self.stop_event.wait(5.0):
            payload = self.queen.gossip_export()
            msg = {
                "type": "gossip",
                "payload": payload,
            }
            for peer in CONFIG["multi_host_peers"]:
                host, port_str = peer.split(":")
                port = int(port_str)
                try:
                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    ...
                    try:
                        ...
                        r = {"status": "unknown"}
                    self.peer_health[peer] = {"last_gossip_ok": time.time(), "status": r.get("status", "unknown")}
                except Exception as e:
                    self.peer_health[peer] = {"last_gossip_error": time.time(), "error": str(e)}
                finally:
                    try:
                        s.close()
                    except Exception:
...
def demo_cpu_job(n=10_000_000, cancel_reg: Optional[CancellationRegistry] = None, job_id: str = ""):
    s = 0
    for i in range(n):
        if cancel_reg and cancel_reg.is_cancelled(job_id):
            return {"cancelled": True, "partial_sum": s}
        s += i * i
    return s
...
# ---------------------------------------------------------------------------
# REST API (secure token + cancellation + DFS)
# ---------------------------------------------------------------------------

class SwarmREST:
    def __init__(self, swarm: BeastSwarm, queen: Queen, memory: MemoryManager,
                 job_history: JobHistory, dfs: DistributedFS):
        self.swarm = swarm
        self.queen = queen
        self.memory = memory
        self.job_history = job_history
    ...
    def _setup_routes(self):
        ...
        def metrics():
            ...
            peer_health = self.memory.get_peer_health()
            dfs_meta = self.memory.get_dfs_metadata()
            return jsonify({
                "nodes": nodes_data,
                "queen": queen_risk,
        ...
        def job_cpu():
            ...
            job_id = self.swarm.submit_cpu(demo_cpu_job, n, priority=1, name="rest_cpu")
            return jsonify({"status": "submitted", "type": "cpu", "n": n, "job_id": job_id})
        ...
        def job_vram_bw():
            ...
            job_id = self.swarm.submit_vram_bandwidth(size_mb=size_mb, priority=1, name="rest_vram_bw")
            return jsonify({"status": "submitted", "type": "vram_bw", "size_mb": size_mb, "job_id": job_id})
        ...
        def job_vram_frag():
            ...
            job_id = self.swarm.submit_vram_fragmentation(chunks=chunks, priority=1, name="rest_vram_frag")
            return jsonify({"status": "submitted", "type": "vram_frag", "chunks": chunks, "job_id": job_id})

        @self.app.route("/job/cancel", methods=["POST"])
        def job_cancel():
            if not self._check_auth(request):
                return jsonify({"error": "unauthorized"}), 401
            payload = request.json or {}
            ...
            self.swarm.cancel_job(job_id)
            return jsonify({"status": "cancel_requested", "job_id": job_id})

        @self.app.route("/dfs/put", methods=["POST"])
        def dfs_put():
            if not self._check_auth(request):
                return jsonify({"error": "unauthorized"}), 401
            ...
            data = request.data
            res = self.dfs.put(key, data)
            return jsonify({"status": "ok", "result": res})

        @self.app.route("/dfs/get", methods=["GET"])
        def dfs_get():
            if not self._check_auth(request):
                return jsonify({"error": "unauthorized"}), 401
            key = request.args.get("key")
            if not key:
                return jsonify({"error": "missing key"}), 400
            data = self.dfs.get(key)
            if data is None:
                return jsonify({"error": "not found"}), 404
            return data

        @self.app.route("/dfs/list", methods=["GET"])
        def dfs_list():
            if not self._check_auth(request):
                return jsonify({"error": "unauthorized"}), 401
            keys = self.dfs.list_keys()
...
# ---------------------------------------------------------------------------
# GUI — Command Center with real-time charts + multi-host visualization
# ---------------------------------------------------------------------------
...
class SwarmGUI(QtWidgets.QMainWindow):
    def __init__(self, swarm: BeastSwarm, queen: Queen,
        ...
        self.setWindowTitle("Beast Swarm Engine v8.4 — Command Center")
        ...
        self.btn_dfs = QtWidgets.QPushButton("DFS")

        for b in [self.btn_overview, self.btn_nodes, self.btn_gpu,
                  self.btn_vram, self.btn_queen, self.btn_zmq,
                  self.btn_plugins, self.btn_history, self.btn_multi, self.btn_dfs]:
            b.setCheckable(True)
        ...
        self.dfs_panel = self._build_dfs_panel()
        ...
        self.stack.addWidget(self.dfs_panel)       # 9
        ...
        self.btn_dfs.clicked.connect(lambda: self._set_panel(9, self.btn_dfs))
    ...
    def _build_multi_panel(self):
        ...
        self.peer_health_list = QtWidgets.QListWidget()
        ...
        layout.addWidget(QtWidgets.QLabel("Peer health"))
        layout.addWidget(self.peer_health_list)
        layout.addStretch()
        return w

    def _build_dfs_panel(self):
        w = QtWidgets.QWidget()
        layout = QtWidgets.QVBoxLayout(w)
        layout.setContentsMargins(12, 12, 12, 12)
        layout.setSpacing(8)

        self.dfs_keys_list = QtWidgets.QListWidget()
        self.dfs_put_key = QtWidgets.QLineEdit()
        self.dfs_put_key.setPlaceholderText("key")
        self.dfs_put_data = QtWidgets.QLineEdit()
        self.dfs_put_data.setPlaceholderText("data (string)")
        self.dfs_put_btn = QtWidgets.QPushButton("Put")
        self.dfs_put_btn.clicked.connect(self.dfs_put_local)

        layout.addWidget(QtWidgets.QLabel("DFS keys"))
        layout.addWidget(self.dfs_keys_list)
        layout.addWidget(QtWidgets.QLabel("Put key"))
        layout.addWidget(self.dfs_put_key)
        layout.addWidget(QtWidgets.QLabel("Put data"))
        layout.addWidget(self.dfs_put_data)
        layout.addWidget(self.dfs_put_btn)
        layout.addStretch()
        return w
    ...
    def dfs_put_local(self):
        key = self.dfs_put_key.text().strip()
        data = self.dfs_put_data.text().encode("utf-8")
        if not key:
            QtWidgets.QMessageBox.warning(self, "DFS", "Enter key.")
            return
        res = self.dfs.put(key, data)
        QtWidgets.QMessageBox.information(self, "DFS", f"Stored key={key} size={res['size']}")
        self.refresh_dfs_panel()
    ...
    def refresh_all(self):
        ...
        if self.gpu_governor:
            ...
            for idx, p in pol.items():
                ...
                if p["suggest_fan_up"]:
                    txt += "fan↑ "
                if p["suggest_clock_down"]:
                    txt += "clock↓ "
                if p["suggest_power_limit_down"]:
                    txt += "power↓ "
                txt += "]\n"
        ...
        for j in jobs:
            ...
            self.history_list.addItem(
                f"{ts} job={j['job_id']} node={j['node_id']} type={j['task_type']} name={j['name']} "
                f"status={j['status']} dur={j['duration']:.3f}"
            )

        # Multi-host peer health
        self.peer_health_list.clear()
        ph = self.memory.get_peer_health()
        for peer, info in ph.items():
            self.peer_health_list.addItem(f"{peer}: {info}")

        # DFS panel
        self.refresh_dfs_panel()

    def refresh_dfs_panel(self):
        self.dfs_keys_list.clear()
        keys = self.dfs.list_keys()
        for k in keys:
            info = self.dfs.info(k)
            self.dfs_keys_list.addItem(f"{k} size={info.get('size', '?')}")
...
def build_local_swarm():
    ...
    queen = Queen(memory=memory, node_id="local")
    ...
    dfs = DistributedFS(CONFIG["dfs_root"], memory)
    ...
    swarm = BeastSwarm(nodes, scheduler, cancel_registry)
    tuner = SwarmTuner(nodes)
    auto_spawner = AutoSpawner(nodes, queen, memory, gpu_telemetry, gpu_governor,
                               plugin_manager, scheduler, job_history, cancel_registry)
    zmq_swarm = ZMQSwarm(swarm, CONFIG["zmq_bind"]) if zmq is not None else None
    rest_api = SwarmREST(swarm, queen, memory, job_history, dfs) if Flask is not None else None
    multi_host = MultiHostSwarm(swarm, queen, memory)

    return swarm, queen, gpu_governor, tuner, zmq_swarm, memory, plugin_manager, auto_spawner, scheduler, rest_api, job_history, multi_host, dfs


def main():
    swarm, queen, gpu_governor, tuner, zmq_swarm, memory, plugin_manager, auto_spawner, scheduler, rest_api, job_history, multi_host, dfs = build_local_swarm()
    ...
    gui = SwarmGUI(swarm, queen, gpu_governor, zmq_swarm, plugin_manager, memory, job_history, multi_host, dfs)
    gui.show()
