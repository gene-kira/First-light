...
# ============================================================
# Rogue Process Monitor v13.6 (SAFE, NON-DESTRUCTIVE)
# Ultra-Sentient Borg Swarm Edition
#
# Concepts:
...
LOG_FILE = "rogue_monitor_v13_6_log.txt"
...
def sandbox_append_history(event):
    ...
    with sandbox_lock:
        ...
        if len(sandbox_history_events) > 2500:
...
def prune_scan_cache(max_age=150.0):
    now = time.time()
    with scan_cache_lock:
...
# -------------------------
# Borg Tech: Queen + Workers + Health
# -------------------------
...
class BorgQueen:
    ...
    def decide_worker_count(self, process_count):
        ...
        if process_count <= 50:
            ...
            return min(self.max_worker_count, self.base_worker_count + 2)
        else:
...
class SandboxScanner:
    ...
    def scan_loop(self):
        log("SandboxScanner v13.6 Borg swarm scan loop started")
        while self.running:
            try:
    ...
    def tree_loop(self):
        log("SandboxScanner v13.6 tree loop started")
        while self.running:
            try:
                self.build_tree()
            except Exception as e:
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v13.6 (SAFE, Ultra-Sentient Borg Swarm)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v13.6 (SAFE, Borg Swarm)", font=("Segoe UI", 14, "bold"))
...
def main():
    log("Rogue Process Monitor v13.6 (SAFE, Ultra-Sentient Borg Swarm) starting")
    root = tk.Tk()
