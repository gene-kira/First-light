...
def autoload_libraries():
    ...
    for lib in REQUIRED_LIBS:
        try:
            ...
            print(f"[AUTOLOADER] Installing missing library: {lib}", flush=True)
            try:
                subprocess.check_call(
                    [sys.executable, "-m", "pip", "install", lib],
                    timeout=180
                )
                print(f"[AUTOLOADER] Installed: {lib}", flush=True)
            except Exception as exc:
                failures.append((lib, str(exc)))
                print(f"[AUTOLOADER] Failed to install {lib}: {exc}", flush=True)

    if failures:
        names = ", ".join(name for name, _ in failures)
        raise RuntimeError(f"Required libraries unavailable: {names}")
...
def relaunch_as_admin():
    if os.name == "nt":
        import ctypes
        print(
            "[ADMIN] Guardian requires elevated privileges. "
        ...
        try:
            ...
            if rc <= 32:
                print(f"[ADMIN] Elevation request rejected (code {rc}).", flush=True)
                return False
            return True
        except Exception as exc:
            print(f"[ADMIN] Elevation failed: {exc}", flush=True)
            return False

    print("[ADMIN] Guardian requires elevated privileges. Relaunching with sudo...", flush=True)
    try:
...
if not is_admin():
    ...
    raise PermissionError(
        "Administrator/root privileges are required and elevation failed."
    )
...
# ---------------------------------------------------------
# PROTECTIVE RUNTIME WRAPPER
# ---------------------------------------------------------

WRAPPER_LOG_NAME = "guardian_wrapper.log"
WRAPPER_LOCK_NAME = "guardian_wrapper.lock"
WRAPPER_MAX_RESTARTS = 5
WRAPPER_RESTART_WINDOW = 300.0
WRAPPER_BACKOFF_SECONDS = 3.0


def get_wrapper_log_path() -> str:
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), WRAPPER_LOG_NAME)


def wrapper_log(message: str) -> None:
    stamp = time.strftime("%Y-%m-%d %H:%M:%S")
    line = f"{stamp} {message}"
    try:
        with open(get_wrapper_log_path(), "a", encoding="utf-8") as f:
            f.write(line + "\n")
    except Exception:
        pass
...
def acquire_single_instance_lock():
    if os.name == "nt":
        ...
        mutex = kernel32.CreateMutexW(None, False, "Global\\AISystemGuardianV8")
        if not mutex:
            raise RuntimeError("Unable to create Guardian instance mutex.")
        if kernel32.GetLastError() == 183:
            kernel32.CloseHandle(mutex)
            return None
        return ("windows_mutex", mutex)

    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), WRAPPER_LOCK_NAME)
    try:
        fd = os.open(path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        os.write(fd, str(os.getpid()).encode("ascii"))
        return ("posix_file", (fd, path))
    except FileExistsError:
        return None


def release_single_instance_lock(lock) -> None:
    if not lock:
        return
    kind, value = lock
    try:
        if kind == "windows_mutex":
            import ctypes
...
def run_guardian_once() -> int:
    wrapper_log("[START] Creating Qt application.")
    ...
    wrapper_log("[HEALTH] Guardian GUI initialized successfully.")
    return int(app.exec_())


def protective_runtime_wrapper() -> int:
    install_exception_hook()
    lock = acquire_single_instance_lock()

    if lock is None:
        wrapper_log("[GUARD] Another Guardian instance is already running.")
        return 2
    ...
    try:
        wrapper_log("[GUARD] Protective runtime wrapper started.")

        while True:
            now = time.monotonic()
            restart_times = [
                t for t in restart_times
                if now - t <= WRAPPER_RESTART_WINDOW
            ]

            try:
                rc = run_guardian_once()

                if rc == 0:
                    wrapper_log("[GUARD] Clean shutdown; no restart required.")
                    return 0

                wrapper_log(f"[GUARD] Non-zero application exit: {rc}.")

                if len(restart_times) >= WRAPPER_MAX_RESTARTS:
                    wrapper_log("[GUARD] Restart limit reached.")
                    return rc if rc else 1

            except KeyboardInterrupt:
                wrapper_log("[GUARD] Keyboard interrupt; clean shutdown.")
                return 0

            except SystemExit as exc:
                code = exc.code if isinstance(exc.code, int) else 1
                wrapper_log(f"[GUARD] SystemExit received: {code}.")
                return code
                ...
                if len(restart_times) >= WRAPPER_MAX_RESTARTS:
                    wrapper_log("[GUARD] Maximum restart attempts reached; stopping.")
                    return 1
                ...
                delay = min(
                    30.0,
                    WRAPPER_BACKOFF_SECONDS * (2 ** min(attempt - 1, 4))
                )
                wrapper_log(
                    f"[RECOVERY] Restarting in {delay:.1f}s "
                    f"(attempt {attempt})."
                )
                time.sleep(delay)
        ...
        wrapper_log("[GUARD] Protective runtime wrapper stopped.")
...
def scan_system(stats: Dict, borg_memory: Dict, physics: Dict, swarm_state: Dict) -> Tuple[List[AssetIncident], Dict, Dict, Dict, Dict, float, float]:
    ...
    try:
        process_iter = psutil.process_iter(['pid', 'name'])
    except Exception as exc:
        append_guardian_log(f"[SCAN] Process enumeration unavailable: {exc}")
        process_iter = []
...
class SocGuardianWindow(QMainWindow):
    ...
    def guardian_tick(self, manual: bool = False):
        try:
            self._guardian_tick_impl(manual=manual)
        except Exception as exc:
            append_guardian_log(f"[TICK] Recovered from guardian tick error: {exc}")
            wrapper_log(f"[TICK] Recovered from periodic scan error: {exc}")

    def _guardian_tick_impl(self, manual: bool = False):
...
def main():
    return protective_runtime_wrapper()
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        wrapper_log("[FATAL] Supervisor-level failure:\n" + traceback.format_exc())
        raise
