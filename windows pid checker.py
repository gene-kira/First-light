    ...
    for m in modules:
        try:
            ...
            try:
                ...
                # non-fatal: some parts will fall back to stubs
...
class Config:
    VERSION = "overwatch-god-honeypot-v6"
    ...
    # OverWatch
    BASELINE_FILE = "overwatch_baseline_v6.json"
...
class AnomalyEngine:
    @staticmethod
    def score(proc, profile: ProcessProfile):
        score = 0
...
class OverWatchCore(threading.Thread):
    def __init__(self, event_queue: queue.Queue, baseline: Baseline, honeypot: HoneypotCore):
    ...
    def scan_once(self):
        for proc in psutil.process_iter(['pid', 'name', 'exe', 'ppid']):
            try:
                ...
                score = AnomalyEngine.score(proc, profile)
                ...
                if score >= 50:
                    ...
                    if profile.honeypot_flag or profile.data_tx_flag:
...
class FusionGUI:
    def __init__(self, root, baseline: Baseline, event_queue: queue.Queue,
                 honeypot: HoneypotCore, analytics: DeceptionAnalytics):
        self.root = root
        self.root.title("OverWatch + God-Mode Honeypot v6")
    ...
    def build_ribbon(self):
        ...
        self.ribbon.add(self.tab_honeypot, text="Honeypot")
...
def main():
    log_event("bootstrap", "INFO", "Starting OverWatch + God-Mode Honeypot v6", {"version": Config.VERSION})
    fusion = FusionCore()

    root = tk.Tk()
    gui = FusionGUI(root, fusion.baseline, fusion.event_queue, fusion.honeypot, fusion.analytics)
