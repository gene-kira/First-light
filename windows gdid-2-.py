...
# -*- coding: utf-8 -*-
"""
Codex Purge Shell – Unified Python File v2
...
# ============================================================
# Persona Engine (with Persona Lock)
# ============================================================
...
class PersonaEngine:
    ...
    Supports Persona Lock to freeze identity.
    """
...
# ============================================================
# IdentityCRL Shadow Layer (with Kill Switch + Leak Detector)
# ============================================================
...
class IdentityShadowLayer:
    ...
    def get_effective_lid(self, subsystem: str) -> int:
        ...
        - If Kill Switch is ON: always fake LID.
        - If Shadow is active: fake LID.
        - Else: real LID, but Leak Detector logs a warning.
        """
        with self._lock:
            ...
            # Leak Detector: real LID would be used
...
# ============================================================
# Fake Telemetry Engine (with Leak Detector integration)
# ============================================================

class FakeTelemetryEngine:
    """
    Uses PersonaEngine + IdentityShadowLayer to generate fake telemetry.
    Integrates Kill Switch and Leak Detector.
    """
    def __init__(
        self,
    ...
    def _handle_identity_access(self, event: Dict[str, Any]):
        ...
        glyph = self._glyph_for_state(shadow_active, kill_switch)
        msg = (
            f"{glyph} Identity access by [{subsystem}] -> "
            f"LID={lid} (shadow_active={shadow_active}, kill_switch={kill_switch}) "
    ...
    def _glyph_for_state(self, shadow_active: bool, kill_switch: bool) -> str:
        if kill_switch:
            return "■"  # Hard block
        if shadow_active:
...
# ============================================================
# GUI (Glyph-coded warning system + Kill Switch + Persona Lock)
# ============================================================
...
class CodexGUI:
    ...
    def __init__(
        ...
        self.root.title("Codex Purge Shell – Identity Shadow Layer v2")
    ...
    def _build_layout(self):
        ...
        self.shadow_info_text = tk.Text(self.shadow_frame, height=6, width=100)
        self.shadow_info_text.grid(row=2, column=0, columnspan=3, padx=5, pady=5)
        ...
        self.persona_info_text = tk.Text(self.persona_frame, height=7, width=100)
        self.persona_info_text.grid(row=0, column=0, columnspan=3, padx=5, pady=5)
        ...
        self.telemetry_text = tk.Text(self.telemetry_frame, height=15, width=100)
        ...
        legend_text = (
            "■ Kill Switch ACTIVE – hard block, real identity never leaves.\n"
    ...
    def _refresh_log(self):
        recent = logger.get_recent(limit=80)
