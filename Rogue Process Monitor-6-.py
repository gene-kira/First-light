...
# ============================================================
# Rogue Process Monitor v13.4 (SAFE, NON-DESTRUCTIVE)
# God-Tier Edition
# - Full sandbox isolation
# - Incremental scanning with PID cache
# - Multi-threaded scoring pool
# - Diff-based GUI updates
# - Virtualized live view (windowed paging)
# - Deep snapshot mode (DLLs, connections, threads)
# - Smoothed threat meter
# - Throttled alerts
# - Still NO killing, blocking, or destructive actions
# ============================================================

LOG_FILE = "rogue_monitor_v13_4_log.txt"
...
def sandbox_append_history(event):
    ...
    with sandbox_lock:
        ...
        if len(sandbox_history_events) > 1500:
...
def prune_scan_cache(max_age=90.0):
    now = time.time()
    with scan_cache_lock:
...
class SandboxScanner:
    ...
    def scan_loop(self):
        log("SandboxScanner v13.4 scan loop started")
        while self.running:
            try:
    ...
    def tree_loop(self):
        log("SandboxScanner v13.4 tree loop started")
        while self.running:
            try:
                self.build_tree()
            except Exception as e:
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v13.4 (SAFE, God-Tier Sandbox)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v13.4 (SAFE, Sandbox)", font=("Segoe UI", 14, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))

        desc = ttk.Label(
            frame,
            text=(
                "All heavy scanning runs in a separate sandbox with incremental scanning and multi-threaded scoring.\n"
                "GUI uses diff-based updates, paging, and throttling to stay responsive.\n"
    ...
    def _build_snapshot_tab(self):
        ...
        title = ttk.Label(frame, text="Process Snapshot (Deep)", font=("Segoe UI", 12, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))
...
def main():
    log("Rogue Process Monitor v13.4 (SAFE, God-Tier Sandbox) starting")
    root = tk.Tk()
