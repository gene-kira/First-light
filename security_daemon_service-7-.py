#!/usr/bin/env python3
# Security Daemon v12 – Mode C (Full Manual + Hybrid Honeypot, Advanced)
...
def init_ml_model():
    ...
    try:
        ...
        ML_MODEL.eval()
        enc_log("ML_MODEL initialized (autoencoder stub)", "INFO")
    except Exception as e:
...
def ml_score_process(path, cmdline, parent_pid):
    ...
    features = [len(path), len(cmdline), parent_pid or 0, bonus, 0, 0]
...
def ml_score_network(remote_ip, remote_port):
    ...
    features = [remote_port, bonus, 0, 0, 0, 0]
    score = ml_anomaly_score(features)
    if score > 0.01:
        bonus += 5
...
class StatusGUI(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Security Daemon v12 – Mode C Hybrid Honeypot (Advanced)")
