#!/usr/bin/env python3
"""
Python Trust Console v7 (Compact Dark, Safe Apply Trust, OS-Level Aware)
...
- Safe Apply Trust: never crashes the app, logs error instead
"""
...
# ==========================
# COMPACT PySide6 GUI (Dark, v7, Safe Apply Trust)
# ==========================
from PySide6 import QtWidgets, QtCore, QtGui
...
class TrustConsoleWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Trust Console v7 (Compact Dark, Safe Apply Trust)")
    ...
    def on_apply_trust(self):
        def worker():
            try:
                ...
                err_msg = f"[ERROR] Apply Trust crashed: {e}"
                self.gui_log(err_msg)
                log_error(err_msg)
    ...
    def on_generate_trust(self):
        def worker():
            try:
                ...
                err_msg = f"[ERROR] Generate Trust crashed: {e}"
                self.gui_log(err_msg)
                log_error(err_msg)
    ...
    def on_reset_defaults(self):
        def worker():
            try:
                ...
                err_msg = f"[ERROR] Reset Defaults crashed: {e}"
                self.gui_log(err_msg)
                log_error(err_msg)
    ...
    def on_backup(self):
        def worker():
            try:
                ...
                err_msg = f"[ERROR] Backup crashed: {e}"
                self.gui_log(err_msg)
                log_error(err_msg)
    ...
    def on_rollback(self):
        def worker():
            try:
                ...
                err_msg = f"[ERROR] Rollback crashed: {e}"
                self.gui_log(err_msg)
                log_error(err_msg)
