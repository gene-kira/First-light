#!/usr/bin/env python3
"""
Borg AI Static Background Upscaler (CPU-Only, RVM + Real-ESRGAN + Multi-Thread + Audio)
---------------------------------------------------------------------------------------
...
Run GUI:
    python borg_upscaler_cpu_rvm_esrgan_mt_audio.py

Run batch:
    python borg_upscaler_cpu_rvm_esrgan_mt_audio.py --batch input_folder --export_folder output_folder
"""
...
def make_profile(profile: str) -> UpscaleConfig:
    ...
    if profile == "movies":
        ...
        return UpscaleConfig(profile_name="auto", use_optical_flow=True, borg_read_ahead_frames=15, use_multithread=True)
...
class BackgroundReconstructorAI:
    def __init__(self, onnx_path: Optional[str] = None, downloader: Optional[DownloadManager] = None):
    ...
    def reconstruct(self, bg: np.ndarray) -> np.ndarray:
        if self.session is None:
            log("[BG-AI] ESRGAN-style ONNX not available; using bilateral filter.")
...
class BackgroundModeler:
    def __init__(self, config: UpscaleConfig, bg_ai: Optional[BackgroundReconstructorAI] = None):
        self.config = config
        self._frames_buffer = []
...
# ---------------------------------------------------------
# PIPELINE (with multi-threaded CPU acceleration)
# ---------------------------------------------------------
...
class StaticBackgroundUpscalerPipeline:
    def __init__(
        ...
        self.bg_ai = BackgroundReconstructorAI(downloader=downloader)
        self.bg_modeler = BackgroundModeler(config, bg_ai=self.bg_ai)

        self.rvm_segmenter = None
        if use_rvm_onnx and downloader is not None:
            rvm_path = os.path.join("models", "rvm_model.onnx")
            if not os.path.isfile(rvm_path):
                dl_path = downloader.auto_download_rvm_onnx()
                if dl_path:
                    rvm_path = dl_path
            if os.path.isfile(rvm_path):
                self.rvm_segmenter = RVMOnnxSegmenter(rvm_path)

        self.motion_detector = MotionDetector(config, segmenter_name=segmenter_name, rvm_segmenter=self.rvm_segmenter)
    ...
    def initialize(self, vin: VideoInput) -> None:
        ...
        if self.auto_profile and self.config.profile_name == "auto":
            ...
            self.bg_modeler = BackgroundModeler(self.config, bg_ai=self.bg_ai)
            self.motion_detector = MotionDetector(self.config, segmenter_name=self.motion_detector.segmenter_name, rvm_segmenter=self.rvm_segmenter)
...
class UpscalerGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Borg AI Static Background Upscaler (CPU-Only, RVM+ESRGAN+MT+Audio)")
