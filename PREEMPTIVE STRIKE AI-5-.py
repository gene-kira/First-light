#!/usr/bin/env python3
# ================================================================
#  PREEMPTIVE STRIKE AI — ULTRA NEXT-GEN v7 (Qt Edition)
...
# ================================================================
# GAME RAM + VRAM HYBRID PRELOADER + OPTIMIZER
# ================================================================
...
class WaterPhysicsEngine:
    ...
    def sample_state(self):
        ...
        # Event-driven: only log if changed significantly (5% threshold)
    ...
    def compute_pressure(self, flows):
        pressure = (flows["ram_level"] + flows["cpu_level"]) / 2.0
        return pressure

    def is_stable(self, flows):
        pressure = self.compute_pressure(flows)
...
class PreemptiveStrikeAI:
    def __init__(self, injector: CredentialInjector, preloader: GameRAMPreloader,
        ...
        self.last_telemetry = None  # for 5% CPU/RAM threshold
    ...
    def _should_push_telemetry(self, phase: str, cpu: float, ram_used: float) -> bool:
        ...
        cpu_changed = abs(cpu - last_cpu) >= 5.0  # 5% absolute CPU difference
        ram_changed = abs(ram_used - last_ram) / total_ram >= 0.05  # 5% RAM difference

        return phase_changed or cpu_changed or ram_changed

    def _push_phase_telemetry(self, phase: str, flows: dict):
        vm = psutil.virtual_memory()
        cpu = psutil.cpu_percent()
        ram_used = vm.used
    ...
    def loop(self):
        while self.running:
            try:
                ...
                if days < 1.0:
                    # Day 1: Observation only, event-driven telemetry
                    self._push_phase_telemetry("observe", flows)

                elif days < 2.0:
                    # Day 2: Prediction only (no actions), event-driven telemetry
                    ...
                    self._push_phase_telemetry("predict", flows)

                else:
                    # Day 3+: Full reasoning: predict + multi-step plan + execute
                    prediction, confidence = self.predictor.predict_with_uncertainty()
                    self._push_phase_telemetry("act", flows)
...
class PreemptiveMainWindow(QMainWindow):
    def __init__(self, service: BackendService):
        ...
        self.resize(900, 600)
        self.last_telemetry_ts = None  # to avoid GUI spam
        self._build_ui()
        self._setup_timer()
    ...
    def _build_ui(self):
        ...
        # Status / Phase Indicator (Color style B)
        ...
        game_row1 = QHBoxLayout()
        self.game_name = QLineEdit()
        self.game_name.setPlaceholderText("Profile Name")
        self.game_folder = QLineEdit()
        self.game_folder.setPlaceholderText("Game Folder")
        browse_btn = QPushButton("Browse")
        browse_btn.clicked.connect(self.browse_folder)
        game_row1.addWidget(self.game_name)
        game_row1.addWidget(self.game_folder)
        game_row1.addWidget(browse_btn)
        layout.addLayout(game_row1)

        game_row2 = QHBoxLayout()
        self.game_mode = QComboBox()
        self.game_mode.addItems(["ram", "vram", "hybrid"])
        add_profile_btn = QPushButton("Add Profile")
        add_profile_btn.clicked.connect(self.add_profile)
        game_row2.addWidget(QLabel("Mode:"))
        game_row2.addWidget(self.game_mode)
        game_row2.addWidget(add_profile_btn)
        layout.addLayout(game_row2)

        # Manual actions / training
        ...
        record_game_btn = QPushButton("Record: Launch Game (selected)")
        record_game_btn.clicked.connect(self.record_launch_selected)
    ...
    def add_profile(self):
        ...
        folder = self.game_folder.text().strip()
        mode = self.game_mode.currentText()
        if not name or not folder:
            self.log("[GUI] Error: Profile name and folder required.")
            return
        profile = GameProfile(name=name, folder=folder, preload_mode=mode)
        self.service.preloader.add_profile(profile)
        self.log(f"[GUI] Added game profile: {name} ({mode})")
    ...
    def record_launch_selected(self):
        name = self.game_name.text().strip()
        if not name:
            self.log("[GUI] Error: Profile name required.")
            return
        self.record_action(f"launch_game:{name}")
    ...
    def poll_telemetry(self):
        ...
        if tele:
            ...
            if self.last_telemetry_ts is not None and ts == self.last_telemetry_ts:
                # Same telemetry as last time: do not log again
                return
