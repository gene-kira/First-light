#!/usr/bin/env python3
"""
Borg AI Video Lab (CPU-Only, RAM+Configurable NVMe Foresight, RVM+ESRGAN+SwinIR+CodeFormer+Deblur+Tracking+Dissection+Color+Objects+Foreground-SR)
-------------------------------------------------------------------------------------------------------------------
...
Upgrades:
    ...
    - AI motion deblurring via MPRNet-style ONNX (CPUExecutionProvider)
    - Face tracking (SORT-style tracker)
    - Picture dissection with AI (region tagging)
    - AI color enhancement (ColorNet-style ONNX, CPU-only, with fallback)
    - AI object detection (YOLO-style ONNX, CPU-only, with fallback)
    - Foreground-only super-resolution (SR applied only to motion/seg regions)
    ...
    - Batch mode, mirror scanner, plugin system
    - 1GB Borg Read-Ahead RAM Manager (deep foresight buffer)
    - Configurable NVMe scratchpad buffer (disk-backed foresight beyond RAM)
"""
...
def bootstrap():
    ...
    print("[BOOTSTRAP] Launching Borg AI Video Lab...")
...
class UpscaleConfig:
    def __init__(
        ...
        nvme_max_bytes: int = 10_000_000_000,
        nvme_base_dir: str = "scratchpad_nvme",
    ):
...
class BorgReadAheadManager:
    def __init__(self, max_bytes: int = 1_000_000_000):
        self.max_bytes = max_bytes
        self.raw_frames: List[np.ndarray] = []
        self.small_frames: List[np.ndarray] = []
        self.motion_masks: List[np.ndarray] = []
        self.seg_masks: List[np.ndarray] = []
        self.lock = threading.Lock()

    def _current_usage(self) -> int:
        total = 0
        for lst in (self.raw_frames, self.small_frames, self.motion_masks, self.seg_masks):
            for a in lst:
                total += a.nbytes
        return total
    ...
    def _evict_if_needed(self):
        ...
        if usage <= self.max_bytes:
            return
        log(f"[Borg1G] Memory usage {usage} > {self.max_bytes}, evicting older entries...")
        for lst in (self.seg_masks, self.motion_masks, self.small_frames, self.raw_frames):
            while lst and usage > self.max_bytes:
                arr = lst.pop(0)
                usage -= arr.nbytes

    def add_raw_frame(self, frame: np.ndarray):
        with self.lock:
            self.raw_frames.append(frame.copy())
            self._evict_if_needed()

    def add_small_frame(self, frame: np.ndarray):
        with self.lock:
            self.small_frames.append(frame.copy())
            self._evict_if_needed()

    def add_motion_mask(self, mask: np.ndarray):
        with self.lock:
            self.motion_masks.append(mask.copy())
            self._evict_if_needed()

    def add_seg_mask(self, mask: np.ndarray):
        with self.lock:
            self.seg_masks.append(mask.copy())
            self._evict_if_needed()

    def get_latest_small_frames(self, count: int = 30) -> List[np.ndarray]:
        with self.lock:
            return self.small_frames[-count:].copy()

    def get_latest_motion_masks(self, count: int = 30) -> List[np.ndarray]:
        with self.lock:
            return self.motion_masks[-count:].copy()

    def get_latest_seg_masks(self, count: int = 30) -> List[np.ndarray]:
        with self.lock:
            return self.seg_masks[-count:].copy()


class NVMEScratchpadBuffer:
    """
    NVMe scratchpad buffer:
        - disk-backed foresight beyond RAM
        - size and base directory controlled via config
    """

    def __init__(self, base_dir: str = "scratchpad_nvme", max_bytes: int = 10_000_000_000):
        ...
        if os.path.isfile(self.index_path):
            try:
                with open(self.index_path, "r") as f:
                    ...
                    self.current_bytes = data.get("bytes", 0)
            except Exception:
                self.index = []
                self.current_bytes = 0
    ...
    def _save_index(self):
        try:
            with open(self.index_path, "w") as f:
                json.dump({"entries": self.index, "bytes": self.current_bytes}, f)
        except Exception as e:
            log(f"[NVMe] Failed to save index: {e}")

    def _evict_if_needed(self):
        while self.current_bytes > self.max_bytes and self.index:
            entry = self.index.pop(0)
            path = entry["path"]
            size = entry["size"]
            try:
    ...
    def add_frame(self, frame: np.ndarray, tag: str = "frame"):
        fname = f"{tag}_{uuid.uuid4().hex}.npz"
        fpath = os.path.join(self.base_dir, fname)
        try:
            np.savez_compressed(fpath, frame=frame)
            size = os.path.getsize(fpath)
            self.index.append({"path": fpath, "size": size, "tag": tag})
            self.current_bytes += size
            self._evict_if_needed()
            self._save_index()
        except Exception as e:
            log(f"[NVMe] Failed to store frame: {e}")

    def get_recent_frames(self, count: int = 50, tag: Optional[str] = None) -> List[np.ndarray]:
        frames = []
...
class MirrorScanner:
    ...
    def scan_mprnet_deblur_sites(self) -> List[str]:
        sites = []
        api_url = "https://api.github.com/repos/swz30/MPRNet/releases"
        data = self._safe_get_json(api_url)
        if data:
            for rel in data:
                for asset in rel.get("assets", []):
                    ...
                    if dl_url and dl_url.endswith(".onnx"):
                        sites.append(dl_url)
            self._log(f"[MirrorScanner] Found {len(sites)} MPRNet ONNX candidates via GitHub.")
        else:
            self._log("[MirrorScanner] Failed to query MPRNet GitHub for ONNX models.")
        return sites

    def scan_colornet_sites(self) -> List[str]:
        sites = []
        api_url = "https://api.github.com/repos/someuser/ColorNet/releases"
        data = self._safe_get_json(api_url)
        if data:
            for rel in data:
                for asset in rel.get("assets", []):
                    ...
                    if dl_url and dl_url.endswith(".onnx"):
                        sites.append(dl_url)
            self._log(f"[MirrorScanner] Found {len(sites)} ColorNet ONNX candidates via GitHub.")
        else:
            self._log("[MirrorScanner] Failed to query ColorNet GitHub for ONNX models.")
        return sites

    def scan_yolo_sites(self) -> List[str]:
        sites = []
        api_url = "https://api.github.com/repos/ultralytics/yolov5/releases"
        data = self._safe_get_json(api_url)
        if data:
            for rel in data:
                for asset in rel.get("assets", []):
                    ...
                    if dl_url and dl_url.endswith(".onnx"):
                        sites.append(dl_url)
            self._log(f"[MirrorScanner] Found {len(sites)} YOLO ONNX candidates via GitHub.")
        else:
            self._log("[MirrorScanner] Failed to query YOLO GitHub for ONNX models.")
        return sites
...
class DownloadManager:
    ...
    def auto_download_mprnet_onnx(self) -> Optional[str]:
        sites = self.mirror_scanner.scan_mprnet_deblur_sites()
        if not sites:
            self._log("[Downloader] No MPRNet ONNX model sites found.")
            return None
        url = sites[0]
        self._log(f"[Downloader] Auto-downloading MPRNet ONNX model from: {url}")
        path = self.download_from_url(url, "models")
        return path

    def auto_download_colornet_onnx(self) -> Optional[str]:
        sites = self.mirror_scanner.scan_colornet_sites()
        if not sites:
            self._log("[Downloader] No ColorNet ONNX model sites found.")
            return None
        url = sites[0]
        self._log(f"[Downloader] Auto-downloading ColorNet ONNX model from: {url}")
        path = self.download_from_url(url, "models")
        return path

    def auto_download_yolo_onnx(self) -> Optional[str]:
        sites = self.mirror_scanner.scan_yolo_sites()
        if not sites:
            self._log("[Downloader] No YOLO ONNX model sites found.")
            return None
        url = sites[0]
        self._log(f"[Downloader] Auto-downloading YOLO ONNX model from: {url}")
        path = self.download_from_url(url, "models")
        return path
...
class MotionDeblurrerAI:
    def __init__(self, onnx_path: Optional[str] = None, downloader: Optional[DownloadManager] = None, ai_level: int = 2):
        self.onnx_path = onnx_path or os.path.join("models", "deblur_mprnet.onnx")
        self.downloader = downloader
        self.session = None
        self.ai_level = ai_level
        self._auto_init()
    ...
    def _auto_init(self):
        if self.ai_level <= 0:
            log("[DeblurAI] AI level 0: skipping MPRNet ONNX init, using simple unsharp mask.")
            return
        ...
        if os.path.isfile(self.onnx_path):
            ...
            return
        if self.downloader is not None and self.ai_level >= 1:
            log("[DeblurAI] No local MPRNet ONNX; attempting auto-download.")
            dl_path = self.downloader.auto_download_mprnet_onnx()
            if dl_path and os.path.isfile(dl_path):
                self.onnx_path = dl_path
                self._init_session()
                return
        log("[DeblurAI] No MPRNet ONNX available; will fall back to unsharp mask.")
    ...
    def _init_session(self):
        try:
            ...
            providers = cpu_only_onnx_providers()
            self.session = ort.InferenceSession(self.onnx_path, providers=providers)
            log("[DeblurAI] MPRNet ONNX motion deblurrer loaded (CPUExecutionProvider).")
        except Exception as e:
            log(f"[DeblurAI] Failed to load MPRNet ONNX: {e}")
            self.session = None

    def deblur(self, frame: np.ndarray) -> np.ndarray:
        if self.session is None or self.ai_level <= 0:
            log("[DeblurAI] Using unsharp mask fallback.")
            blur = cv2.GaussianBlur(frame, (0, 0), 3)
            out = cv2.addWeighted(frame, 1.5, blur, -0.5, 0)
            return out
        ...
        out = np.transpose(out, (1, 2, 0))
        out = np.clip(out * 255.0, 0, 255).astype(np.uint8)
        log("[DeblurAI] Frame deblurred via MPRNet ONNX.")
        return out


class ColorEnhancerAI:
    def __init__(self, onnx_path: Optional[str] = None, downloader: Optional[DownloadManager] = None, ai_level: int = 2):
        self.onnx_path = onnx_path or os.path.join("models", "color_enhance_colornet.onnx")
        self.downloader = downloader
        self.session = None
        self.ai_level = ai_level
        self._auto_init()
    ...
    def _auto_init(self):
        if self.ai_level <= 0:
            log("[ColorAI] AI level 0: skipping ColorNet ONNX init, using simple color boost.")
            return
        ...
        if os.path.isfile(self.onnx_path):
            ...
            return
        if self.downloader is not None and self.ai_level >= 1:
            log("[ColorAI] No local ColorNet ONNX; attempting auto-download.")
            dl_path = self.downloader.auto_download_colornet_onnx()
            if dl_path and os.path.isfile(dl_path):
                self.onnx_path = dl_path
                self._init_session()
                return
        log("[ColorAI] No ColorNet ONNX available; will fall back to simple color boost.")
    ...
    def _init_session(self):
        try:
            ...
            providers = cpu_only_onnx_providers()
            self.session = ort.InferenceSession(self.onnx_path, providers=providers)
            log("[ColorAI] ColorNet ONNX color enhancer loaded (CPUExecutionProvider).")
        except Exception as e:
            log(f"[ColorAI] Failed to load ColorNet ONNX: {e}")
            self.session = None

    def enhance_color(self, frame: np.ndarray) -> np.ndarray:
        if self.session is None or self.ai_level <= 0:
            log("[ColorAI] Using simple saturation/contrast boost fallback.")
        ...
        out = np.transpose(out, (1, 2, 0))
        out = np.clip(out * 255.0, 0, 255).astype(np.uint8)
        log("[ColorAI] Frame color enhanced via ColorNet ONNX.")
        return out


class ObjectDetectorAI:
    def __init__(self, onnx_path: Optional[str] = None, downloader: Optional[DownloadManager] = None, ai_level: int = 2):
        self.onnx_path = onnx_path or os.path.join("models", "objects_yolo.onnx")
        self.downloader = downloader
        self.session = None
        self.ai_level = ai_level
        self._auto_init()
    ...
    def _auto_init(self):
        if self.ai_level <= 0:
            log("[ObjectAI] AI level 0: skipping YOLO ONNX init, using no detection.")
            return
        ...
        if os.path.isfile(self.onnx_path):
            ...
            return
        if self.downloader is not None and self.ai_level >= 1:
            log("[ObjectAI] No local YOLO ONNX; attempting auto-download.")
            dl_path = self.downloader.auto_download_yolo_onnx()
            if dl_path and os.path.isfile(dl_path):
                self.onnx_path = dl_path
                self._init_session()
                return
        log("[ObjectAI] No YOLO ONNX available; will fall back to no detection.")
    ...
    def _init_session(self):
        try:
            ...
            providers = cpu_only_onnx_providers()
            self.session = ort.InferenceSession(self.onnx_path, providers=providers)
            log("[ObjectAI] YOLO ONNX object detector loaded (CPUExecutionProvider).")
        except Exception as e:
            log(f"[ObjectAI] Failed to load YOLO ONNX: {e}")
            self.session = None
...
class BackgroundModeler:
    def __init__(self, config: UpscaleConfig, bg_ai: Optional[BackgroundReconstructorAI] = None, bg_refiner: Optional[SwinIRBackgroundRefiner] = None, borg_manager: Optional[BorgReadAheadManager] = None, nvme_buffer: Optional[NVMEScratchpadBuffer] = None):
        self.config = config
        self._frames_buffer = []
        ...
        self.borg_manager = borg_manager
        self.nvme_buffer = nvme_buffer

    def accumulate_initial(self, frame: np.ndarray) -> None:
        if not self.initial_built:
            self._frames_buffer.append(frame.copy())
            if self.borg_manager is not None:
                small = cv2.resize(frame, (frame.shape[1] // 4, frame.shape[0] // 4), interpolation=cv2.INTER_AREA)
                self.borg_manager.add_small_frame(small)
            if self.nvme_buffer is not None:
                self.nvme_buffer.add_frame(frame, tag="bg_init")
    ...
    def borg_read_ahead(self, vin, max_frames: int) -> None:
        ...
        while count < max_frames:
            ...
            if self.borg_manager is not None:
                small = cv2.resize(frame, (frame.shape[1] // 4, frame.shape[0] // 4), interpolation=cv2.INTER_AREA)
                self.borg_manager.add_small_frame(small)
                self.borg_manager.add_raw_frame(frame)
            if self.nvme_buffer is not None:
                self.nvme_buffer.add_frame(frame, tag="bg_future")
            count += 1
        log(f"[BorgReadAhead] Buffered {count} future frames for foresight (RAM+NVMe).")
...
class MotionDetector:
    def __init__(self, config: UpscaleConfig, segmenter_name: Optional[str] = None, rvm_segmenter: Optional[RVMOnnxSegmenter] = None, borg_manager: Optional[BorgReadAheadManager] = None, nvme_buffer: Optional[NVMEScratchpadBuffer] = None):
        self.config = config
    ...
    def _apply_segmenter(self, frame: np.ndarray, motion_mask: np.ndarray) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        seg_mask = None
        if self.segmenter_name == "rvm_onnx" and self.rvm_segmenter is not None:
            ...
            if self.borg_manager is not None:
                self.borg_manager.add_seg_mask(seg_mask)
                self.borg_manager.add_motion_mask(motion_mask)
            if self.nvme_buffer is not None:
                self.nvme_buffer.add_frame(seg_mask, tag="seg_mask")
            return combined, seg_mask

        if not self.segmenter_name:
            if self.borg_manager is not None:
                self.borg_manager.add_motion_mask(motion_mask)
            if self.nvme_buffer is not None:
                self.nvme_buffer.add_frame(motion_mask, tag="motion_mask")
    ...
    def compute_motion_mask(self, frame: np.ndarray) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        curr_gray = self._to_gray(frame)

        if self.prev_frame_gray is None:
            self.prev_frame_gray = curr_gray
            return np.zeros_like(curr_gray, dtype=np.uint8), None
        ...
        combined, seg_mask = self._apply_segmenter(frame, mask)
...
class FaceTracker:
    ...
    def update(self, faces: List[Tuple[int, int, int, int]]) -> List[Dict]:
        updated_tracks = []
        for (x, y, w, h) in faces:
            track = {
                "id": self.next_id,
...
class PictureDissectorAI:
    def __init__(self):
        pass

    def dissect(self, frame: np.ndarray, motion_mask: np.ndarray, seg_mask: Optional[np.ndarray] = None) -> List[Dict]:
        h, w = frame.shape[:2]
        regions = []
        grid_x = 4
        grid_y = 4
        ...
        for gy in range(grid_y):
            for gx in range(grid_x):
                ...
                mm = motion_mask[y0:y1, x0:x1]
                motion_ratio = float(np.count_nonzero(mm)) / (mm.size + 1e-6)
                ...
                if seg_mask is not None:
                    sm = seg_mask[y0:y1, x0:x1]
                    seg_ratio = float(np.count_nonzero(sm)) / (sm.size + 1e-6)
                    if seg_ratio > 0.1:
                        tag = "foreground"
...
class Compositor:
    def __init__(
        self,
        upscaled_background: np.ndarray,
        original_size: Tuple[int, int],
        face_enhancer: Optional[CodeFormerFaceEnhancer] = None,
        deblurrer: Optional[MotionDeblurrerAI] = None,
        face_tracker: Optional[FaceTracker] = None,
        dissector: Optional[PictureDissectorAI] = None,
        color_enhancer: Optional[ColorEnhancerAI] = None,
        object_detector: Optional[ObjectDetectorAI] = None,
        sr_model: Optional[BackgroundUpscaler] = None,
    ):
        ...
        self.color_enhancer = color_enhancer
        self.object_detector = object_detector
        self.sr_model = sr_model

    def _foreground_only_sr(self, frame_up: np.ndarray, mask_up: np.ndarray) -> np.ndarray:
        if self.sr_model is None:
            return frame_up

        fg_mask_3c = np.stack([mask_up] * 3, axis=2)
        fg = np.where(fg_mask_3c == 255, frame_up, 0)
        bg = np.where(fg_mask_3c == 0, frame_up, 0)

        fg_sr = self.sr_model.upscale(fg)
        bg_sr = cv2.resize(bg, (fg_sr.shape[1], fg_sr.shape[0]), interpolation=cv2.INTER_LINEAR)

        combined = np.clip(fg_sr + bg_sr, 0, 255).astype(np.uint8)
        return combined

    def composite(self, frame: np.ndarray, motion_mask: np.ndarray, seg_mask: Optional[np.ndarray] = None) -> np.ndarray:
        h, w = frame.shape[:2]
        ...
        out = self._foreground_only_sr(out, mask_up)

        if self.deblurrer is not None:
            out = self.deblurrer.deblur(out)

        if self.face_enhancer is not None:
            out = self.face_enhancer.enhance_faces(out)

        if self.color_enhancer is not None:
            out = self.color_enhancer.enhance_color(out)

        if self.object_detector is not None:
            detections = self.object_detector.detect(out)
            for det in detections:
                x, y, w, h = det["bbox"]
                conf = det["conf"]
                cv2.rectangle(out, (x, y), (x+w, y+h), (0, 0, 255), 1)
                cv2.putText(out, f"{conf:.2f}", (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
        ...
        if self.face_tracker is not None:
            ...
            tracks = self.face_tracker.update(list(faces))
            for t in tracks:
                x, y, w, h = t["bbox"]
                cv2.rectangle(out, (x, y), (x+w, y+h), (0, 255, 0), 1)
                cv2.putText(out, f"ID {t['id']}", (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)

        if self.dissector is not None:
            regions = self.dissector.dissect(out, motion_mask, seg_mask)
            for r in regions:
                x, y, w, h = r["bbox"]
                tag = r["tag"]
                if tag == "motion":
...
class StaticBackgroundUpscalerPipeline:
    def __init__(
        ...
        self.borg_manager = BorgReadAheadManager(max_bytes=1_000_000_000)
        self.nvme_buffer = NVMEScratchpadBuffer(
            base_dir=config.nvme_base_dir,
        ...
        self.deblurrer: Optional[MotionDeblurrerAI] = None
        self.face_tracker: Optional[FaceTracker] = None
        self.dissector: Optional[PictureDissectorAI] = None
        self.color_enhancer: Optional[ColorEnhancerAI] = None
        self.object_detector: Optional[ObjectDetectorAI] = None
    ...
    def initialize(self, vin: VideoInput) -> None:
        ...
        self.bg_modeler = BackgroundModeler(self.config, bg_ai=self.bg_ai, bg_refiner=self.bg_refiner, borg_manager=self.borg_manager, nvme_buffer=self.nvme_buffer)
        ...
        self.motion_detector = MotionDetector(self.config, segmenter_name=self.segmenter_name, rvm_segmenter=self.rvm_segmenter, borg_manager=self.borg_manager, nvme_buffer=self.nvme_buffer)
        ...
        self.deblurrer = MotionDeblurrerAI(downloader=self.downloader, ai_level=self.ai_level)
        self.face_tracker = FaceTracker()
        self.dissector = PictureDissectorAI()
        self.color_enhancer = ColorEnhancerAI(downloader=self.downloader, ai_level=self.ai_level)
        self.object_detector = ObjectDetectorAI(downloader=self.downloader, ai_level=self.ai_level)
        ...
        log(f"NVMe scratchpad: dir={self.config.nvme_base_dir}, max_bytes={self.config.nvme_max_bytes}")
        ...
        self.compositor = Compositor(
            ...
            deblurrer=self.deblurrer,
            face_tracker=self.face_tracker,
            dissector=self.dissector,
            color_enhancer=self.color_enhancer,
            object_detector=self.object_detector,
            sr_model=self.bg_upscaler,
        )
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.borg_manager is not None:
            ...
            self.borg_manager.add_raw_frame(frame)
        if self.nvme_buffer is not None:
            self.nvme_buffer.add_frame(frame, tag="frame")

        if self.executor:
            future_motion = self.executor.submit(self.motion_detector.compute_motion_mask, frame)
            motion_mask, seg_mask = future_motion.result()
            self.bg_modeler.update_background_slow(frame, motion_mask)
            future_comp = self.executor.submit(self.compositor.composite, frame, motion_mask, seg_mask)
            enhanced = future_comp.result()
        else:
            motion_mask, seg_mask = self.motion_detector.compute_motion_mask(frame)
            self.bg_modeler.update_background_slow(frame, motion_mask)
            enhanced = self.compositor.composite(frame, motion_mask, seg_mask)

        enhanced_stable = self.temporal_stabilizer.stabilize(enhanced, frame)
        return frame, enhanced_stable


def auto_select_profile(fps: float, w: int, h: int, is_stream: bool) -> UpscaleConfig:
    pixels = w * h
    if is_stream:
        if pixels <= 640 * 480:
...
class UpscalerGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Borg AI Video Lab (CPU-Only, RAM+Config NVMe, RVM+ESRGAN+SwinIR+CodeFormer+MPRNet+Color+Objects+Foreground-SR)")
        ...
        self.mprnet_sites_list = QListWidget()
        self.mprnet_sites_list.setMaximumHeight(80)
        self.colornet_sites_list = QListWidget()
        self.colornet_sites_list.setMaximumHeight(80)
        self.yolo_sites_list = QListWidget()
        self.yolo_sites_list.setMaximumHeight(80)
    ...
    def _build_ui(self):
        ...
        self.nvme_dir_edit = QLineEdit("scratchpad_nvme")
        self.nvme_dir_edit.setPlaceholderText("NVMe scratchpad directory")

        self.nvme_size_edit = QLineEdit("10000000000")
        self.nvme_size_edit.setPlaceholderText("NVMe max bytes (e.g., 10000000000)")
        ...
        nvme_row = QHBoxLayout()
        nvme_row.addWidget(QLabel("NVMe dir"))
        nvme_row.addWidget(self.nvme_dir_edit)
        nvme_row.addWidget(QLabel("NVMe bytes"))
        nvme_row.addWidget(self.nvme_size_edit)
        controls_layout.addLayout(nvme_row)
        ...
        scan_mprnet_btn = QPushButton("Scan MPRNet ONNX Models")
        scan_mprnet_btn.clicked.connect(self.scan_mprnet_sites)

        scan_colornet_btn = QPushButton("Scan ColorNet ONNX Models")
        scan_colornet_btn.clicked.connect(self.scan_colornet_sites)

        scan_yolo_btn = QPushButton("Scan YOLO ONNX Models")
        scan_yolo_btn.clicked.connect(self.scan_yolo_sites)
        ...
        dl_mprnet_selected_btn = QPushButton("Download selected MPRNet ONNX Model")
        dl_mprnet_selected_btn.clicked.connect(self.download_selected_mprnet)

        dl_colornet_selected_btn = QPushButton("Download selected ColorNet ONNX Model")
        dl_colornet_selected_btn.clicked.connect(self.download_selected_colornet)

        dl_yolo_selected_btn = QPushButton("Download selected YOLO ONNX Model")
        dl_yolo_selected_btn.clicked.connect(self.download_selected_yolo)
        ...
        mirror_layout.addWidget(scan_mprnet_btn)
        mirror_layout.addWidget(self.mprnet_sites_list)
        mirror_layout.addWidget(dl_mprnet_selected_btn)

        mirror_layout.addWidget(scan_colornet_btn)
        mirror_layout.addWidget(self.colornet_sites_list)
        mirror_layout.addWidget(dl_colornet_selected_btn)

        mirror_layout.addWidget(scan_yolo_btn)
        mirror_layout.addWidget(self.yolo_sites_list)
        mirror_layout.addWidget(dl_yolo_selected_btn)
    ...
    def scan_mprnet_sites(self):
        self.mprnet_sites_list.clear()
        sites = self.mirror_scanner.scan_mprnet_deblur_sites()
        for s in sites:
            self.mprnet_sites_list.addItem(QListWidgetItem(s))

    def scan_colornet_sites(self):
        self.colornet_sites_list.clear()
        sites = self.mirror_scanner.scan_colornet_sites()
        for s in sites:
            self.colornet_sites_list.addItem(QListWidgetItem(s))

    def scan_yolo_sites(self):
        self.yolo_sites_list.clear()
        sites = self.mirror_scanner.scan_yolo_sites()
        for s in sites:
            self.yolo_sites_list.addItem(QListWidgetItem(s))
    ...
    def download_selected_mprnet(self):
        item = self.mprnet_sites_list.currentItem()
        if item:
            url = item.text()
            path = self.downloader.download_from_url(url, "models")
            self.log_widget.append(f"Downloaded MPRNet ONNX model at: {path}")

    def download_selected_colornet(self):
        item = self.colornet_sites_list.currentItem()
        if item:
            url = item.text()
            path = self.downloader.download_from_url(url, "models")
            self.log_widget.append(f"Downloaded ColorNet ONNX model at: {path}")

    def download_selected_yolo(self):
        item = self.yolo_sites_list.currentItem()
        if item:
            url = item.text()
            path = self.downloader.download_from_url(url, "models")
            self.log_widget.append(f"Downloaded YOLO ONNX model at: {path}")
    ...
    def start_processing(self):
        ...
        nvme_dir = self.nvme_dir_edit.text().strip() or "scratchpad_nvme"
        try:
            nvme_size = int(self.nvme_size_edit.text().strip())
        except ValueError:
            nvme_size = 10_000_000_000
        ...
        config.nvme_base_dir = nvme_dir
        config.nvme_max_bytes = nvme_size
