...
LOG_PATH = os.path.join(BASE_DIR, "core_isolation_log.jsonl")
...
# Auto-repair mode can be gated by manual override
AUTO_REPAIR_MODE_DEFAULT = False  # default: off, requires manual override in GUI
...
def save_snapshot_backup(path: str = SNAPSHOT_BACKUP_PATH):
    try:
        snap = get_security_snapshot()
...
class SecurityGovernorGUI:
    ...
    def toggle_hvci(self):
        ...
        if messagebox.askyesno("Confirm", msg):
            ok_backup, err_backup = save_snapshot_backup(SNAPSHOT_BACKUP_HVCI_PATH)
            if not ok_backup:
    ...
    def remediate_drivers(self):
        ...
        if AUTO_REPAIR_REQUIRE_MANUAL_OVERRIDE and not self.manual_override_var.get():
            messagebox.showwarning(
                ...
                "Enable the checkbox before attempting driver auto-remediation."
            )
            return
        ...
        if not self.auto_repair_var.get():
            messagebox.showinfo(
                ...
                "Auto-Repair Mode is currently disabled.\n\n"
        ...
        # Backup snapshot before any remediation
        ok_backup, err_backup = save_snapshot_backup(SNAPSHOT_BACKUP_PATH)
        if not ok_backup:
            if not messagebox.askyesno(
                "Snapshot Backup Failed",
                "Failed to save security snapshot backup before remediation.\n\n"
                f"Error: {err_backup}\n\nProceed with auto-repair anyway?"
            ):
                return
        else:
            messagebox.showinfo(
                ...
                "Proceeding with heuristic driver auto-remediation."
            )
...
def main():
    # CLI override: --json for snapshot, --auto-repair to force auto-repair mode
    if len(sys.argv) > 1:
        if sys.argv[1] == "--json":
            print_json_snapshot()
            return
