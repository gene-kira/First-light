#!/usr/bin/env python
# Codex Purge Shell v4.5 (Safe Edition) — Win11 Security Bridge & Adjustment Console
# - Auto-elevation
# - Suricata v6 (file-tail + UDP socket config stub)
...
# - v4.5: better threat scoring, richer swarm view, more detailed meta in snapshots
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4.5] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4_5"
        else:
            self.model = None
...
class SwarmSyncManager:
    ...
    def _simulate_peers(self):
        # Simple simulated peers for visualization
        self.peers = {
            "node-alpha": {
...
# =========================
# Main GUI / Codex Purge Shell v4.5
# =========================

class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4.5 (Safe Edition) — Win11 Security Bridge & Adjustment Console")
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4.5 (Safe Edition) — Windows 11 Security Bridge & Adjustment Console",
            font=("Arial", 16)
        )
        ...
        self.log_alert("Codex Purge Shell v4.5 (Safe Edition) started with delta snapshots, extended scoring, and swarm peer simulation.")
