...
#     - threats, probes, swarm, telemetry, rules, consensus, chains, events
#     - UI/engine settings (AI mode, Borg mode, window geometry, last tab)
...
class Threat:
    ...
    def from_snapshot(data):
        ...
        t.assimilated = data.get("assimilated", False)
...
class SwarmManager:
    ...
    def _restore_from_history(self):
        if not self.history:
            return
        last = self.history[-1]
        nodes_data = last.get("nodes", [])
        restored = []
        for nd in nodes_data:
            restored.append(SwarmNode.from_snapshot(nd))
        if restored:
            self.nodes = restored
...
class Probe:
    ...
    @staticmethod
    def from_snapshot(data):
        p = Probe(data.get("probe_id"))
        p.status = data.get("status", "IDLE")
        p.sector = data.get("sector", "N/A")
        p.reading = data.get("reading", 0.0)
...
class ProbeManager:
    ...
    def _restore_from_history(self):
        if not self.history:
            return
        last = self.history[-1]
        probes_data = last.get("probes", [])
        restored = []
        for pd in probes_data:
            restored.append(Probe.from_snapshot(pd))
        if restored:
            self.probes = restored
...
class GameTelemetryManager:
    ...
    def _restore_from_history(self):
        if not self.history:
            return
        last = self.history[-1]
        self.last_load_factor = last.get("load_factor", 0.0)
        self.last_snapshot = last.get("heavy_procs", [])
        self.gpu_curve = [(h.get("timestamp"), h.get("gpu")) for h in self.history if "gpu" in h]
...
class TargetingGUI:
    def __init__(self, root):
        ...
        self.root.geometry(self.settings.get("window_geometry", "1450x900"))
        ...
        # Restore AI/Borg settings
        self.engine.ai_enabled = self.settings.get("ai_enabled", True)
        self.engine.borg_mode = self.settings.get("borg_mode", False)
    ...
    def _restore_last_tab(self):
        last_tab = self.settings.get("last_tab", "Overview")
    ...
    def _on_close(self):
        # Save geometry and engine modes
        self.settings["window_geometry"] = self.root.geometry()
        self.settings["ai_enabled"] = self.engine.ai_enabled
        self.settings["borg_mode"] = self.engine.borg_mode
        save_settings(self.settings)
        self.root.destroy()
    ...
    def build_targeting_panel(self, parent):
        ...
        self.ai_var = tk.BooleanVar(value=self.settings.get("ai_enabled", True))
        tk.Checkbutton(frame, text="AI-Controlled Advisor Mode",
                       variable=self.ai_var,
                       command=self.toggle_ai).pack(anchor="w")

        self.borg_var = tk.BooleanVar(value=self.settings.get("borg_mode", False))
    ...
    def build_game_panel(self, parent):
        ...
        self.game_load_label = tk.Label(btn_frame, text=f"LoadFactor: {self.game_telemetry.last_load_factor:.3f}")
