...
# -*- coding: utf-8 -*-
"""
CODEX CONTROL CONSOLE v7.6 (Windows Edition)
AI-driven, Suricata-aware, MITRE-mapped, swarm-enabled SOC brain
with real training mode, compact SOC-style GUI, and Math Reasoning Suite.

Major upgrades vs v7.5:
- MathReasoningSuite: integration, geometry, multiplication, division, percentage, algebra
- Math-driven anomaly shaping (integral-based density, geometric drift)
- Dual-path reasoning (old vs new math, mathematician vs engineer)
- Hooks for future visual math panels in GUI
...
class MathReasoningSuite:
    """
    Encodes the math styles from your images:
    - Area by integration
    - Triangle geometry (school vs university)
    - Old vs new multiplication
    - USA vs China division
    - Percentage simplification
    - Mathematician vs engineer algebra
    Used to shape anomaly and drift scores and provide dual-path reasoning.
    """
    def __init__(self):
        self.enabled = True

    def area_under_curve_constant(self, value: float, a: float, b: float) -> float:
        # Area = ∫_a^b value dx = value * (b - a)
        return value * (b - a)

    def triangle_pythagoras(self, a: float, b: float) -> float:
        # School math: x^2 = a^2 + b^2
        return (a**2 + b**2) ** 0.5

    def triangle_university_style(self, a: float, b: float) -> float:
        # University math: same result, different path
        # x = sqrt( (a^2 + b^2) ) but we can factor or scale
        return (a**2 + b**2) ** 0.5

    def old_vs_new_multiplication(self, n: int, m: int) -> Dict[str, int]:
        # Old math: direct product
        old = n * m
        # New math: break into place values
        digits = [int(d) for d in str(n)]
        # approximate: sum(d * m * 10^(pos)) but keep simple
        new = 0
        for i, d in enumerate(reversed(digits)):
            new += d * m * (10**i)
        return {"old": old, "new": new}

    def division_usa_china(self, n: int, d: int) -> Dict[str, float]:
        # USA: long division result
        usa = n / d
        # China: quick transform (e.g., multiply numerator/denominator)
        # For 280/5 -> 28*2 = 56; we mimic a shortcut
        china = (n / 10) * 2 if d == 5 else n / d
        return {"usa": usa, "china": china}

    def percentage_simplification(self, percent: float, value: float) -> float:
        # 8% of 500 -> 0.08 * 500
        return (percent / 100.0) * value

    def algebra_mathematician(self, x_value: float) -> float:
        # 8(x+5)=56 -> x=2; we encode the style as a check
        # 8x + 40 = 56 -> 8x = 16 -> x = 2
        return x_value

    def algebra_engineer(self, x_value: float) -> float:
        # 8(x+5)/8 = 56/8 -> x+5=7 -> x=2
        return x_value

    def shape_anomaly_with_integral(self, anomaly: float) -> float:
        # Treat anomaly as a constant function over [1,2]
        # area = anomaly * (2-1) = anomaly
        # but we can scale it to emphasize high anomalies
        area = self.area_under_curve_constant(anomaly, 1.0, 2.0)
        return area * 1.1

    def shape_drift_with_geometry(self, drift: float) -> float:
        # Use triangle geometry as a metaphor: hyp = sqrt(drift^2 + drift^2)
...
class AIBrain:
    ...
    def infer(self, data, target=None):
        ...
        # Math shaping: integral and geometry
...
class CodexGUI:
    def __init__(self):
        ...
        self.root.title("Codex Control Console v7.6 (Windows)")
...
def run_codex_daemon():
    ...
    print("[CORE] Codex Control Console daemon v7.6 (Windows: TRAIN + eve.json + PCAP + MITRE + COMPACT GUI + SWARM + RESPONSE + THRESHOLD + SURICATA_CFG + CAMPAIGNS + MATH_SUITE)")
    def loop():
        while True:
            daemon_cycle(node_id, console, brain, backbone, gpu_telemetry, swarm, mitre, consensus, responder, campaign_engine)
            time.sleep(5)
    wd = Watchdog("CodexDaemonV7_6_Windows", loop, restart_delay=3.0)
    if gui and gui.root:
