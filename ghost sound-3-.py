...
SUSPICIOUS_PORTS = [
    3389,   # RDP
    5938,   # TeamViewer
    7070,   # AnyDesk (example)
    47984,  # Parsec (example)
]
...
class SecurityBridgeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Security Bridge - Audio, Network, File, Honeypot, Auto-Block")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            control_frame,
            text="Security Bridge - Full Upgrade (Audio + Network + File + Honeypot + Auto-Block)",
...
def main():
    log_event("INFO", "Security Bridge (Full Upgrade v2) started.")
    start_honeypot()
    root = tk.Tk()
