...
- ZMQ "port stacking": first process binds, others connect to same port
...
# ---------------------------------------------------------------------------
# LAN/WAN Swarm via ZeroMQ (manual start, port stacking)
# ---------------------------------------------------------------------------
...
class ZMQSwarm:
    ...
    def _loop(self):
        ...
        try:
            ...
            # Use localhost for client connect
    ...
    def _client_loop(self, socket):
        # Simple heartbeat client; could be extended to send jobs to the server.
        while not self._stop_event.is_set():
            try:
...
class SwarmGUI(QtWidgets.QMainWindow):
    def __init__(self, swarm: BeastSwarm, tuner: SwarmTuner, queen: Queen, memory: MemoryManager, zmq_swarm: ZMQSwarm):
        ...
        self.setWindowTitle("Beast Swarm Engine v6.1 - Real-Time Queen")
