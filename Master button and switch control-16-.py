#!/usr/bin/env python
# Codex Purge Shell v4.10 (Safe+Hardening Edition) — Win11 Security Bridge & Control Nexus
# - Auto-elevation
# - Suricata v6 (file-tail, richer event meta)
# - GPU DL stub threat scoring (extended features)
# - Agentic reasoning (suggest + optional auto-hardening)
...
# - Correlation engine (process + Suricata + honeypot), anomaly scoring stub, correlation tab
# - v4.8: reboot-persistent config (codex_config.json), green Suricata status, green Persona Sentinel
# - v4.9: Windows Control Nexus tab (unified catalog of services, tasks, adapters, power, update, Defender, telemetry keys)
# - v4.10: Auto-hardening engine (optional, controlled via config/UI) + Snapshot diff viewer (registry/firewall/profile deltas)
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4.10] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass

def load_snapshot_meta(folder):
    meta_path = os.path.join(folder, "codex_meta.json")
...
class SuricataEventIngestor:
    def __init__(self, log_path=None, udp_port=None):
        ...
        self.udp_port = udp_port or 5514
        self.running = False
        self.thread = None
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4_10"
        else:
            self.model = None
...
class CorrelationEngine:
    def __init__(self, log_fn=None, timeline_callback=None):
        self.log = log_fn
    ...
    def add_event(self, kind, data):
        ...
        if self.timeline_callback:
            self.timeline_callback(evt)

    def correlate(self):
        correlations = []
        procs = [e for e in self.events if e["kind"] == "process"]
        surs = [e for e in self.events if e["kind"] == "suricata"]
        hones = [e for e in self.events if e["kind"] == "honeypot"]

        for p in procs:
            pname = (p["data"].get("name") or "").lower()
            pid = p["data"].get("pid")
            pscore = p["data"].get("score", 0.0)
            for s in surs:
                sig = (s["data"].get("signature") or "").lower()
                sscore = s["data"].get("score", 0.0)
                if any(k in pname for k in ["powershell", "cmd", "python"]) and ("cnc" in sig or "trojan" in sig):
                    correlations.append({
                        "type": "proc_suricata",
                        "process_name": pname,
                        "pid": pid,
                        "signature": sig,
                        "combined_score": (pscore + sscore) / 2.0,
                        "time": max(p["time"], s["time"]),
                    })
        for h in hones:
            src_ip = h["data"].get("src_ip")
            hscore = h["data"].get("score", 0.0)
            for s in surs:
                sig = (s["data"].get("signature") or "").lower()
                sscore = s["data"].get("score", 0.0)
                if src_ip and ("scan" in sig or "cnc" in sig):
                    correlations.append({
                        "type": "honeypot_suricata",
                        "src_ip": src_ip,
                        "signature": sig,
                        "combined_score": (hscore + sscore) / 2.0,
                        "time": max(h["time"], s["time"]),
                    })
        return correlations
    ...
    def anomaly_score(self):
        ...
        return min(1.0, avg + 0.05 * high)

class AutoHardeningEngine:
    def __init__(self, adjustment_engine, log_fn, enabled=False):
    ...
    def set_enabled(self, enabled: bool):
        self.enabled = enabled
        self.log(f"[Hardening] Auto-hardening {'ENABLED' if enabled else 'DISABLED'}.")

    def maybe_harden_process(self, decision):
        if not self.enabled:
            return
        score = decision.get("score", 0.0)
        name = (decision.get("name") or "").lower()
        pid = decision.get("pid")

        if score > 0.9:
            self.log(f"[Hardening] High-risk process detected: {name} (PID={pid}) score={score:.2f}. Suggesting stop.")
            try:
                p = psutil.Process(pid)
                p.terminate()
                self.log(f"[Hardening] Process terminated: {name} (PID={pid})")
            except Exception:
                self.log(f"[Hardening] Failed to terminate process: {name} (PID={pid})")

    def maybe_harden_suricata(self, decision):
        if not self.enabled:
            return
        score = decision.get("score", 0.0)
        sig = (decision.get("signature") or "").lower()
        remediation = decision.get("remediation", [])

        if score > 0.9 and "suggest_block_ip" in remediation:
            self.log(f"[Hardening] High-risk Suricata event: {sig} score={score:.2f}. Auto-block suggestion (manual firewall rule).")

class AgenticReasoner:
    def __init__(self, scorer, ndis_plane, correlation_engine, hardening_engine, timeline_callback=None):
        self.scorer = scorer
        self.ndis = ndis_plane
        self.decisions = []
        self.corr = correlation_engine
        self.timeline_callback = timeline_callback
        self.hardening = hardening_engine
    ...
    def evaluate_process(self, proc_info, conn_count, resurrected=False):
        ...
        self.corr.add_event("process", decision)
        ...
        self.hardening.maybe_harden_process(decision)
        return decision
    ...
    def evaluate_suricata_event(self, evt):
        ...
        self.decisions.append(decision)
        self.corr.add_event("suricata", decision)
        self._record_timeline({
            "kind": "suricata",
            "signature": evt.get("alert", {}).get("signature"),
            "score": score,
        ...
        self.hardening.maybe_harden_suricata(decision)
        return decision
    ...
    def evaluate_honeypot_summary(self, summary):
        ...
        self.decisions.append(decision)
        self.corr.add_event("honeypot", decision)
        self._record_timeline({
            "kind": "honeypot",
            "src_ip": summary.get("src_ip"),
            "score": base,
...
class WindowsAdjustmentEngine:
    ...
    def list_firewall_rules(self):
        out = safe_call(["netsh", "advfirewall", "firewall", "show", "rule", "name=all"])
        return out
...
class ControlNexusPanel:
    def __init__(self, parent, log_fn, adjustment_engine):
        self.parent = parent
        self.log = log_fn
        self.engine = adjustment_engine

        frame = ttk.LabelFrame(parent, text="Windows Control Nexus (Unified Catalog)")
        frame.pack(fill="both", expand=True, padx=5, pady=5)

        top = ttk.Frame(frame)
        top.pack(fill="x", pady=2)

        ttk.Label(top, text="Search / Filter:").pack(side="left", padx=5)
        ...
        ttk.Button(top, text="Refresh All", command=self.refresh_all).pack(side="right", padx=5)
        ...
        ttk.Button(btns, text="Start Service", command=lambda: self._service_action("Start")).pack(side="left", padx=3)
        ttk.Button(btns, text="Stop Service", command=lambda: self._service_action("Stop")).pack(side="left", padx=3)
        ttk.Button(btns, text="Restart Service", command=lambda: self._service_action("Restart")).pack(side="left", padx=3)

        ttk.Button(btns, text="Enable Task", command=lambda: self._task_change(True)).pack(side="left", padx=3)
        ttk.Button(btns, text="Disable Task", command=lambda: self._task_change(False)).pack(side="left", padx=3)

        ttk.Button(btns, text="Enable Adapter", command=lambda: self._adapter_change(True)).pack(side="left", padx=3)
        ttk.Button(btns, text="Disable Adapter", command=lambda: self._adapter_change(False)).pack(side="left", padx=3)

        ttk.Button(btns, text="Pause Updates", command=self.engine.pause_updates).pack(side="left", padx=3)
        ttk.Button(btns, text="Resume Updates", command=self.engine.resume_updates).pack(side="left", padx=3)

        ttk.Button(btns, text="Enable Defender RT", command=lambda: self.engine.toggle_defender_realtime(True)).pack(side="left", padx=3)
        ttk.Button(btns, text="Disable Defender RT", command=lambda: self.engine.toggle_defender_realtime(False)).pack(side="left", padx=3)

        ttk.Button(btns, text="Apply Telemetry Suppression", command=self._apply_telemetry).pack(side="right", padx=3)
    ...
    def _load_services(self):
        ...
        for svc in services:
            ...
            self.tree.insert("", "end", values=("Service", name, status, f"StartType={starttype}"))
    ...
    def _load_tasks(self):
        ...
        for t in tasks:
            name = t.get("TaskName")
            state = t.get("State")
            self.tree.insert("", "end", values=("Task", name, state, ""))

    def _load_adapters(self):
        adapters = self.engine.list_network_adapters()
        if isinstance(adapters, dict):
            adapters = [adapters]
        for a in adapters:
            name = a.get("Name")
            status = a.get("Status")
            self.tree.insert("", "end", values=("Adapter", name, status, ""))

    def _load_power(self):
        out = self.engine.list_power_plans()
        for line in out.splitlines():
            line = line.strip()
            if not line:
                continue
            self.tree.insert("", "end", values=("PowerPlan", "", "", line))

    def _load_update(self):
        self.tree.insert("", "end", values=("Update", "wuauserv", "Managed via service", "Pause/Resume buttons"))

    def _load_defender(self):
        self.tree.insert("", "end", values=("Defender", "Real-time Monitoring", "Toggle via buttons", ""))

    def _load_telemetry_keys(self):
        for key in GPO_TELEMETRY_KEYS:
            vals = read_registry_key(key)
            if vals:
                for name, value in vals:
                    self.tree.insert("", "end", values=("TelemetryKey", f"{key}\\{name}", value, ""))
            else:
                self.tree.insert("", "end", values=("TelemetryKey", key, "No values", ""))

    def _load_firewall_rules(self):
        out = self.engine.list_firewall_rules()
        for line in out.splitlines():
            line = line.strip()
            if not line:
                continue
            self.tree.insert("", "end", values=("FirewallRule", "", "", line))

    def _service_action(self, action):
        sel = self.tree.selection()
        if not sel:
            messagebox.showinfo("Service", "No item selected.")
            return
        vals = self.tree.item(sel[0], "values")
        if vals[0] != "Service":
            messagebox.showinfo("Service", "Selected item is not a Service.")
            return
        name = vals[1]
        self.engine.change_service_status(name, action)

    def _task_change(self, enable):
        sel = self.tree.selection()
        if not sel:
            messagebox.showinfo("Task", "No item selected.")
            return
        vals = self.tree.item(sel[0], "values")
        if vals[0] != "Task":
            messagebox.showinfo("Task", "Selected item is not a Task.")
            return
        name = vals[1]
        self.engine.change_task_state(name, enable)

    def _adapter_change(self, enable):
        sel = self.tree.selection()
        if not sel:
            messagebox.showinfo("Adapter", "No item selected.")
            return
        vals = self.tree.item(sel[0], "values")
        if vals[0] != "Adapter":
            messagebox.showinfo("Adapter", "Selected item is not an Adapter.")
            return
        name = vals[1]
        self.engine.change_adapter_status(name, enable)

    def _apply_telemetry(self):
        apply_telemetry_suppression(self.log)
        self.refresh_all()
...
class GUIWatchdog:
    ...
    def _build_manifest(self):
        ...
        self._register("correlation_text", self.app.correlation_text)
        self._register("control_nexus_tree", self.app.control_nexus_panel.tree)
        self._register("diff_tree", self.app.diff_tree)
...
class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4.10 (Safe+Hardening Edition) — Win11 Security Bridge & Control Nexus")
        ...
        self.correlation_engine = CorrelationEngine(log_fn=self._log_correlation_event)
        self.threat_scorer = ThreatScorer()
        self.hardening_engine = AutoHardeningEngine(
            adjustment_engine=None,
            log_fn=self._hardening_log,
            enabled=self.config.get("auto_hardening_enabled", False)
        )
        self.adjustment_engine = WindowsAdjustmentEngine(self.log_alert)
        self.hardening_engine.engine = self.adjustment_engine
        self.agent = AgenticReasoner(
            self.threat_scorer,
            self.ndis_plane,
            self.correlation_engine,
            self.hardening_engine,
            timeline_callback=self._add_timeline_entry
        )
        ...
        self.process_seen = {}
        self.persona = self.config.get("persona", "Sentinel")

        self.baseline_fw = None
        self.baseline_profiles = None
        self.baseline_settings = None
        self.last_snapshot_time = 0
        self.snapshot_cooldown = self.config.get("snapshot_cooldown", 3600)
        self.suricata_snapshot_enabled = self.config.get("suricata_snapshot_enabled", False)
        self.update_snapshot_enabled = self.config.get("update_snapshot_enabled", False)

        self.fw_change_counter = 0
        self.prof_change_counter = 0
        self.set_change_counter = 0
        self.persistence_threshold = self.config.get("persistence_threshold", 6)

        self.persona_color = self.config.get("persona_color", "green")
        self.suricata_color_idle = self.config.get("suricata_color_idle", "green")
        self.suricata_color_active = self.config.get("suricata_color_active", "green")
    ...
    def _hardening_log(self, msg):
        timestamp = time.strftime("[%H:%M:%S] ")
        self.alert_box.insert(tk.END, timestamp + msg + "\n")
    ...
    def request_snapshot(self, meta, reason="generic"):
        ...
        meta["correlation_anomaly_score"] = self.correlation_engine.anomaly_score()
        meta["correlation_count"] = len(self.correlation_engine.correlate())
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4.10 (Safe+Hardening Edition) — Windows 11 Security Bridge, Control Nexus & Diff Console",
            font=("Arial", 16)
        )
        ...
        self.status_suricata = ttk.Label(
            status_frame,
            text="Suricata: IDLE",
            foreground=self.suricata_color_idle
        )
        self.status_suricata.pack(side="left", padx=10)

        self.persona_label = ttk.Label(
            status_frame,
            text=f"Persona: {self.persona}",
            foreground=self.persona_color
        )
        self.persona_label.pack(side="left", padx=10)

        self.hardening_var = tk.BooleanVar(value=self.config.get("auto_hardening_enabled", False))
        hardening_check = ttk.Checkbutton(
            status_frame,
            text="Auto-Hardening",
            variable=self.hardening_var,
            command=self._toggle_hardening
        )
        hardening_check.pack(side="right", padx=10)
        ...
        diff_btn = ttk.Button(snapshot_frame, text="Show Diff vs Baseline", command=self.show_snapshot_diff)
        ...
        self.log_alert("Codex Purge Shell v4.10 (Safe+Hardening Edition) started with Control Nexus, auto-hardening, and snapshot diff viewer.")
        ...
        nexus_tab = ttk.Frame(left_notebook)
        left_notebook.add(nexus_tab, text="Control Nexus")

        self.control_nexus_panel = ControlNexusPanel(nexus_tab, self.log_alert, self.adjustment_engine)
        ...
        correlation_tab = ttk.Frame(left_notebook)
        left_notebook.add(correlation_tab, text="Correlation & Anomaly")

        self.correlation_text = tk.Text(correlation_tab, height=12, width=80, bg="#080808", fg="#00bfff")
        self.correlation_text.pack(fill="both", expand=True, padx=5, pady=5)

        diff_tab = ttk.Frame(left_notebook)
        left_notebook.add(diff_tab, text="Snapshot Diff")

        self.diff_tree = ttk.Treeview(
            diff_tab,
            columns=("component", "baseline", "current", "changed"),
            show="headings",
            height=12
        )
        self.diff_tree.heading("component", text="Component")
        self.diff_tree.heading("baseline", text="Baseline Hash")
        self.diff_tree.heading("current", text="Current Hash")
        self.diff_tree.heading("changed", text="Changed")
        self.diff_tree.column("component", width=160)
        self.diff_tree.column("baseline", width=260)
        self.diff_tree.column("current", width=260)
        self.diff_tree.column("changed", width=80)
        self.diff_tree.pack(fill="both", expand=True, padx=5, pady=5)

        diff_scroll = ttk.Scrollbar(diff_tab, orient="vertical", command=self.diff_tree.yview)
    ...
    def _toggle_hardening(self):
        enabled = self.hardening_var.get()
        self.hardening_engine.set_enabled(enabled)
        self.config["auto_hardening_enabled"] = enabled
    ...
    def _log_correlation_event(self, evt):
        pass

    def _refresh_correlation_view(self):
        correlations = self.correlation_engine.correlate()
        anomaly = self.correlation_engine.anomaly_score()
        self.correlation_text.delete("1.0", tk.END)
        self.correlation_text.insert(tk.END, f"Anomaly Score: {anomaly:.2f}\n\n")
        for c in correlations:
            ts = time.strftime("%H:%M:%S", time.localtime(c["time"]))
            if c["type"] == "proc_suricata":
                line = f"[{ts}] PROC+SUR: {c['process_name']} (PID={c['pid']}) <-> {c['signature']} combined={c['combined_score']:.2f}\n"
            elif c["type"] == "honeypot_suricata":
                line = f"[{ts}] HONEYPOT+SUR: src={c['src_ip']} <-> {c['signature']} combined={c['combined_score']:.2f}\n"
            else:
                line = f"[{ts}] {c}\n"
            self.correlation_text.insert(tk.END, line)
        self.correlation_text.see(tk.END)

    def _toggle_suricata_snapshots(self):
        self.suricata_snapshot_enabled = self.sur_snap_var.get()
        self.config["suricata_snapshot_enabled"] = self.suricata_snapshot_enabled
        save_config(self.config)
    ...
    def _toggle_update_snapshots(self):
        ...
        self.config["update_snapshot_enabled"] = self.update_snapshot_enabled
        save_config(self.config)
    ...
    def show_snapshot_diff(self):
        folder = self.snapshot_var.get()
        if not folder:
            messagebox.showinfo("Snapshot Diff", "No snapshot selected.")
            return
        meta = load_snapshot_meta(folder)
        baseline = meta.get("baseline_hashes", {})
        current = meta.get("current_hashes", {})
        delta = meta.get("delta", {})

        self.diff_tree.delete(*self.diff_tree.get_children())
        components = ["fw", "profiles", "settings"]
        labels = {
            "fw": "Firewall",
            "profiles": "Profiles",
        ...
        for comp in components:
            b = baseline.get(comp, "")
            c = current.get(comp, "")
            changed = "Yes" if b != c else "No"
            self.diff_tree.insert(
                "",
                "end",
                values=(labels[comp], b, c, changed)
            )
        self.log_alert(f"[Diff] Snapshot diff displayed for: {folder}")
    ...
    def _handle_suricata_event(self, evt):
        self.status_suricata.config(text="Suricata: ACTIVE", foreground=self.suricata_color_active)
