...
4. GUI status panel (optional, tkinter-based).
5. Daemonized watchdog loop for continuous monitoring.
6. Robust Optuna objective that never returns None and handles Sequence correctly.
...
class TemporalSequence:
    """
    Keras-compatible temporal augmentation sequence.

    Inherits from tf.keras.utils.Sequence so Keras/Optuna can consume it
    without "unrecognized data type" errors.
    """

    def __init__(self, np, X, y, batch_size, training=True):
        from tensorflow.keras.utils import Sequence as KSequence
        # Dynamically set base to avoid import at module top.
        class _BaseSequence(KSequence):
            pass

        # Monkey-patch class to actually inherit.
        self.__class__ = type(
