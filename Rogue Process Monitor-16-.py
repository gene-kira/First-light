...
# SAFE (manual) + Alerts + Borg + ML + GPU (optional) + Suricata v6-style rules + Kernel stubs + Agentic learning
...
DEFAULT_CONFIG = {
    # v17: always start in SAFE (manual) mode with alerts only
...
def load_config():
    global config
    # v17: force SAFE mode on startup regardless of saved config
    config = DEFAULT_CONFIG.copy()
    ...
    try:
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        # force SAFE mode even if file says otherwise
        data["mode"] = "SAFE"
        with config_lock:
            config.update(data)
        log("Config loaded (mode forced SAFE/manual).")
    except Exception as e:
...
class BorgQueen:
        ...
        with self.results_lock:
            for r in worker_results:
                ...
                if r["history"] is not None:
                    sandbox_append_history(r["history"])
                if r["alert"] is not None:
                    sandbox_add_alert(r["alert"])
    ...
    def self_heal_and_distribute(self, procs):
        ...
        if n < worker_count * 10:
            worker_count = max(self.min_worker_count, n // 10 or 1)

        chunk_size = max(1, (n + worker_count - 1) // worker_count)
    ...
    def finalize_cycle(self):
        ...
        TimelineEngine.build_timeline(history, threat_level)
        AIInsightEngine.generate_insights(history, threat_level)
...
class RogueMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Rogue Process Monitor v17.0 (SAFE/manual, Borg + ML + GPU + Suricata + Kernel Stub + Agentic)")
    ...
    def _build_overview_tab(self):
        ...
        title = ttk.Label(frame, text="Rogue Process Monitor v17.0 (SAFE/manual, Borg + ML + GPU + Suricata + Kernel Stub + Agentic)", font=("Segoe UI", 14, "bold"))
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))

        desc = ttk.Label(
            frame,
            text=(
                "SAFE/manual mode by default: alerts only, no automatic kill/quarantine.\n"
        ...
        self.status_label = ttk.Label(
            frame,
            text=f"Status: starting... | Mode={mode} (manual SAFE) | ML={ml_status} | GPU={gpu_status} | Plots={plot_status}",
            foreground="#00aa00"
        )
    ...
    def _build_actions_tab(self):
        ...
        desc = ttk.Label(
            ...
            text=(
                ...
                "  SAFE: manual, no destructive actions (default).\n"
    ...
    def _update_status(self, text):
        ...
        self.status_label.config(
            text=f"Status: {text} | Mode={mode} (manual SAFE) | ML={ml_status} | GPU={gpu_status} | Plots={plot_status}",
            foreground="#00aa00"
        )
    ...
    def _schedule_status_refresh(self):
        ...
        # v17: slower GUI refresh for performance
    ...
    def _schedule_tree_refresh(self):
        ...
        self.root.after(60000, self._schedule_tree_refresh)

    def _schedule_meter_refresh(self):
        self._refresh_threat_meter()
        self.root.after(10000, self._schedule_meter_refresh)
...
def main():
    log("Rogue Process Monitor v17.0 (SAFE/manual, Borg + ML + GPU + Suricata + Kernel Stub + Agentic) starting")
    root = tk.Tk()
