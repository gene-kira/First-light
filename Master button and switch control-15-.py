#!/usr/bin/env python
# Codex Purge Shell v4.9 (Safe Edition) — Win11 Security Bridge & Control Nexus
# - Auto-elevation
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4.9] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4_9"
        else:
            self.model = None
...
class AgenticReasoner:
    def __init__(self, scorer, ndis_plane, correlation_engine, timeline_callback=None):
        self.scorer = scorer
...
class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4.9 (Safe Edition) — Win11 Security Bridge & Control Nexus")
        ...
        self.agent = AgenticReasoner(self.threat_scorer, self.ndis_plane, self.correlation_engine, timeline_callback=self._add_timeline_entry)
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4.9 (Safe Edition) — Windows 11 Security Bridge, Control Nexus & Correlation Console",
            font=("Arial", 16)
        )
        ...
        self.log_alert("Codex Purge Shell v4.9 (Safe Edition) started with Control Nexus and reboot-persistent config.")
