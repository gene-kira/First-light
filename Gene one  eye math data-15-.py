...
# -*- coding: utf-8 -*-
"""
CODEX CONTROL CONSOLE v7.8 (Windows Edition)
...
and NEW Borg Swarm Tech (Queens + Workers).

Major upgrades vs v7.7:
- Borg Swarm:
  - Queen nodes and Worker nodes
  - Hierarchical consensus (Queen-led, Worker-voted)
  - Borg tasks: SCAN, WATCH, DEFEND, LURE
  - Borg role awareness in swarm messages
  - Queen can reassign roles and adjust trust
- Everything from v7.7 retained:
...
class ResponseEngine:
    ...
    def apply(self, decision: str, mitre_entries: List[MITREEntry], anomaly: float, drift: float, ae_err: float, isolation_score: float):
        max_sev = max((e.severity for e in mitre_entries), default=0)
        if decision != "ALERT":
            print("[RESPONSE] Decision not ALERT, no action")
            return

        print(f"[RESPONSE] Decision=ALERT Sev={max_sev} Anom={anomaly:.3f} Drift={drift:.3f} AE={ae_err:.3f} Iso={isolation_score:.3f}")

        if max_sev < SEVERITY_THRESHOLD:
            print(f"[RESPONSE] Severity {max_sev} < threshold {SEVERITY_THRESHOLD}, logging only")
            return
...
class CodexGUI:
    def __init__(self):
        ...
        self.root.title("Codex Control Console v7.8 (Windows)")
    ...
    def update_main(self, decision: str, consensus: str, anomaly: float, drift: float, ae_err: float, iso_score: float, gpu: Dict[str, float], max_sev: int):
        if not self.root:
            return
...
def run_codex_daemon():
    ...
    print("[CORE] Codex Control Console daemon v7.8 (Windows: TRAIN + eve.json + PCAP + MITRE + COMPACT GUI + SWARM + BORG + RESPONSE + THRESHOLD + SURICATA_CFG + CAMPAIGNS + MATH_SUITE + DPI + HONEYPOT)")
    def loop():
        while True:
            daemon_cycle(node_id, console, brain, backbone, gpu_telemetry, swarm, mitre, consensus, responder, campaign_engine)
            time.sleep(5)
    wd = Watchdog("CodexDaemonV7_8_Windows", loop, restart_delay=3.0)
    if gui and gui.root:
