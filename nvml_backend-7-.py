...
# -*- coding: utf-8 -*-
"""
Universal GPU Control Center v4.4
Advanced Control + Elevated Privileges Edition

Upgrades over v4.3:
- Advanced control:
  - PID-based thermal controller (per-GPU)
  - Fan curve editor (GUI + config)
  - Power curve editor (GUI + config)
  - Profile presets: gaming / compute / silent / custom
  - Process-aware tuning (heavier when GPU is busy)
- Elevated privileges helper:
  - Detects non-admin on Windows and can relaunch itself elevated
  - NVML write operations (power limit) assume elevated context
- Smarter auto mode:
  - Combines prediction + PID + fan/power curves
- REST API extensions:
  - /profiles/preset
  - /curves/fan
  - /curves/power
...
Safety:
- No VBIOS writes, no strap edits, no voltage hacks.
- Only documented NVML APIs, range-checked.
"""
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "safety": {
        ...
        "thermal_limit_c": 85,
        "emergency_temp_c": 92,
        "allow_vendor_writes": True,  # assumes elevated privileges
...
class ThermalPIDController:
    ...
    def compute(self, gpu_index: int, current_temp: Optional[float]) -> float:
        ...
        return output  # positive -> cooler (more fan / less power), negative -> warmer
...
class TuningEngine:
    ...
    def apply_auto_tuning(self, telemetry_engine: TelemetryEngine) -> None:
        if self.nvml_backend is None:
            return
        if not CONFIG["safety"]["allow_vendor_writes"]:
            return
        ...
        overrides = self.cfg.get("gpu_overrides", {})
        hist = telemetry_engine.get_history()
...
class ProfileManager:
    ...
    def auto_tick(self) -> None:
        ...
        if now - self._last_auto_switch_time < hysteresis:
            return
        self.tuning_engine.apply_auto_tuning(self.telemetry_engine)
...
def start_governor_runtime() -> None:
    ...
    tuning_engine = TuningEngine(CONFIG.get("tuning", {}), nvml_backend, prediction_engine, thermal_pid)
