#!/usr/bin/env python3
# guardian_core.py (regenerated, upgraded)
#
...
# - Full Suricata rule import (enhanced parser, in-memory rule engine)
...
#     * Fake SSH (Cowrie-style)
#     * Fake HTTP (Cowrie-style)
...
#     * Distributed mesh routing (enhanced)
...
# - GUI dashboard with:
#   * Threats pane at top (with audio/visual indicators)
...
# - NDIS packet interception layer (stub-safe)
# - Kernel-level telemetry suppression (stub-safe)
...
DB_PATH = "guardian.db"
SETTINGS_PATH = "guardian_settings.json"
...
DEFAULT_CONFIG = {
    ...
    "agentic_triage_enabled": True,
    "zero_trust_enforcement_enabled": True,
    "ndis_layer_enabled": False,
...
class GuardianConfig:
    def __init__(self):
        ...
        self.agentic_triage_enabled = self.config.get("agentic_triage_enabled", True)
        self.zero_trust_enforcement_enabled = self.config.get("zero_trust_enforcement_enabled", True)
        self.ndis_layer_enabled = self.config.get("ndis_layer_enabled", False)
    ...
    def _apply_settings_overlay(self):
        if not os.path.exists(SETTINGS_PATH):
            return
        try:
            with open(SETTINGS_PATH, "r", encoding="utf-8") as f:
                overlay = json.load(f)
            self.config.update(overlay)
            logger.info("[Config] Applied persistent settings overlay.")
        except Exception as e:
            logger.error(f"[Config] Failed to apply settings overlay: {e}")
    ...
    def save_settings(self):
        try:
            data = {
                ...
                "swarm_sync_enabled": self.swarm_sync_enabled,
                "honeypot_enabled": self.honeypot_enabled,
                "agentic_triage_enabled": self.agentic_triage_enabled,
                "zero_trust_enforcement_enabled": self.zero_trust_enforcement_enabled
            }
            with open(SETTINGS_PATH, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2)
            logger.info("[Config] Persistent settings saved.")
        except Exception as e:
...
class GuardianDB:
    ...
    def _init_schema(self):
        with db_lock:
            ...
            add_column("devices", "zero_trust_state", "TEXT")
            add_column("events", "severity", "INTEGER")
            add_column("events", "mitre_technique", "TEXT")
            add_column("events", "triage_label", "TEXT")
    ...
    def add_event(self, event: dict):
        with db_lock:
            ...
            ts = int(time.time())
            cur.execute("""
                INSERT INTO events (ts, type, src_ip, dst_ip, summary, severity, mitre_technique, triage_label)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ts,
                event.get("type", ""),
    ...
    def get_recent_events(self, limit=100):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("""
                SELECT ts, type, src_ip, dst_ip, summary, severity, mitre_technique, triage_label
                FROM events
                ORDER BY ts DESC
                LIMIT ?
            """, (limit,))
    ...
    def upsert_device(self, ip: str, role: str, dev_type: str, risk: int,
                      ...
                      zero_trust_state: str = "unknown"):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("""
                INSERT INTO devices (ip, role, type, risk, os_hint, ttl_hint, mac_vendor, notes, identity, trust_score, zero_trust_state)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(ip) DO UPDATE SET
                    ...
                    zero_trust_state=excluded.zero_trust_state
            """, (ip, role, dev_type, risk, os_hint, ttl_hint, mac_vendor, notes, identity, trust_score, zero_trust_state))
            self.conn.commit()
    ...
    def update_zero_trust_state(self, ip: str, state: str):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("UPDATE devices SET zero_trust_state = ? WHERE ip = ?", (state, ip))
            self.conn.commit()

    def get_devices(self):
        with db_lock:
            cur = self.conn.cursor()
            cur.execute("SELECT ip, role, type, risk, os_hint, ttl_hint, mac_vendor, notes, identity, trust_score, zero_trust_state FROM devices")
            rows = cur.fetchall()
        devices = {}
        for ip, role, dev_type, risk, os_hint, ttl_hint, mac_vendor, notes, identity, trust_score, zt_state in rows:
            devices[ip] = {
                "role": role,
                "type": dev_type,
                "risk": risk,
                ...
                "zero_trust_state": zt_state or "unknown"
            }
        return devices
...
class MITREMapper:
    def __init__(self):
        self.mapping = {
            "honeypot_hit_ssh": "T1110",
            "honeypot_hit_http": "T1190",
            "honeypot_hit_rdp": "T1076",
            "intrusion": "T1046",
            "per_ip_anomaly": "T1082",
            "deep_anomaly": "T1204",
            "rogue_ai_suspected": "T1059"
        }
...
class AgenticTriageEngine:
    ...
    def triage(self, event: dict) -> str:
        ...
        if sev >= 80 or etype in ("intrusion", "replay_intrusion"):
            return "critical"
        if "honeypot_hit" in etype:
            return "high"
        if "anomaly" in etype:
            return "medium"
...
class NetworkDiscoveryEngine:
        ...
        try:
            ...
            for c in conns:
                if c.raddr and c.raddr.ip:
                    ...
                    trust_score = 50
                    self.db.upsert_device(ip, role, dev_type, risk, os_hint, ttl_hint, mac_vendor, notes, identity, trust_score, "unknown")
        except Exception as e:
    ...
    def mark_critical_node(self):
        ...
        if CRITICAL_NODE_IP:
            ...
            self.db.upsert_device(CRITICAL_NODE_IP, "CRITICAL_NODE", "pc", 0, "Windows (local)", 64, "LocalHost", "Guardian Host", identity, 80, "trusted")
...
class ZeroTrustEngine:
    def __init__(self, db: GuardianDB, config: GuardianConfig):
        self.db = db
        self.config = config

    def evaluate_device(self, ip: str):
        devices = self.db.get_devices()
        info = devices.get(ip)
        if not info:
            return "unknown"
        risk = info.get("risk", 0)
        trust = info.get("trust_score", 50)
        if risk >= 80:
            state = "blocked"
        elif trust >= 80 and risk < 40:
            state = "trusted"
        else:
            state = "monitor"
        self.db.update_zero_trust_state(ip, state)
        return state

class ThreatMatrix:
    def __init__(self, db: GuardianDB, mitre_mapper: MITREMapper, triage_engine: AgenticTriageEngine):
        self.db = db
    ...
    def add_event(self, event: dict):
        ...
        mitre = self.mitre_mapper.map_event(event_type)
        triage_label = self.triage_engine.triage(event)
        event["mitre_technique"] = mitre
        event["triage_label"] = triage_label
        self.db.add_event(event)
        logger.info(f"[ThreatMatrix] Event ({triage_label}, {mitre}): {event.get('summary', 'unknown')}")

    def get_recent_events(self, limit=100):
        return self.db.get_recent_events(limit)

class FirewallManager:
    def __init__(self, config: GuardianConfig, db: GuardianDB, zero_trust: ZeroTrustEngine):
        self.config = config
        self.db = db
    ...
    def block_ip(self, ip: str, manual=False):
        ...
        if not self.config.firewall_block_enabled or self.config.manual_override_enabled:
            ...
            return
        zt_state = self.zero_trust.evaluate_device(ip)
        if zt_state == "trusted" and not manual:
            logger.info(f"[Firewall] Zero-trust: {ip} trusted, skipping block.")
            return
...
class NDISLayerStub:
    def __init__(self, config: GuardianConfig):
        self.config = config
        self.enabled = config.ndis_layer_enabled

    def start(self):
        if not self.enabled:
            logger.info("[NDIS] NDIS interception layer disabled (stub).")
            return
        logger.info("[NDIS] NDIS interception layer would start here (stub-safe, no kernel code).")

class KernelTelemetrySuppressionStub:
    def __init__(self, config: GuardianConfig):
        self.config = config
    ...
    def apply(self):
        if not self.enabled:
            logger.info("[Kernel] Telemetry suppression disabled (stub).")
            return
        logger.info("[Kernel] Telemetry suppression would be applied here (stub-safe, no kernel code).")
        ...
        try:
            if IP in pkt:
                ...
                if malicious or anomalous_ml or anomalous_nn or suricata_matches:
                    ...
                    self.zero_trust.evaluate_device(src)
        except Exception as e:
    ...
    def replay_packet(self, src_ip: str, dst_ip: str, payload: bytes, meta: dict):
        try:
            ...
            if malicious or anomalous_ml or anomalous_nn or suricata_matches:
                ...
                self.zero_trust.evaluate_device(src_ip)
        except Exception as e:
...
class Honeypot(threading.Thread):
    ...
    def _cowrie_style_ssh_banner(self):
        return b"SSH-2.0-OpenSSH_8.9p1 Cowrie-Guardian\r\n"

    def _cowrie_style_http_response(self):
        body = b"<html><body><h1>It works.</h1><p>Guardian Cowrie-style honeypot.</p></body></html>"
        headers = (
            b"HTTP/1.1 200 OK\r\n"
            b"Server: GuardianCowrie\r\n"
        ...
        while self.running:
            for s, proto, port in list(self.sockets):
                try:
                    ...
                    try:
                        if proto == "SSH":
                            banner = self._cowrie_style_ssh_banner()
                        elif proto == "HTTP":
                            banner = self._cowrie_style_http_response()
                        else:
...
class IPControlDialog(QtWidgets.QDialog):
    def __init__(self, ip: str, db: GuardianDB, firewall: FirewallManager, pcap_mgr: PacketCaptureManager, parent=None):
        ...
        zt_state = info.get("zero_trust_state", "unknown")
        ...
        layout.addWidget(QtWidgets.QLabel(f"Zero-Trust State: {zt_state}"))
...
class GuardianGUI:
    ...
    def launch(self):
        ...
        threats_group = QtWidgets.QGroupBox("Threats & Alerts")
        threats_layout = QtWidgets.QVBoxLayout()
        ...
        threats_layout.addLayout(indicator_layout)
        ...
        status_layout.addWidget(self.swarm_label)

        self.honeypot_label = QtWidgets.QLabel("Honeypot Hits: 0")
        status_layout.addWidget(self.honeypot_label)

        self.override_label = QtWidgets.QLabel("Firewall Auto-Block: ON")
        status_layout.addWidget(self.override_label)
        ...
        devices_group = QtWidgets.QGroupBox("Devices (double-click IP for control panel)")
        ...
        events_group = QtWidgets.QGroupBox("Threat Events")
        ...
        firewall_group = QtWidgets.QGroupBox("Firewall Rules (raw, top 50)")
        firewall_layout = QtWidgets.QVBoxLayout()
        ...
        firewall_layout.addWidget(self.firewall_box)
    ...
    def _start_gui_timer(self, events_box):
        ...
        def refresh():
            ...
            events_box.setPlainText("\n".join(
                [f"[{e.get('severity',0)}][{e.get('triage_label','')}]({e.get('mitre_technique','')}) {e.get('summary', '')}" for e in events]
            ))
            if events:
...
def main():
    ...
    triage_engine = AgenticTriageEngine(db)
    net_engine = NetworkDiscoveryEngine(db)
    threat_matrix = ThreatMatrix(db, mitre_mapper, triage_engine)
    alerts = AlertEngine(config)
    integrity = IntegrityManager(config)
    threat_intel = ThreatIntel(config, db)
    zero_trust = ZeroTrustEngine(db, config)
    firewall = FirewallManager(config, db, zero_trust)
    ...
    ndis_layer = NDISLayerStub(config)
    kernel_suppressor = KernelTelemetrySuppressionStub(config)

    ndis_layer.start()
    kernel_suppressor.apply()
    ...
    packet_inspector = PacketInspector(config, threat_intel, firewall,
                                       ...
                                       neural, pcap_mgr, suricata_engine, mesh_router, zero_trust)
