...
# High-Contrast SOC Fluent Ribbon GUI (Home / Events / Deception / System)
...
# ============================================================
# GUI (High-Contrast SOC Fluent Ribbon)
# ============================================================
...
def start_gui():
    ...
    root = tk.Tk()
    root.title("Codex Honeypot Godmode v12 - SOC Console")
    root.geometry("1100x700")

    # High-contrast SOC palette
    bg_main = "#101820"
    bg_ribbon = "#1B2838"
    bg_group = "#141E2A"
    fg_text = "#E5E5E5"
    ...
    style.configure("Ribbon.TFrame", background=bg_ribbon)
    style.configure("RibbonTab.TButton",
                    background=bg_ribbon,
                    foreground=fg_text,
                    padding=6)
    style.map("RibbonTab.TButton",
              background=[("active", "#223447")])
    style.configure("RibbonGroup.TLabelframe",
                    background=bg_group,
                    foreground=fg_text)
    style.configure("RibbonGroup.TLabelframe.Label",
                    background=bg_group,
                    foreground=fg_text)
    ...
    style.map("RibbonButton.TButton",
              background=[("active", "#223447")])
    style.configure("Content.TFrame", background=bg_main)
    style.configure("TLabel", background=bg_main, foreground=fg_text)
    style.configure("TListbox", background="#0D141C", foreground=fg_text)
    ...
    # Ribbon container
    ribbon_container = ttk.Frame(root, style="Ribbon.TFrame")
    ribbon_container.pack(side="top", fill="x")

    # Ribbon tabs row
    ribbon_tabs_frame = ttk.Frame(ribbon_container, style="Ribbon.TFrame")
    ribbon_tabs_frame.pack(side="top", fill="x")

    # Ribbon content frame (groups)
    ribbon_content_frame = ttk.Frame(ribbon_container, style="Ribbon.TFrame")
    ribbon_content_frame.pack(side="top", fill="x")

    # Collapsible ribbon
    ribbon_visible = {"value": True}

    def toggle_ribbon():
        if ribbon_visible["value"]:
            ribbon_content_frame.pack_forget()
            ribbon_visible["value"] = False
            collapse_btn.config(text="Show Ribbon")
        else:
            ribbon_content_frame.pack(side="top", fill="x")
            ribbon_visible["value"] = True
            collapse_btn.config(text="Hide Ribbon")

    collapse_btn = ttk.Button(root, text="Hide Ribbon", style="RibbonButton.TButton", command=toggle_ribbon)
    collapse_btn.pack(side="top", fill="x", padx=4, pady=4)

    # Ribbon tab switching
    current_ribbon_tab = {"name": "Home"}

    def set_ribbon_tab(name: str):
        current_ribbon_tab["name"] = name
        build_ribbon_content()

    ttk.Button(ribbon_tabs_frame, text="Home", style="RibbonTab.TButton",
               command=lambda: set_ribbon_tab("Home")).pack(side="left", padx=4, pady=2)
    ttk.Button(ribbon_tabs_frame, text="Events", style="RibbonTab.TButton",
               command=lambda: set_ribbon_tab("Events")).pack(side="left", padx=4, pady=2)
    ttk.Button(ribbon_tabs_frame, text="Deception", style="RibbonTab.TButton",
               command=lambda: set_ribbon_tab("Deception")).pack(side="left", padx=4, pady=2)
    ttk.Button(ribbon_tabs_frame, text="System", style="RibbonTab.TButton",
               command=lambda: set_ribbon_tab("System")).pack(side="left", padx=4, pady=2)

    # Ribbon content builder
    def build_ribbon_content():
        for child in ribbon_content_frame.winfo_children():
            child.destroy()

        tab = current_ribbon_tab["name"]

        if tab == "Home":
            # Mode group
            mode_group = ttk.Labelframe(ribbon_content_frame, text="Mode", style="RibbonGroup.TLabelframe")
            mode_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ttk.Radiobutton(mode_group, text="Soft (A)", value="A", variable=mode_var,
                            command=lambda v=mode_var: set_honeypot_mode(v.get())).grid(row=0, column=0, sticky="w", padx=4, pady=2)
            ttk.Radiobutton(mode_group, text="Hard (B)", value="B", variable=mode_var,
                            command=lambda v=mode_var: set_honeypot_mode(v.get())).grid(row=1, column=0, sticky="w", padx=4, pady=2)
            ttk.Radiobutton(mode_group, text="Hybrid AI (C)", value="C", variable=mode_var,
                            command=lambda v=mode_var: set_honeypot_mode(v.get())).grid(row=2, column=0, sticky="w", padx=4, pady=2)

            # Scope group
            scope_group = ttk.Labelframe(ribbon_content_frame, text="Scope", style="RibbonGroup.TLabelframe")
            scope_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ttk.Radiobutton(scope_group, text="wuiconwindow only (1)", value=1, variable=scope_var,
                            command=lambda v=scope_var: set_window_scope_mode(v.get())).grid(row=0, column=0, sticky="w", padx=4, pady=2)
            ttk.Radiobutton(scope_group, text="Unknown classes (2)", value=2, variable=scope_var,
                            command=lambda v=scope_var: set_window_scope_mode(v.get())).grid(row=1, column=0, sticky="w", padx=4, pady=2)
            ttk.Radiobutton(scope_group, text="AI decide (3)", value=3, variable=scope_var,
                            command=lambda v=scope_var: set_window_scope_mode(v.get())).grid(row=2, column=0, sticky="w", padx=4, pady=2)

            # Toggles group
            toggles_group = ttk.Labelframe(ribbon_content_frame, text="Subsystems", style="RibbonGroup.TLabelframe")
            toggles_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ...
            for i, (label, key) in enumerate(toggles):
                btn = ttk.Checkbutton(toggles_group, text=label,
                                      command=lambda k=key: toggle_flag(k))
                if config.get(key, True):
                    btn.state(["selected"])
                btn.grid(row=i, column=0, sticky="w", padx=4, pady=2)

            # Tools group
            tools_group = ttk.Labelframe(ribbon_content_frame, text="Tools", style="RibbonGroup.TLabelframe")
            tools_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ttk.Button(tools_group, text="Simulate Events", style="RibbonButton.TButton",
                       command=simulate_window_events).grid(row=0, column=0, padx=4, pady=2, sticky="w")
            ttk.Button(tools_group, text="Export Forensics", style="RibbonButton.TButton",
                       command=forensic_export).grid(row=1, column=0, padx=4, pady=2, sticky="w")

        elif tab == "Events":
            events_group = ttk.Labelframe(ribbon_content_frame, text="Events View", style="RibbonGroup.TLabelframe")
            events_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ttk.Label(events_group, text="Use bottom controls to refresh and auto-nuke.", foreground=accent_blue).grid(
                row=0, column=0, sticky="w", padx=4, pady=2)

        elif tab == "Deception":
            dec_group = ttk.Labelframe(ribbon_content_frame, text="Deception Controls", style="RibbonGroup.TLabelframe")
            dec_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ttk.Button(dec_group, text="Generate Deception", style="RibbonButton.TButton",
                       command=lambda: (fake_net_interfaces(), fake_windows_logs(), fake_linux_logs())).grid(
                row=0, column=0, padx=4, pady=2, sticky="w")
            ttk.Button(dec_group, text="Clear Deception", style="RibbonButton.TButton",
                       command=lambda: clear_deception_global()).grid(
                row=1, column=0, padx=4, pady=2, sticky="w")

        elif tab == "System":
            sys_group = ttk.Labelframe(ribbon_content_frame, text="System Info", style="RibbonGroup.TLabelframe")
            sys_group.pack(side="left", padx=8, pady=6, ipadx=4, ipady=4)
            ttk.Label(sys_group, text=f"Node ID: {config.get('node_id', 'node-1')}").grid(
                row=0, column=0, sticky="w", padx=4, pady=2)
            ttk.Label(sys_group, text=f"Peers: {config.get('swarm_peers', [])}").grid(
                row=1, column=0, sticky="w", padx=4, pady=2)

    def clear_deception_global():
        state["deception_assets"] = []
        save_json(STATE_PATH, state)
        print("[Deception] Cleared all assets.")
    ...
    # Left navigation (SOC style)
    nav_frame = ttk.Frame(main_frame, style="Main.TFrame")
    nav_frame.pack(side="left", fill="y", padx=4, pady=4)

    content_frame = ttk.Frame(main_frame, style="Content.TFrame")
    content_frame.pack(side="left", fill="both", expand=True, padx=4, pady=4)

    events_tab = ttk.Frame(content_frame, style="Content.TFrame")
    deception_tab = ttk.Frame(content_frame, style="Content.TFrame")
    system_tab = ttk.Frame(content_frame, style="Content.TFrame")
    ...
    ttk.Button(nav_frame, text="Events", style="RibbonButton.TButton",
               command=lambda: show_tab(events_tab)).pack(fill="x", pady=2)
    ttk.Button(nav_frame, text="Deception", style="RibbonButton.TButton",
               command=lambda: show_tab(deception_tab)).pack(fill="x", pady=2)
    ttk.Button(nav_frame, text="System", style="RibbonButton.TButton",
               command=lambda: show_tab(system_tab)).pack(fill="x", pady=2)

    # ===== Events Tab =====
    trapped_frame = ttk.LabelFrame(events_tab, text="Trapped Windows", style="RibbonGroup.TLabelframe")
    trapped_frame.pack(fill="x", padx=2, pady=2)
    trapped_list = tk.Listbox(trapped_frame, height=5, bg="#0D141C", fg=fg_text)
    trapped_list.pack(fill="x", expand=False)

    suricata_frame = ttk.LabelFrame(events_tab, text="Suricata Events", style="RibbonGroup.TLabelframe")
    suricata_frame.pack(fill="x", padx=2, pady=2)
    suricata_list = tk.Listbox(suricata_frame, height=5, bg="#0D141C", fg=fg_text)
    suricata_list.pack(fill="x", expand=False)

    process_frame = ttk.LabelFrame(events_tab, text="Process Events", style="RibbonGroup.TLabelframe")
    process_frame.pack(fill="x", padx=2, pady=2)
    process_list = tk.Listbox(process_frame, height=5, bg="#0D141C", fg=fg_text)
    process_list.pack(fill="x", expand=False)

    proto_frame = ttk.LabelFrame(events_tab, text="Protocol Honeypot Events", style="RibbonGroup.TLabelframe")
    proto_frame.pack(fill="x", padx=2, pady=2)
    proto_list = tk.Listbox(proto_frame, height=5, bg="#0D141C", fg=fg_text)
    proto_list.pack(fill="x", expand=False)

    ml_frame = ttk.LabelFrame(events_tab, text="ML Threats", style="RibbonGroup.TLabelframe")
    ml_frame.pack(fill="x", padx=2, pady=2)
    ml_list = tk.Listbox(ml_frame, height=5, bg="#0D141C", fg=fg_text)
    ml_list.pack(fill="x", expand=False)

    # ===== Deception Tab =====
    deception_frame = ttk.LabelFrame(deception_tab, text="Deception Assets (AI-driven)", style="RibbonGroup.TLabelframe")
    deception_frame.pack(fill="both", expand=True, padx=2, pady=2)
    deception_list = tk.Listbox(deception_frame, height=8, bg="#0D141C", fg=fg_text)
    deception_list.pack(fill="both", expand=True)

    # ===== System Tab =====
    swarm_frame = ttk.LabelFrame(system_tab, text="Swarm Messages", style="RibbonGroup.TLabelframe")
    swarm_frame.pack(fill="x", padx=2, pady=2)
    swarm_list = tk.Listbox(swarm_frame, height=5, bg="#0D141C", fg=fg_text)
    swarm_list.pack(fill="x", expand=False)

    persona_frame = ttk.LabelFrame(system_tab, text="Persona Events", style="RibbonGroup.TLabelframe")
    persona_frame.pack(fill="x", padx=2, pady=2)
    persona_list = tk.Listbox(persona_frame, height=5, bg="#0D141C", fg=fg_text)
    persona_list.pack(fill="x", expand=False)

    node_frame = ttk.LabelFrame(system_tab, text="Node / Peers", style="RibbonGroup.TLabelframe")
    ...
    ttk.Button(control_frame, text="Simulate Events", style="RibbonButton.TButton",
               command=simulate_btn).pack(side="left", padx=4)
    ttk.Button(control_frame, text="Auto-Nuke Selected", style="RibbonButton.TButton",
               command=auto_nuke_selected).pack(side="left", padx=4)
    ttk.Button(control_frame, text="Refresh", style="RibbonButton.TButton",
               command=refresh_views).pack(side="left", padx=4)
