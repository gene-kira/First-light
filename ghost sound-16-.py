...
# -*- coding: utf-8 -*-
#
# Codex Control Console v12 + Security Bridge v11
...
# Maximum Protection Patch: Steam / Epic games fully allowlisted (never blocked)
...
def is_allowlisted(name_lower):
    ...
    # Maximum Protection Patch: Steam / Epic games and typical game executables
    if (
        "steam" in name_lower or
        "epic" in name_lower or
        "shipping" in name_lower or
        "win64" in name_lower or
        "win32" in name_lower or
        "eosoverlay" in name_lower or
        "easyanticheat" in name_lower or
        "battleye" in name_lower
    ):
        return True
...
# -----------------------------
# Codex Control Console GUI (merged layout)
# -----------------------------
class CodexControlConsole(tk.Tk):
    def __init__(self):
        ...
        # === WINDOW SETUP ===
        self.title("Codex Control Console v12 — Security Bridge v11")
        self.configure(bg="#101010")

        screen_w = self.winfo_screenwidth()
        screen_h = self.winfo_screenheight()
        target_w = max(1600, int(screen_w * 0.92))
        target_h = max(900, int(screen_h * 0.80))
    ...
    def _build_layout(self):
        ...
        self.audio_lockdown_button.grid(row=0, column=2, padx=(0, 6))
        ...
        mode_combo = ttk.Combobox(row2, textvariable=self.mode_var, values=["AGGRESSIVE", "SILENT"], width=12)
        ...
        profile_combo.grid(row=0, column=3, padx=(0, 8))
        ...
        status_bar.grid(row=2, column=0, sticky="ew", padx=10, pady=3)
...
# -----------------------------
def main():
    log_event("INFO", "Codex Control Console v12 + Security Bridge v11 (Max Protection) started.")
    start_honeypot()
