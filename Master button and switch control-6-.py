#!/usr/bin/env python
# Codex Purge Shell v4 (Safe Edition) — Win11 Security Bridge & Adjustment Console
# - Suricata v6 (file-tail)
# - GPU DL stub threat scoring
# - Agentic reasoning (suggest-only)
# - Honeypot (logical)
# - GPO/Telemetry (safe subset)
# - Event Bus
...
# =========================
# Group Policy / Telemetry suppression (safe subset)
# =========================
...
# =========================
# Snapshots (firewall + registry + meta)
# =========================
    ...
    if os.path.isfile(meta):
        try:
            with open(meta, "r", encoding="utf-8") as f:
                data = json.load(f)
            print("[Codex Purge Shell v4] Restored meta snapshot:", data.get("summary", "no summary"))
        except Exception:
            pass

# =========================
# Suricata v6 log / event ingestion (file-tail)
# =========================
...
class ThreatScorer:
    ...
    def _load_model(self):
        if torch is not None:
            self.model = "codex_dummy_model_v4"
        else:
            self.model = None
...
# =========================
# Honeypot emulation (logical only)
# =========================
...
# =========================
# Agentic reasoning engine (defensive, suggestion-only)
# =========================
...
class WindowsAdjustmentEngine:
    """
    Safe subset of Windows adjustments exposed via GUI:
    - Firewall (rules via netsh)
    - Services (start/stop, change startup type)
    - Scheduled Tasks (enable/disable)
    - Telemetry/GPO (safe subset)
    - Defender (safe toggles via PowerShell)
    - Network (adapter enable/disable via PowerShell)
    - Power (power plan via PowerShell)
    - Windows Update (pause/resume via PowerShell)
    ...
    # ---- Windows Update helpers (safe subset) ----
...
# =========================
# Main GUI / Codex Purge Shell v4
# =========================

class CodexSettingsGuard:
    def __init__(self, root):
        self.root = root
        self.root.title("Codex Purge Shell v4 (Safe Edition) — Win11 Security Bridge & Adjustment Console")
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            top_frame,
            text="Codex Purge Shell v4 (Safe Edition) — Windows 11 Security Bridge & Adjustment Console",
            font=("Arial", 18)
        )
        ...
        # Left: Alerts + Settings Editor + Adjustment Tabs
        self.alert_box = tk.Text(left_frame, height=15, width=80, bg="#1e1e1e", fg="#00ff00")
        self.alert_box.pack(fill="x", padx=5, pady=5)
        self.log_alert("Codex Purge Shell v4 (Safe Edition) started with auto-elevation and advanced subsystems.")

        self.editor_panel = SettingsEditorPanel(left_frame, self.log_alert, self.adjustment_engine)

        adj_notebook = ttk.Notebook(left_frame)
        ...
        # Right: Threat Matrix + Suricata + Overlays
        ...
        sur_frame = ttk.LabelFrame(right_frame, text="Suricata v6 Event Stream (Mapped to MITRE)")
